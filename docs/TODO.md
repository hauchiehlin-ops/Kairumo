...
### 頁面錄音物件的殘留缺口（2026-09-16 新增）

| ID | 內容 | 卡在什麼 / 要做什麼 |
|---|---|---|
| S-40 | **匯出與縮圖畫不出錄音卡片。** `PageThumbnailRenderer` 與 PDF／SVG 匯出還不認得 `audioAttachments`，所以卡片在畫布上看得到、匯出的檔案裡沒有。 | 純軟體。要在算繪端補一個「錄音卡片」的畫法（靜態外觀即可，匯出的東西本來就不能播）。判定：同一頁匯出的 PDF 上看得到卡片，位置與畫布一致。 |
| S-41 | **Android 沒有「從首頁把錄音插進某一本筆記」。** Apple 端的 `RecordingToNotebookSheet` 可以挑筆記本與頁次；Android 的錄音檔住在各自的套件裡，跨本插入要先把音檔複製過去。 | 純軟體，但要先決定「複製一份」還是「跨套件引用」。**不能直接引用** —— 套件是同步的單位，引用會讓另一台裝置拿到一張播不出來的卡片。 |
| S-43 | **Android 的縮圖仍然只有筆畫與底紋。** 它走核心的 `export_page_png`，而 `pdf` feature 在 Android 是關的，落到純 Rust 的 fallback —— 那一條只畫底紋與筆畫，不含文字方塊、表格、圖表與形狀。Apple 端今天已經補齊（`PageThumbnailRenderer`），兩邊的縮圖因此**不一樣**。 | 純軟體。要在 `padnote-render` 補上區塊的算繪，讓 fallback 與 PDF 路徑畫出同一份內容。判定：同一本筆記在兩個平台的第一頁縮圖看起來一致。 |
| S-44 | **Mac Catalyst 上的表格面板沒有實機驗證。** 修法（`Grid` → `VStack`/`HStack`）已在 iPad 上驗過，但原始故障只在 Catalyst 出現，而背景自動化點不到那張筆記卡片（它被曝露成 AXPopUpButton，背景點擊會被擋下）。 | 人工：在 Mac 上開一本筆記 → 打字模式 → 更多 → 表格，確認格子是完整的列欄而不是只有一格。 |
| S-42 | **錄音長度沒有寫進卡片。** Apple 從錄音索引拿得到 `durationSeconds`；Android 掃檔案（`RecordingIndex` 刻意不算長度），所以插進去是 `00:00`。 | 純軟體。走核心讀 Opus 封包算時長，或在錄音結束時把長度寫進套件中繼資料。 |
