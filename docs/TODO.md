...
### S-261b. 工具列位置與文字標籤還沒有開關

`placement`（上／下／左／右／收合）與 `show_labels` 都在核心的設定裡，也
同步得動，但兩端都還沒有 UI —— 理由見 S-261。

要做的話兩邊都得動真格：Apple 要讓工具列真的能擺到左右（現在是固定那一條
`ViewThatFits`），Android 要讓晶片有純圖示型態。只做一邊的話，同一個設定
在另一台裝置上按了沒反應 —— 比沒有這個設定更糟。
...
- **要做**：
  1. **開啟加密筆記本時要問密碼**。兩個平台的**建立**流程都好了，
     但「打開一本已加密的筆記」還沒有解鎖畫面 ——
     目前會走到「這個套件已加密，需要先解鎖」這個錯誤。
     這是接下來最該補的一塊。
  2. **Apple 的本機工作副本仍是明文。**
  ...
  3. **錄音沒有加密**（`crypto_encryption_scope().covers_recordings == false`）。
     錄音是邊錄邊寫的 Ogg 串流，包成框架就不再是播放器打得開的檔案，
     而「可以用 VLC 打開自己的錄音」是明講過的承諾（H4）。
     要嘛接受破壞它、要嘛把錄音排除在加密之外並永遠講清楚 ——
     這個取捨還沒有人決定。
