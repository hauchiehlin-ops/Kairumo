...
### 錄音物件與跨平台版面的殘留缺口（2026-09-16）

| ID | 內容 | 卡在什麼 / 要做什麼 |
|---|---|---|
| S-57 | **Android 的縮圖只有筆畫與底紋。** 它走核心的 `export_page_png`，而 `pdf` feature 在 Android 是關的，落到 `padnote-export::to_png` —— 那一條只畫底紋與筆畫，不含文字方塊、表格、圖表與形狀。Apple 端已補齊（`PageThumbnailRenderer`），兩邊的縮圖因此**不一樣**。首頁的格狀清單目前靠「一片空白就改畫佔位」繞過去。 | 純軟體。要在 `padnote-export::to_png` 補上區塊的算繪（文字要字型、圖片要解碼），讓 fallback 與 PDF 路徑畫出同一份內容。判定：同一本筆記在兩個平台的第一頁縮圖看起來一致。 |
| S-58 | **Mac Catalyst 上的表格面板沒有實機驗證。** 修法（`Grid` → `VStack`/`HStack`）已在 iPad 上驗過，但原始故障只在 Catalyst 出現，而背景自動化點不到那張筆記卡片（它被曝露成 AXPopUpButton，背景點擊會被擋下）。 | 人工：在 Mac 上開一本筆記 → 打字模式 → 更多 → 表格，確認格子是完整的列欄而不是只有一格。 |
| S-59 | **Android 編輯器的工具列仍然是一整片 FilterChip。** 首頁已經比照 Apple 重排，編輯器還沒 —— Apple 是「模式切換 + 目前模式的工具列」兩層，Android 是所有東西攤在同一個 FlowRow 裡，在手機上會佔掉半個螢幕。 | 純軟體。拆成兩列：上排模式與分頁導覽，下排目前模式的工具。 |
