...
| S-60 | **縮圖上的文字是灰條，不是字。** 沒有 PDFium 時的 fallback（Android 第一版走這裡）畫不出字形 —— 要畫真字得內嵌一份字型，而中文字型好幾 MB 且授權要另外拍板。位置、行寬、行數、行高全部照實算，只有字形是抽象的。 | **需要人拍板**：要不要內嵌字型、內嵌哪一份（授權必須相容 `deny.toml` 的立場）。定了之後在 `padnote-export::image` 接上光柵化即可。 |
