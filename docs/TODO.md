...
| ID | 項目 | 判定條件 |
|---|---|---|
| C-01 | `CanvasRepresentable` 拆出不自帶捲動的變體（`isScrollEnabled = false`） | 單頁模式行為完全不變 |
| C-02 | 連續容器：`LazyVStack` 疊 N 頁，每頁自己的畫布與物件層 | 100 頁的筆記捲動不卡、記憶體不爆 |
| C-03 | 每頁各自載入／儲存筆跡 | 在第 7 頁寫字只動第 7 頁的檔案 |
| C-04 | 「目前頁」改由最可見的那一頁決定（插入物件要知道插到哪一頁） | 捲到第 3 頁插圖，圖在第 3 頁 |
| C-05 | 協同、掌拒、套索逐頁接上 | 兩台裝置在不同頁同時寫，各自落在正確的頁 |
| C-06 | 頁尾的模式切換（整頁／連續），選擇要記住 | 重開筆記本維持上次的模式 |
| C-07 | Android 同樣一套 | 同一本筆記兩個平台捲起來一致 |

**風險**：C-02 與 C-05 動到的是這個 App 最核心、最沒有本錢壞掉的一段。
建議獨立分支進行，單頁模式的行為要有測試釘住才動手。

**目前狀態**：尚未開始。整頁模式是現況的預設，不需要改；
缺的是連續模式本身與那個切換開關。
