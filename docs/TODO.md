...
### H-MILESTONE. 里程碑快照的跨裝置驗證（S-99 已全部實作）

- **已驗證（本機）**：核心的四項端到端測試（文字與筆跡一起回到同一刀、
  還原本身可以反悔、還原之後寫的東西不會被吃掉、清單不會縮短）、
  壓實不會移動里程碑的那一項（把界線拿掉它就會紅），
  以及兩端各自編得過、面板開得起來。
- **還沒驗證（需要兩台真機 + 同一個雲端帳號）**：
  1. **iPad 建的里程碑，Android 看不看得到。** 快照是 oplog 裡的一筆，
     理論上跟著同步走；但這條路沒有在真的 Google Drive 上跑過。
  2. **一邊還原、另一邊同時在寫。** 遮蔽區間用的是 `upto` 上界，
     設計上不會吃掉對方**之後**寫的東西 —— 要在真的併發下確認。
  3. **壓實界線的傳播。** 裝置 B 的 manifest 靠 `absorb_milestone_barriers`
     在開套件時從 oplog 補齊。如果 B 在還沒同步到那筆里程碑之前就壓實了
     自己的碎檔，A 的還原點會不會跑掉 —— 這一條**我不確定**，
     要實際造出那個時序來看。
- **驗收**：三條都跑過，而且第 3 條若真的會壞，要在核心補一層保護
  （例如壓實時保留最近 N 個碎檔不併）。

### H-CRYPTO-2. 套件加密：核心與 Apple 已完成，Android 介面與錄音待補
