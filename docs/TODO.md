...
- **已完成**（五個決策依 2026-09-22 的討論拍板，記在 `ffi_crypto.rs` 的模組說明）：
  ...
  - Apple 的三步啟用流程（設密碼 → 抄復原碼 → **把復原碼輸回來**）
- **要做**：
  1. **Android 的啟用流程**。核心的 FFI 已經開好，缺的是畫面。
     在補上之前，Android 開不了加密的筆記本 —— 而同步會把它傳過去。
     **這是目前最該補的一塊**：使用者在 iPad 上加密，Android 上看到一本
     打不開的筆記，而且訊息不會告訴他為什麼。
  2. **錄音沒有加密**（`crypto_encryption_scope().covers_recordings == false`）。
     錄音是邊錄邊寫的 Ogg 串流，包成框架就不再是播放器打得開的檔案，
     而「可以用 VLC 打開自己的錄音」是明講過的承諾（H4）。
     要嘛接受破壞它、要嘛把錄音排除在加密之外並永遠講清楚 ——
     這個取捨還沒有人決定。
  3. **解鎖狀態的生命週期**。目前 `FfiUnlockedNotebook` 拿在呼叫端手上，
     沒有「閒置多久自動上鎖」也沒有「進背景就鎖」。
  4. 實機：Argon2id 在低階 Android 機上要跑多久（64 MiB 記憶體成本）。
