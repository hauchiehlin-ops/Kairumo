...
| ID | 項目 | 判定條件 |
|---|---|---|
| P-01 | 畫布改用固定頁面座標空間，縮放至視窗寬度 | 同一則筆記在 iPhone 與 Mac 上物件位置相同 |
| P-02 | 畫布四周畫出頁面界線（與可列印區一致） | 界線位置 = 匯出邊界 |
| P-03 | 移除「延長本頁」，內容超出時自動新增下一頁 | 寫到頁尾會自己換頁，不會有半個物件跨頁 |
| P-04 | 既有過長頁面的遷移（依 1132pt 重新分頁） | 原檔不動、可回滾，逐頁比對內容未遺失 |
| P-05 | Android 同步採用同一個頁面模型 | 兩平台的頁面界線與分頁位置一致 |
| P-06 | 匯出／列印以頁面界線為準 | 內容不會超出界線 |
