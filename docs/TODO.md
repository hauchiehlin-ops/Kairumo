...
### S-261d. 「更多」選單裡的東西沒有任何執行期測試守著

做 S-261 時實測到的：**SwiftUI `Menu` 的內容不會出現在 XCUITest 的無障礙
樹裡**。點開 `editor.more` 之後掃整棵樹，回來的二十個識別碼全部是工具列上的
`editor.*`，一個選單項目都沒有。

這解釋了編輯器棘輪那 40 項 —— 它們不是「沒接上識別碼」，是**看不到**。
原本的註解寫的是「藏在選單／浮層裡」，現在知道更精確的原因了。

影響範圍不只自訂工具列：`editor.insert.*` 十四項、`editor.export.*` 四項，
加上這次的 `editor.customize_toolbar`，全部只有靜態對照閘門（掃原始碼字面值）
在守。**「這顆按鈕按下去有沒有反應」目前沒有任何自動檢查。**

可能的做法：
1. 給每個選單項目一條啟動變數的直達入口（自訂工具列這次就是這樣做的，
   `KAIRUMO_UITEST_TOOLBAR`）—— 有效，但十九個項目要十九條變數
2. 改用 `.popover` 或自繪的選單 —— 無障礙樹看得到，但那是為了測試改產品
3. 查清楚 `Menu` 的內容到底掛在哪個 element 底下（可能在別的 application
   element 或 springboard 層），找到之後一次解決全部

先記著。這是提案 ② 那類「畫面上有但按不到」的死角，而它正好是最大的一塊。
