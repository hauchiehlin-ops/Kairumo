...
- 狀態：**已關閉 —— 不再需要重新評估**（2026-09-22 更新；原為「已接受」2026-09-15）
- 決策：S-35 的處置
- 相關：`docs/TODO.md` §待實作、ADR-0002（筆畫不進 CRDT）、`apple/InkSpike/`

> **2026-09-22：Windows 已正式排除在支援範圍外。**
> 產品只出 iOS / iPadOS / macOS / Android。這份 ADR 原本留了一句
> 「有 Windows 版的時候再回來看」——那個時候不會來了。
>
> **這份文件不刪。** ADR 記的是「當初為什麼這樣決定」，刪掉它，
> 下一個問「為什麼不做 Windows 低延遲墨跡」的人就只能重新調查一次。
> 底下的技術結論（Compose MP Desktop 走 Skia/JVM 做不到 9ms）仍然成立，
> 只是現在它是一個**不需要回頭的**結論。
