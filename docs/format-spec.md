...
### 6.3 批次座標（`BatchOrigin`，op 類型 37）

每一批寫入的**第一筆**操作是 `BatchOrigin { lamport, device }`，
內容與檔名相同。

**為什麼要重複記一次**：壓實會把 `0002..0006` 併成一個叫 `0006` 的檔
（純位元組串接），併完之後只剩一個檔名。里程碑（§6.4）拿座標當「歷史上
的一刀」，若座標只存在檔名裡，被併進去的操作會全部宣稱自己是最大的那個
lamport —— 於是「回到 lamport 3 那一刻」會把那一刀**之前**就該留下來的
內容也一起收走。**那是資料遺失，而且沒有任何錯誤訊息。**

讀取端的規則：座標先取自檔名，遇到 `BatchOrigin` 就改用它帶的那一組，
直到下一個 `BatchOrigin`。沒有這一筆的舊檔案照樣讀得動。

⚠️ 因此 `min_reader_version` 的問題比過去更明顯：**新版寫出來的每一個
oplog 檔，舊讀取器都會在 `decode` 時撞上未知的 op 類型**。
見 `TODO.md` 的 H-OPLOG-COMPAT。
...
錄音住在**筆記本套件內**的 `media/audio/<uuid>.opus`，跟著套件一起同步
（`gdrive_sync_media`）。它不是一個獨立的同步單位，理由與 §7.3 相同：
獨立單位要重新處理刪除墓碑、命名衝突與孤兒清理，而跨單位「引用」檔案
會讓卡片與音檔在同步之後分家。

三條規則：

1. **格式固定 Ogg-Opus。** iOS 原生播不動它，所以 Apple 端走核心解碼
   （`padnote-audio::decoder`）。選 Opus 而不是 AAC 的理由是檔案大小：
   語音 32 kbps 一小時約 14 MB，而同步走的是使用者自己的雲端空間。
2. **`OpusHead` 的 pre-skip 必須等於編碼器的前視延遲**
   （`OpusEncoder::lookahead_48k()`）。填大了就會剪掉真正的聲音，
   而且是任何照規格實作的播放器都會剪。
3. **長度一律由核心算**（`ogg_opus_duration_us`）。兩個平台各自問系統
   API 的話，同一段錄音會顯示不同秒數，而使用者會以為同步壞了。

沒有指定筆記本的錄音落在「錄音收件匣」，那是一本 **id 寫死**的筆記本
（`recording_inbox_notebook_id()`）。寫死是為了讓兩台裝置收斂成同一本；
用隨機 id 的話每台一本，使用者會看到「錄音收件匣 2」「錄音收件匣 3」。
