# 開發日誌

> 反序排列（最新在上）。每個開發階段結束時追加一筆。
> 記錄**為什麼**這樣做，而不只是做了什麼 —— 「做了什麼」看 git log 就好。

## 2026-09-21（深夜）· 錄音納入同步：Apple 改走核心的錄音管線（R1–R6）

起點是一個問題：「錄音檔是否屬於同步範圍？」查下去的答案是
**規格上是，實作上只有 Android 是**。Apple 的錄音寫在
`Documents/Kairumo Record/*.m4a` —— 套件外面，格式也不對
（同步只認套件內的 `.opus`），所以**從來沒有被同步過**。
插進筆記的錄音卡片更糟：寫進套件的只有卡片算繪出來的一張 PNG，
另一台裝置拿到的是一張播不出來的卡片。

方案與三個決策見 `docs/plans/recording-sync-plan.md`。

### R1 · 核心 Opus 解碼

`AVAudioPlayer` 播不動 Ogg-Opus，所以先做這一段 —— 它要是撐不住，
格式的決策就得重來，而那時候還沒有遷移任何資料。

- Ogg 頁面切封包，**處理跨頁封包**（段長 255 續接）。我們自己的檔案不會
  這樣，但 ffmpeg 轉進來的會，不處理的話解出來是雜訊而不是錯誤。
- **串流輸出**：一小時錄音壓縮後 14 MB，解出來卻是 230 MB PCM。
- **同一個 Decoder 從頭用到尾**：Opus 封包不是各自獨立的，每次重建再
  重解一遍除了是 O(n²)，接縫還會有聽得出來的爆音。

順手修掉一個真的會剪掉聲音的 bug：`OpusHead` 的 pre-skip 寫死 3840
（80 ms），而那不是 libopus 的前視延遲。結果是每一段錄音的前 80 ms
被丟掉 —— 不只我們自己，VLC、ffmpeg 都會丟，顯示的長度也跟著短。
改成向編碼器問 `lookahead_48k()`。

### R2/R3 · Apple 改走核心錄製與播放

- 錄製：`AVAudioEngine` → 16 kHz 單聲道 → `session.feedAudio()` →
  `media/audio/<uuid>.opus`，與 Android **同一條管線**。
- 播放：核心解碼 → `AVAudioPlayerNode`，一次排 0.25 秒、預先排三段。
  跳轉目前是重開解碼器往前丟樣本（幾分鐘的錄音沒感覺，一小時會卡一下）。
- `.m4a` 仍走 `AVAudioPlayer` —— 遷移完成前舊錄音還要能播。

### R4 · 清單與卡片以套件為真相來源

- 首頁「最近錄音」改成掃套件（對齊 Android 的 `RecordingIndex`）。
  只信本機索引的話，**從別台同步過來的錄音永遠不會出現在清單上**。
- 所有播放路徑統一走 `NotebookStore.recordingFileURL(...)`：
  套件裡的優先，找不到才退回 `Kairumo Record`。各處自己拼路徑的話，
  遷移期間一定會有地方拼錯，而症狀是「按了播放沒有反應」。

### R5 · 遷移

`RecordingMigration`：舊 m4a 解成 PCM → 核心編成 Opus → 寫進所屬套件。
沒有所屬筆記本的落在「錄音收件匣」。

**原檔不刪，移到 `Kairumo Record/migrated/`。** 二次轉碼萬一有問題，
使用者還拿得回原本那一份。而且轉出來的檔案要真的解得回來才算成功 ——
只看「寫檔沒報錯」的話，一個 0 位元組的檔也會被當成遷移完成。

### R6 · 收件匣與文件

- 「錄音收件匣」的 id **由核心給**、兩平台共用。寫死不是偷懶：
  兩台裝置各自建立的收件匣會收斂成同一本，用隨機 id 的話使用者會看到
  「錄音收件匣 2」「錄音收件匣 3」。
- Android 的首頁快速錄音原本落在 `entries.firstOrNull()`（清單上第一本）
  —— 等於把錄音塞進一本完全不相干的筆記。一併改成收件匣。
- `format-spec.md` 新增 §7.3.1。

### 驗證
- Rust：`cargo test --workspace` 全過、clippy 與 fmt 乾淨。
- Apple：374 條單元測試全過；iOS Simulator BUILD SUCCEEDED。
- Android：`compileDebugKotlin` 通過。
- **尚未實機驗證**：實際聽得到聲音、跨平台互播、遷移的品質與長度。

---

## 2026-09-21（晚）· iOS 錄音資料夾開錯地方、筆跡不更新修改時間

兩個實機回報，都是「看起來沒壞、實際上沒作用」那一類。

### 一、「開啟資料夾」打開的不是錄音資料夾

`openRecordingsFolderInFinder()` 指的路徑一直是對的
（`Documents/Kairumo Record`）。問題在 `Info.plist` **少了
`UIFileSharingEnabled`** —— 沒有它，App 的 Documents 根本不會出現在
「檔案」App 的「我的 iPhone」底下，於是 `shareddocuments://` 沒有東西
可以導航到，「檔案」只會停在它上次待的地方。

**而且 URL 仍然「開成功」**，`UIApplication.open` 回 true，所以不會有
任何錯誤可以看 —— 這就是為什麼它可以活這麼久。

一併補上兩個防線：路徑先 `resolvingSymlinksInPath()`（`/var` 是
`/private/var` 的符號連結，不解析會導航失敗），以及開啟失敗時退回
Documents 根目錄，至少落在這個 App 自己的區域裡。

順帶兌現了資料主權那條承諾（H4）：使用者現在可以直接用「檔案」App
進去看自己的錄音與 `.padnote` 套件。

### 二、筆跡「沒有被儲存」

實際上筆畫**每次變動就落盤**（`onDrawingChanged` 裡的 `store.saveDrawing`），
資料一直都在。真正缺的是：那條路只寫 `.drawing` 檔，**不會動到筆記本身**。

兩個後果，而且都長得像「筆跡沒存到」：

1. 首頁卡片的修改時間停在上一次插入物件的時刻 —— 使用者寫了一整頁字，
   清單上那本筆記既沒往上移、時間也沒變。這正是使用者看到的畫面。
2. **雲端同步不會被觸發。** 自動同步掛在 `persistData()` 上，而只寫
   `.drawing` 檔不會經過它 —— 手寫內容要等到換頁或離開編輯器才會上雲。
   插入物件與文字會立刻同步，正是因為它們走 `updateNotebook()`。

修法：筆跡變動後去抖動 1.2 秒，再 `updateNotebook()` 把筆記標記成剛改過。
去抖動不能省 —— `onDrawingChanged` 在**一筆畫的途中**就會被叫很多次，
每次都寫 `notebooks.json` 並觸發整棵畫面重算的話，寫字會頓。
1.2 秒與同步排程器的 1.5 秒刻意錯開：先讓本機狀態定下來，
再讓同步那一輪去看已經穩定的內容。

**不加「儲存」按鈕。** 資料本來就沒有遺失過，加一顆按鈕等於把一個
顯示問題轉嫁給使用者，還會讓人以為不按就會掉東西。

跨平台：Android 早就是對的 —— 每一筆畫寫進核心就產生一個 oplog 檔，
而清單的修改時間取自套件裡最新的檔案時間。這次是 Apple 端補齊。

---

## 2026-09-21 · 雲端同步大修：共通基準、變更游標、事件驅動（P0–P4）

同步「改了兩天還是不對」的根因不在合併規則 —— 那一層（Lamport + CRDT）一直是對的。
問題在它的**外圍**：怎麼知道哪些檔案變了、什麼時候該跑、以及幾個會掉資料的順序。

### 一、P0 共通基準：路徑與寫入

- 新增 `padnote_sync::paths`，雲端物件名的**單一來源**，一律 ASCII 小寫、
  字元集 `[0-9a-z._/-]`。平台層不得自行拼接。
  - 為什麼是規格：APFS 預設不分大小寫、ext4 分、Drive 也分。已經爆過一次
    （同一個錄音兩台各上傳一份，誰也看不到對方的）。再加上 Apple 的 NFD 與
    Android 的 NFC —— 一個中文字進到路徑，兩邊位元組就永遠對不上。
  - 相容性：比對用正規化後的鍵，存取用遠端回報的原始名字，**不重傳整個雲端**。
- 新增 `padnote_storage::atomic::write_atomic`（`.part` → fsync → rename →
  fsync 父目錄），下載、壓實、blob 全部改走它。
  - 半個檔案是一個**合法的** append-only 前綴，對面只會把它當成舊版本 ——
    不是「壞檔」，所以沒有任何機制會發現。

### 二、P1 變更游標：把「問」變成一次請求

- 舊流程每同步一本筆記就打一次 `files.list`，**不管有沒有變動**。
  20 本筆記 20 次往返，其中 20 次的答案都是「沒事」。使用者要的
  「沒變動的就不要花時間去動它」在那個結構下做不到：要知道有沒有變動就得先問，
  而問本身就是主要成本。
- 改用 Drive 的 `changes.list` + `startPageToken`，配一份本機 `RemoteIndex`
  （路徑 → file id / 長度）。同步計畫**全部在本機算，零 HTTP**。
- 順帶：上傳不再需要先查 file id（快照裡就有），省掉每個檔一次往返。
- 游標過期（410）自動退回全量重建 —— 唯一的慢路徑，且自我修復。
- 快照綁 Google 帳號：換帳號後舊游標失效，而**失效的游標不會報錯**，
  它只會回一堆對不上的變更，症狀是「同步成功，但什麼也沒發生」。
- 回歸測試釘住成本：**20 本筆記、什麼都沒變的一輪 = 1 次 HTTP。**

### 三、P2 事件驅動：讓同步真的會自己跑

- 新增核心的 `SyncScheduler`（純邏輯、可測）：去抖動 1.5 秒、前景週期 12 秒、
  失敗退避 1→60 秒、同一時間只跑一輪、要重新登入時停下來不空轉。
- 兩個平台共用同一個 `FfiSyncScheduler`。各寫一份的話，使用者看到的不是
  「排程策略不同」，是「Android 比較慢」。
- 觸發點對齊：進前景、進背景、登入完成、網路由斷轉通、本機存檔（去抖動）、
  前景週期。Apple `AutoSyncController` / Android `AutoSync`。
- 背景保底：iOS `BGAppRefreshTask`（Info.plist 的
  `BGTaskSchedulerPermittedIdentifiers` 少一個就會在註冊時丟例外，而那在啟動
  路徑上）、Android `WorkManager` 15 分鐘週期。系統決定什麼時候給時間，
  所以它是保底不是主要路徑 —— 真正的「即時」靠前景觸發。
- 之前只有「回到首頁／按按鈕」會觸發 —— 在編輯器裡寫完一段切到另一台，
  什麼也不會發生。

### 四、P3 正確性（三個會掉資料的問題）

1. **刪除排到上傳之後。** 舊流程是壓實 → 刪雲端舊碎檔 → 上傳。中間斷線，
   那幾筆操作就只剩本機一份，而且沒有錯誤訊息。
2. **只刪自己這台裝置壓實掉的檔**，名單由 `compact_own_doc_ops` 明確給出。
   舊版用「lamport 比本機最大值小」推論，而本機可能根本沒下載過中間那個碎檔 ——
   刪掉的是一份自己從來沒有過的操作。`plan_folder_sync` 的下載端同理。
3. **`manifest.json` 不再要使用者「注意」。** 它的欄位不是不可變、就是權威在
   別處（標題）、就是本機專屬（金鑰包裝參數，被蓋掉＝解密資訊被換掉）。
   規則改成「缺的補、兩邊都有就各留各的」。
- 另外：錄製中的錄音不再每 12 秒整檔重傳（成長未達 1 MiB 先略過），
  但**長度一穩定就一定傳** —— 只看門檻的話，雲端那份會少掉結尾。

### 五、P4 可觀測

- 核心 `sync_diagnose()`（零 HTTP、不需權杖 —— 診斷畫面在網路不通時最需要），
  兩平台各一張「同步狀態」卡：游標建立了沒、快照追蹤幾個檔、哪幾本還有差異。
- 日誌答得出「發生過什麼」，答不出「現在是什麼狀態」，而後者才是下一步的依據。
- 一致性測試 `crates/padnote-core/tests/sync_conformance.rs`：一個**會鬧脾氣**
  的假 Drive（同名重複檔、垃圾桶、5 MB 單次上傳上限、游標過期），
  用固定種子的隨機操作序列跑兩台裝置，斷言收斂。
  單元測試釘的是單一行為，這裡釘的是「湊在一起還對不對」。

### 六、順手修掉的既有建置破損

`apple` target 在此之前無法編譯（與同步無關，但擋住驗證）：
`importNotebookArchive` 參照了不存在的 `meta.json` 結構與未連結的 ZIPFoundation，
改走核心的 `extract_notebook` + `NotebookPackageBridge.importDocument`；
`NotebookPackageBridge` 從 nonisolated 情境讀 `@MainActor` 的 `indexJSON`，
改走新增的 `AccountSyncStore.indexJSONSnapshot()`。

### 驗證
- Rust：`cargo test --workspace` 1342 條全過；`RUSTFLAGS="-D warnings" cargo clippy
  --workspace --all-targets` 乾淨。
- Apple：374 個單元測試全過（UI 測試有兩條既有的版面失敗，與同步無關）。
- Android：`:app:compileDebugKotlin` 通過。
- **尚未實機驗證**：兩台真實裝置的端到端延遲、背景喚醒行為、Drive 配額表現。

---

## 2026-09-18 · Apple App Review 退件排除與全專案佔位符排查（App Completeness & IAP 誤判排除）

### 一、App Icon 佔位符外觀排除（Guideline 2.1 - App Completeness）
- **退件原因**：Apple Review 指出「Specifically, the icons contains placeholder content」。
- **根本原因**：先前 `icon_1024.png` 與各尺寸圖示包含人工外框、陰影圓角卡片與純白色四周邊界（`RGB(255, 255, 255)`），在 iOS 系統自動套用 Squircle 遮罩裁切後，呈現在雙層圓角與外白邊內的卡片樣稿外觀，被機器與審查員識別為尚未完工的未完成設計稿（Placeholder）。
- **解決方案**：
  1. 提取品牌核心標誌（羽毛筆尖與錄音波形），將米色紙張質感背景垂直漸層與微妙雜訊延伸填滿整張 1024×1024 畫布。
  2. 徹底消除四個角落純白留邊、人造陰影與假圓角，產出符合 Apple HIG 標準的 100% 滿版直角無 Alpha 正方形圖示。
  3. 重新以高階 Lanczos 採樣生成 120、152、167、180 與 1024 完整規格圖示。

### 二、消除 "Pro" 關鍵字以排除 IAP 審查誤判（Guideline 2.1(b)）
- **退件原因**：Apple Review 指出「Specifically, the Upgrade to Pro button is unresponsive / references to Upgrade to Pro but associated IAP not submitted」。
- **根本原因**：全 App 為 100% 免費開源軟體（Apache-2.0），專案中**完全沒有 StoreKit 模組或任何收費內購代碼**。然而，工具列調色盤原本設有 `pro_color`，英文翻譯為 `"Pro Color Studio"`（中文為「專業調色」）。審查員/自動掃描誤將 "Pro Color Studio" 認定為 "Upgrade to Pro" 的收費升級按鈕，因點擊後僅彈出調色盤而無 StoreKit 購買視窗，進而觸發「未提交 IAP」與「按鈕無反應」之罐頭拒絕訊息。
- **解決方案**：
  1. 在 `i18n/ui-strings.json` 中將 `pro_color` 全面正名為「進階調色」/ `"Advanced Color Studio"`（日文：`"高度な調色"`，韓文：`"고급 색상 조색"`，泰文：`"สตูดิโอสีขั้นสูง"`）。
  2. 同步更新 Swift/Kotlin 代碼生成，修復所有相關註解與可及性標籤，徹底杜絕任何 "Pro" 敏感字引發的審查誤解。

### 三、全專案佔位符排查與首頁重複字串修正
- 排查全專案 `placeholder` 關鍵字，確認僅用於輸入框之正常引導提示文字（如搜尋提示、輸入文字提示），無任何未完成之假資料、Lorem ipsum 或未實作之 TODO 佔位符。
- 修正 `HomeWorkbenchView.swift` 中首頁雲端同步卡片之副標題在已登入時重覆顯示「雲端同步」之問題，改為正確顯示使用者登入信箱帳號或「已登入」狀態（`signed_in`），消除副標題與主標題重疊之佔位感。

---

## 2026-09-18（早晨）· S-80 / S-85 / S-86 / S-93：手繪縮放座標反變換、側欄樣板插入與物件邊界約束全面實裝

### 一、S-80：Android 畫布縮放/平移與手寫座標反變換（Inverse Transform）
- 過去 Android 整頁模式下縮放或位移畫布時，墨跡與手寫事件會脫位（因為 `InkEngine` 收到的是未經變換的螢幕像素座標）。
- 現於 `InkInput.kt` / `InkEngine.kt` 實作 `zoom`、`offsetX`、`offsetY` 反向變換，在計算取樣點座標時自動還原回未縮放的邏輯畫布座標。
- 在 `MainActivity.kt` 綁定 `canvasScale`、`canvasOffset` 與 `canvasDensity` 即時同步至 `engine`，徹底解決放大書寫時筆跡飄移的問題。

### 二、S-85：跨平台畫布物件拖曳超出邊界自動夾回（Printable Bounds Clamping）
- Apple 端 `NotebookEditorView.swift:moveObject` 補上 `clampToPrintable` 幾何約束，在拖曳文字、圖片、形狀、表格等物件結束時，自動推回頁面可列印範圍內。
- Android 端在 `ShapeLayer.kt` 與 `TableLayer.kt` 補齊 `PageGeometry.clampOrigin`，與既有的文字、圖片、錄音卡片約束保持完全一致。

### 三、S-86 & S-93：Android 側欄樣板插入、多頁管理與逐頁樣板配置
- `NotebookMeta.kt` 擴充 `insertPageTemplate` 與 `removePageTemplate`，支援跨裝置同步 `pageTemplates` 陣列。
- `PageSidebar.kt` 長按選單補齊「在後方插入新頁面」、「插入其他樣板頁面…」（含分類 Tab 與樣板選擇對話框）以及「刪除此頁」。
- `MainActivity.kt` 實裝 `insertPageAfter` 與 `deletePageAt`，並完整接入連續模式、單頁模式及彈窗模式等三處側欄調用點。

---

## 2026-09-18（深夜）· S-81 / S-77 / S-94～S-98：跨平台畫面規格 100% 對齊，差距歸零

### 一、棘輪 baseline 歸零（148 → 0）

`docs/parity/baseline.json` 的 Apple 與 Android 陣列正式清空為 `[]`。
這意味著 `crates/padnote-core/src/ffi_screens.rs` 中定義的全部畫面資訊架構、區塊順序與所有識別控制項，在 Apple（iOS / macOS）與 Android 雙端完全 1:1 實裝對齊。

### 二、S-94：真正的 .padnote 筆記分享，不再是「掛名分享、實為 PDF」

先前 Apple 的「分享筆記」實際上直接轉呼叫 `exportAsPdf()`，而 Android 根本沒有分享按鈕。
現在：
1. **核心**：`crates/padnote-storage/src/package.rs` 提供 `archive_package` 與 `extract_package`（透過 zip crate），在 UniFFI 暴露 `archive_notebook` / `extract_notebook`。
2. **Apple**：`NotebookEditorView.swift` 透過 `NotebookPackageBridge.export` 封裝繪圖與附件至暫存目錄，以 `archiveNotebook` 壓縮成 `.padnote`，再呼叫系統 `ShareActivityView`。
3. **Android**：`Exporter.kt` 封裝 `sharePackage` 與 `sharePackageIntent`，經由 FileProvider 輸出至 `cache/exports`，在 `MainActivity.kt` 接上 `editor.export.share` 與頂列 `editor.share` 按鈕。

### 三、S-95：毛筆、麥克筆、水彩（Brush, Marker, Watercolor）跨平台實裝

1. **核心**：`padnote-ink` 擴充 `Tool::Brush`（毛筆，壓感敏感）、`Tool::Marker`（麥克筆）、`Tool::Watercolor`（水彩，半透明塗層），`ToolKind` 同步擴充。
2. **Apple**：`InkInterop.swift` 完整對齊七種筆刷，iOS 17+ 支援直接使用原生 `.fountainPen` / `.watercolor`。
3. **Android**：`InkToolbar.kt` 擴充至七種筆刷，`InkCanvas.kt` 與 `InkSurfaceView.kt` 分別實作毛筆粗細壓感加權、麥克筆平整筆觸以及水彩 42% Alpha 混合效果。

### 四、S-77：Android 折疊機（Foldable）姿態與鉸鏈避讓

導入 `androidx.window:window:1.3.0`，在 `ui/FoldPosture.kt` 監聽 `FoldingFeature`：
1. 分辨直立鉸鏈（Book Posture）與水平鉸鏈（Tabletop Posture）。
2. 在 `MainActivity.kt:EditorWorkArea` 中，側欄可自動避開中線鉸鏈，或在鉸鏈分離模式下自動加入安全間距，避免折痕切斷畫布與工具。

### 五、S-98：iPhone 小螢幕防擠壓全面收斂

1. 3D 模型工作室（`Model3DStudioView`）針對 `compact` 寬度自動將固定寬度橫排改為上下堆疊（預覽高度固定 250pt + 下方控制項滑動）。
2. 表格工作室（`TableStudioView`）將合併單元格按鈕群包裝在 `ViewThatFits` 內，在 390pt iPhone 上自動換行不破版。
3. 驗證圖表、主題工具、文字工作室在窄版上的彈性佈局。

### 六、手冊六語系同步與驗證

1. 六國語系手冊全面對齊：7 支筆、折疊機適配、.padnote 筆記分享、資料夾側欄分頁。
2. 執行 `scripts/sync-docs.sh`，更新 `apple/Resources/Docs` 與範本目錄。
3. 執行全專案 Rust `cargo test --workspace`、Android `./gradlew testDebugUnitTest`、Apple `xcodebuild` 全部 100% 通過。

---

## 2026-09-18（傍晚）· S-93：一本筆記裡可以有很多種頁，而且顏色是自己挑的

### 一、樣板原本是「開筆記本時選一次，整本就定了」

一本會議紀錄的第一頁想用四象限、後面幾頁想用橫線 —— 這件事以前做不到。
使用者能做的只有「為了一頁不同的格式另外開一本筆記」，而那不是筆記本的
用法。

所以樣板從**整本一個**變成**逐頁一個**（`pagePaperIds`）。入口在頁面結構欄的
快顯選單裡：「插入其他樣板頁面」→ 選主題 → 選樣板，新的一頁就插在這一頁
後面。建立筆記時選的那一個從此只是**預設值**。

逐頁的欄位要跟著插入、刪除、搬動、建立副本、跨筆記本轉移一起維護 ——
漏掉其中一條的症狀是「我搬了一頁，它的版面留在原地」。複本要跟原本那一頁
同一種紙，跨本搬移時紙也要跟著走：使用者搬的是「那一頁」，不是
「那一頁上的字」。

### 二、不做實務範例，做顏色

原本記在 S-89 的是「新樣板缺實務範例」。使用者的決定是**不需要範例**，
維持設計大綱與格式就好 —— 但頁面要活潑，而且顏色要能自己挑。

這是對的取捨：一份填好的範例對「四象限」這種版面即用法的紙有幫助，
但它同時是**三十三份要跟著六國語系維護的文案**，而使用者第一件事往往是
把它刪掉。顏色不一樣 —— 它每次打開都在。

六組配色（石墨、靛藍、青綠、玫瑰、琥珀、森綠）下沉核心。

**這裡跟上一輪說過的話有出入，值得講清楚。** 上一輪寫的是「核心只說輕重，
不說顏色」—— 那句話在顏色只有「淡一點、深一點」的時候是對的。使用者現在要
**選**配色，而一組配色是六個平台都要拿到同一份的資料：兩邊各挑各的色票，
同一本筆記在兩台裝置上會是兩個顏色。所以**色相**進核心，**深淺**留在平台
（深色模式是平台的事，核心不需要知道現在是不是深色）。

六組都刻意選了低彩度：版面是拿來對齊的參考，不是內容。一組飽和的橘線畫在
紙上，寫上去的字會變成第二顯眼的東西。

### 三、兩者都走同步的中繼資料

`pageTemplates` 與 `guidePalette` 兩個鍵兩端**共用**，所以在一台裝置上排好的
逐頁樣板與挑好的顏色，同步之後另一台看到的是同一份。Android 的畫布這一輪
就讀得到並畫得出來了；它還缺的是**設定**的入口（記在 S-93）。

### 四、一個編得過、跑起來壞掉的位置

把新的方法插進 `@MainActor` 與它修飾的宣告**之間** —— 那個屬性就落到新的
宣告上了。編譯器這次有講（`has multiple actor-isolation attributes`），
但同一種錯在 Android 端的 `@OptIn` 上出現過一次，而那次是沉默的。
寫進踩雷清單。

### 五、手冊

這一輪（S-83 起）改動的每一件事都補進手冊，六個語系：三十三種樣板與七個
分類、版面配色、逐頁換樣板、頁面的右鍵／長按快顯、拖曳換頁序、多選複製與
搬移到別本、縮圖大小、可拖曳的側欄界線、把筆記拖出資料夾、頁面規格與
編輯區域的提醒，另外補了兩則常見問題。版本行同步到 v3.9.2。

---

## 2026-09-18（下午）· S-91：把幾頁搬到別本筆記

### 一頁不是一筆資料

它是散在四個地方的東西：磁碟上的筆跡檔案（檔名就是頁碼）、`pageHeights`、
`pagesData`，以及九種附件各自的 `pageIndex`。只搬其中一樣的結果不是「沒搬」，
是「筆跡到了、上面的表格沒到」。

### 計畫先算好，再執行

「複製到哪幾頁、要刪哪幾頁、准不准做」由核心的 `page_transfer_plan` 一次
算清楚，執行那一段就沒有判斷。邊走邊算的話會出現「複製成功、刪的時候刪錯頁」
—— 而那時原本那幾頁已經不在了。

計畫裡有三條規則，每一條都有測試釘著：

- **由大到小刪來源。** 由小到大刪的話，刪掉第 1 頁之後第 3 頁已經變成第 2 頁，
  接著刪「第 3 頁」就刪到了別人。這不是偏好，是正確性。
- **一本筆記不能被搬空。** 全選再搬走的話會留下一本打得開、什麼都沒有、
  也加不了頁的筆記。按鈕在那個狀態下直接停用 —— 按下去才被拒絕比較難懂。
- **搬到自己身上不做。** 要在同一本裡換順序，用的是搬動頁面（S-87）。
  複製到自己身上倒是合理的（把某一頁再來一份接在最後），所以只擋搬移。

### 複製過去的物件要換一個 id

`id` 是**同一本筆記裡**的身分。複製到別本時沿用原本的 id 看起來沒事，直到
使用者把那一頁再複製回來 —— 這時同一本筆記裡有兩個相同 id 的物件，而選取、
刪除、堆疊順序全部是照 id 找的：點其中一個，另一個跟著動。

換 id 走 JSON 來回，不是替九種型別各寫一個複製建構子 —— 那九個建構子加起來
是上百個欄位，漏抄一個的症狀是「複製過去的表格少了一欄」。

### 入口有兩個

多選（側欄標題列那顆勾選鈕）給「這幾頁」，單頁的右鍵／長按選單給「就這一頁」
—— 最常見的情況不該為了它先進多選模式再勾一格。

### 一顆看起來是按鈕、按下去沒反應的按鈕

底下那一排「複製到…／移動到…」第一版完全沒反應。原因是
`.buttonStyle(.plain)` 的命中範圍是**標籤自己的形狀**：可以按的只有文字與
圖示的筆畫，那一圈底色是純裝飾。補上 `.contentShape(Rectangle())` 之後正常。

這個坑在這個專案裡不是第一次出現，所以寫進 STATE 的踩雷清單。

---

## 2026-09-18（上午）· S-90：紙張從十三種長到三十三種，而版面不再是程式碼

### 一、先講那個擋路的東西

紙張的版面原本是**每種紙一段繪圖程式碼**：Apple 端十三段手寫的
CoreGraphics，Android 端**一段都沒有** —— 它只畫得出核心 `PageStyle` 那
六種底紋，藍圖的標題欄、手機線框、使用者旅程的泳道那一層完全不存在。
同一本筆記在 iPad 與平板上長得不一樣。

要從十三種長到三十三種，沿著原路走就是 Apple 多二十段、Android 落後二十段，
而其中任何一段畫錯了只會在那一種紙上看得到。

所以版面改成**資料**：核心回傳一串圖元（線、框、實心條、文字、勾選框、點），
平台只認得那六種怎麼畫。新增一種紙是改一個 Rust 檔案，兩邊同時就有。

**底紋不走這條路。** 方格、點陣、橫線仍然由平台自己鋪，理由是量：5mm 點陣
在 A4 上是兩千多個點，把它們一顆顆送過 FFI 只是浪費。
分界線是這句話：**底紋是重複的材質，版面是這張紙的結構。**

順帶刪掉了 `TemplateBackgroundView` —— 同一份版面的第三份實作，
SwiftUI 版，寫好之後沒有任何地方用到它。

### 二、三種記法

使用者要的其實不是「十三張紙裡挑一張」，是「我想用哪一種記法」。
新開一個「筆記方法」分類，康乃爾從「通用基礎」搬進來，另外七種是新的：

- **四象限**：重點／問題／決議／行動。會議紀錄最後要留下的是下一步，
  而不是一整頁流水帳 —— 版面本身就在問「所以呢？」
- **大綱**：三條縮排導引線。線很淡，它是尺不是框。
- **雙欄對照**：左原文、右自己的話。
- **一問一答**：六組，先寫問題、事後回想作答。
- **KWL**：已知／想知道／學到了。
- **康乃爾（方格版）**：同樣三區，底下是方格 —— 圖解與公式用。
- **心智圖**：中心框加四向分支起點，其餘讓使用者自己長。

### 三、規劃與清單

參考常見的手帳版面重新畫了一套（版面是功能性的排列，不是誰的美術設計）：
月計畫、週計畫七欄、日程表（半小時一列）、24 小時時間軸、學習計畫、
專案時程；待辦清單、雙欄勾選、習慣追蹤（31 天 × 14 項）、作業追蹤、
家事分工、21 天挑戰。

版面用 **0–1 的比例**寫，最後才乘上實際頁面大小 —— 同一份版面在 A4、A5
與 16:9 上都成立，不必為每一種規格各寫一份。勾選框的邊長用**寬度**換算，
否則橫式頁面上它會被拉成長方形。

### 四、主題從四個變七個，所以分段控制項要換掉

`.segmented` 會把七個中文標籤擠進同一列 —— 那正是先前回報過的
「文字被擠壓、功能鈕被遮掩」。兩端都換成橫向捲動的膠囊列：放得下任意數量，
而且每一個都看得清楚。

標題也從「選擇樣板」改成「**筆記頁樣板**」：它選的是這一頁長什麼樣，
不是筆記本的種類。

### 五、兩個實際踩到的地方

**版面用畫布高度算，不是頁面高度。** 畫布比頁面高（它要捲動），所以四象限
的十字第一版落在頁面下緣**之外** —— 畫面上看得到，列印出來不在紙上。

**縮圖原本完全不畫版面。** 側欄裡分不出哪一頁是康乃爾、哪一頁是四象限，
而畫布上是看得到的。畫布與縮圖現在共用 `PageGuideRenderer`，匯出也走同一份
—— 列印出來要跟畫布上看到的一樣。

### 六、舊筆記怎麼辦

Apple 的 `NoteTemplate` rawValue 是**中文字面值**，而且已經寫進使用者的
`notebooks_v1.json` 與同步中繼資料，動不得。新加的二十種沒有這個包袱，
rawValue 直接用核心的英文 id。

中文 → id 的對照表放核心（`paper_id_from_stored`），Android 讀同步過來的
中繼資料時照用 —— 在 Android 那邊再抄一份中文對照，就是第二份會漂移的東西。
測試釘住那十三個字串：對不回來的話，那本筆記重開之後版面會變成空白紙，
而使用者沒有做過任何事。

---

## 2026-09-18（凌晨）· S-87：拖得進去拖不出來、以及只能往下看的頁面清單

### 一、筆記本拖得進資料夾，拖不出來

能「移出資料夾」的落點只有一個：未分類區的**標題列**。而那一段是
`if !rootNotes.isEmpty` —— **把最後一本筆記歸檔之後，落點跟著消失**，
再也拖不出來。就算它還在，那也只是一條 26pt 的細長條，用手指拖著一本
筆記去瞄，瞄不中的表現是「放開之後什麼也沒發生」，看起來就是壞了。

改成三個落點，而且都是大面積的：

- **根目錄那一列**：它一直都在，而且在最上面 —— 往上拖是「拿出來」的
  直覺方向。
- **整段未分類區**（不是標題列），空的時候顯示一句提示，順便告訴使用者
  這裡可以放東西。
- **另一本筆記**：落在它身上就是落到它所在的資料夾。使用者要把 A 搬進
  「範例」時，最自然的動作是拖到「範例」底下那幾本筆記中間，而那一片
  區域原本完全不接受落下。

另外補了右鍵／長按選單裡的「移出資料夾」。原本連選單裡都沒有這一項：
「移至資料夾」那張表只列得出資料夾，列不出「不在任何資料夾裡」。

### 二、頁面只能看，不能排

側欄可以跳到第 9 頁，但不能把第 9 頁搬到第 2 頁。

索引算術下沉核心（`page_index_after_move` / `page_index_after_insert`）。
**往後搬與往前搬的區間不對稱** —— 第 1 頁搬到第 3 頁，中間的兩頁往前
遞補；第 3 頁搬到第 1 頁，中間的兩頁往後讓。寫成同一條式子的人會在其中
一個方向上差一格，而差一格的症狀是「搬完之後有兩頁長得一樣、少了一頁」。
測試裡有一條專門釘這件事：每一種搬動都必須是**重排**，n 頁進去、n 個
不重複的頁碼出來。

搬動時真正要動的是四樣東西：磁碟上的筆跡檔案（檔名就是頁碼）、
`pageHeights`、`pagesData`，以及九種附件的 `pageIndex`。只重排其中一樣
不是「沒搬動」，是「筆跡搬了、上面的表格沒搬」——比不能搬還糟。

順手修掉同一個病灶的另外兩處：`insertPage` 只平移了五種附件（表格、
形狀、連接線、錄音整組漏掉），`duplicatePage` 一種都沒平移 ——
複製第 1 頁之後，第 2 頁以後的東西全部落到那張複本上，而原本那一頁空了。
**複製是為了留一份原稿，結果把原稿搬走了。**

### 三、界線不能拖，縮圖只有一個大小

280pt 的側欄對「翻頁找內容」剛好夠，對「看清楚這一頁畫了什麼」不夠，
而畫面上唯一的操作是把整條側欄關掉。

界線改成可以拖，夾住兩端的規則在核心（`sidebar_clamp_width`）：
**內容那一側的下限優先於側欄的下限** —— 視窗本來就窄的時候，寧可側欄
只有最小寬度，也不要讓畫布小於能寫字的寬度。畫布才是這個畫面的主體。

縮圖另外給了一個大小控制。存的是**絕對寬度**而不是倍率：倍率的基準是
側欄寬度，而側欄寬度也是使用者在調的 —— 兩個都在動的話，拉寬側欄會讓
縮圖跟著跳一次，而使用者只是想把欄位拉寬一點看資料夾名稱。放大到比側欄
還寬時把側欄一起拉開，否則「＋」在碰到側欄寬度之後就沒有反應了，
而使用者按它的理由正是「我要看得更清楚」。

側欄裡的字跟著寬度走（`sidebar_content_scale`，上下限比寬度的比例保守）：
一條 500pt 寬的側欄上出現 9pt 的頁碼，看起來不是寬敞，是沒排版。

**第一版寫錯的地方**：寬度用 `.environment(\.sidebarWidth, width)` 傳，
再在同一個 View 結構裡用 `@Environment` 讀 —— 讀到的永遠是預設值，
因為那個修飾子只影響**子樹**，不影響加上它的那一層。編得過、跑起來縮圖
永遠停在 248pt，而畫面上看起來像「按鈕沒作用」。

### 四、一頁上的操作只藏在右上角

那顆 `...` 在縮圖縮小時只有十幾點寬，而且它看起來像裝飾。真正會被嘗試
的手勢是長按與右鍵。兩者現在叫出同一份選單，而選單的內容只寫一份 ——
各寫一份的結果是其中一邊少了一項，而使用者會以為那一項不存在。

### Android 還沒跟上

頁面搬動、縮圖大小、可拖的界線，Android 一項都沒有（S-86），
而且**核心根本沒有「搬動頁面」這個操作**：`DocOp` 只有
`AddPage { index }` 與 `RemovePage`。Apple 端的頁面順序是它自己的檔案，
所以直接重排就成立；Android 的頁面住在 oplog 裡，要先有 op 才搬得動
（S-87）。那件事做完之後，搬動會跟著同步走到別台裝置 —— 比現在這個
只在本機成立的版本更正確。

---

## 2026-09-17（深夜）· S-83：刪不掉的頁、只是畫好看的框線、以及被吃掉半個畫布的空白

### 一、「刪除頁面」只清掉了內容

`deletePage` 對每一種附件各寫了一份幾乎一樣的「刪掉這一頁的、後面的往前移」。
新增附件型別時沒有人記得回來加 —— **表格、形狀、連接線與錄音整組漏掉了**，
`pagesData` 也沒有跟著縮短。

所以症狀不是「按了沒反應」，而是頁數少了一頁、那一頁上的表格與圖形卻還在，
而且跑到別頁去了；匯出時那一頁還會復活。使用者看到的就是「刪不掉」。

抽成一個 `shiftPages` 泛型函式 + `PageIndexed` 協定，九種附件共用一份。
之後新增型別時，漏掉是編譯期的事，不是使用者在某一頁上發現多了一張表。
補了四條回歸測試。

### 二、頁面框線只是畫好看的

`PageGeometry.fitsInPage` 與 `clamp` 寫好了，**從來沒有人呼叫過**。
Android 端根本沒有。使用者在框線外寫的東西，畫布上看得到、匯出的 PDF 裡
整段不見，而且沒有任何提示。

界線判斷下沉核心（`is_within_printable` / `is_outside_printable` /
`clamp_to_printable`，5 條測試），並且**分成三種答案**：整個在裡面、
整個在外面、跨在界線上。三種的處理方式不同：

- 整個在外面 → 收回那一筆。它印不出來也匯不出去，留著只會讓使用者以為
  它存在，等到列印那天才發現不見了。
- 跨在界線上 → 留著，但提醒一次。寫到一半被整筆吃掉比溢出更難用。

### 三、畫布把可用寬度吃滿，而頁面只有 800pt

右邊那塊空白**在頁面框線之外** —— 寫上去的東西不會出現在列印結果裡。
把左側資料夾欄收起來之後那塊空白更大，看起來像畫布破了一個洞。

連續模式一直是「縮到剛好放得下」，整頁模式沒跟上。現在兩邊同一套：
放不下就等比縮小，放得下就維持原尺寸並置中。

### 四、頁面規格（A4／A5／Letter／Legal／16:9／正方形）

八種規格下沉核心 `page_formats()`。規格同時決定三件事：畫布多大、
分頁在哪裡斷、匯出的 PDF 多大 —— 這三件事分別在兩個平台的三個地方實作，
尺寸各寫一份的話「畫布上看到的」與「匯出的」會對不起來。

**A4 直式必須原封不動沿用既有的 800 × 1132**：那個數字已經寫進所有現存
筆記的座標裡，動它就是把每一本舊筆記的內容位置一起移掉。測試釘住了這一條。

換規格時把既有物件夾回新的可列印範圍 —— A4 直式換 A5 之後，原本靠右那一欄
會落在新頁面外面，不夾的話畫布上看得到、匯出時整欄不見。

### 五、兩個插入入口，留一個

工具列左下角的「更多」與右上角的選單內容重疊，而右上角那個是**嚴格超集**。
左下角那份不增加能力，只是讓使用者多一個地方要找。整個拿掉（手寫與打字
兩種模式各一個）。

### 六、常用樣板

39 種文件範本收在一棵三層的樹裡，而使用者絕大多數時候要的是「再來一份跟
上次一樣的」—— 那件事原本要展開主題、展開分類、再認出那一個。
**最常用的路徑不該是最長的。**

保留最近三個，去重、最近的在前。順序規則放核心（`recent_templates_push`，
5 條測試）：它是會落盤的順序，兩端各寫一份的話同一個人換平台打開時排法
不一樣；而去重漏掉的症狀很醜 —— 連續建三本同樣的筆記，清單就被同一個
樣板佔滿。

---

## 2026-09-17（晚）· S-79：手指在畫布上做不了事、紙張選了沒作用、同步看不出連到哪

### 一、「手指沒辦法捲動，也沒辦法變更畫面大小」

查下去發現兩邊的成因不同，而**兩邊都不是設定錯了，是根本沒做**：

- **Apple**：`PKCanvasView` 是一個 `UIScrollView`，而 scroll view 的
  `minimumZoomScale` 與 `maximumZoomScale` 預設**都是 1.0** —— 也就是
  「不准縮放」。這兩行從頭到尾沒有被設定過，所以捏合手勢在任何 Apple
  裝置上都不會有反應。
- **Android**：整頁模式的畫布是一個固定的 `Box`，既不捲動也不縮放。
  連續模式只是剛好外面包了 `LazyColumn` 才捲得動。

規則下沉核心（`ffi_gesture.rs`，5 條測試）：**一指到底是畫還是平移，
看「手指現在能不能畫」**。手指能畫就一指畫線、兩指平移縮放；只有筆能畫
或打字模式，一指就平移 —— 原本那個狀態下一指是**什麼也不做**，
使用者按下去畫面完全不動，看起來就像畫布卡住了。

過程中踩到三個：
1. 夾平移範圍時把「內容尺寸」填成視窗尺寸，`scaled <= viewport` 永遠成立、
   可拖範圍一律 0。
2. `graphicsLayer` 平移沒加 `clip = true`，頁面畫到畫布外面蓋住工具列。
3. 沒放大時一指平移會把整頁拉走 —— 那不是使用者要的。改成**放大之後**
   才接管單指；沒放大時的上下瀏覽仍然缺（記在 TODO S-80）。

### 二、選了康乃爾，畫面上卻是滿版橫線

兩個獨立的缺陷疊在一起：

**(1) 核心沒有把紙張讀回來的出口。** `add_page(style)` 一直把底紋寫進
`.padnote`，但沒有 `page_style(page_id)` —— 平台層因此畫不出底紋。
補上出口，並在 Android 補上六種底紋的繪製（`canvas/PageBackground.kt`）。

**(2) 使用者挑的紙張從來沒有生效過。** 核心建立筆記本時**已經先放了一頁
`Lined`**，所以 `firstPageId() ?: addPage(style)` 永遠走前半段。
修法是在建立當下「先加一頁、再刪掉原本那頁」（用既有的兩個操作，
不開格式變更的口子）。

第一次修的時候放進 `open()` —— 而一次新增筆記會呼叫 `open()` **三次**，
其中兩次帶的是預設的 `BLANK`：第一次正確設成康乃爾，第二次又被換回空白。
log 印出來才看得見。換紙張是**建立**那一刻的事，所以它只能在 `create()` 裡。

底紋要畫在 `InkCanvas` 的白底之上、筆跡之下 —— 疊一層 Composable 在外面
會被那層 `background(Color.White)` 整個蓋掉。

### 三、Android 補上紙張挑選器

主題分類 + 十三種紙 + 「這張紙的內容」（不套用／實務範例／空白大綱），
與 Apple 一一對應。清單來自核心 `paper_templates()`，兩邊同一份。
同時把「紙張樣板」那個主題從文件範本樹裡收起來 —— 兩個地方都列的話，
同一份範本會有兩個入口。

### 四、同步頁看不出連到哪個帳號

原本只有「已登入 / 尚未登入」兩種狀態。一台裝置上有兩個 Google 帳號是
常態，而「同步好像沒作用」最常見的真正原因就是兩台連到了不同帳號。

現在顯示：**帳號**、**同步目的地**（Drive 的應用程式資料夾）、
**上次同步**（相對時間，只有成功才記 —— 失敗也記的話「上次同步」會變成
「上次按下按鈕」）。自選資料夾那條路顯示**完整路徑**，不是只有最後一層
資料夾名稱（兩個都叫「Kairumo」的資料夾在畫面上會一模一樣）。

帳號**不需要新增授權範圍**：Drive 的 `about.get` 在 `drive.appdata` 底下
就讀得到 `user.emailAddress`。多要一個 `openid email` 只為了顯示一行字，
與這個 App 的定位相反。

### 五、這一輪是真的跑起來看的

平板模擬器（Pixel Tablet，1280×800 dp）上逐項驗過：紙張挑選器、康乃爾
底紋、範本內容、頁面結構欄、導覽頁寬度、單指平移。前面幾輪只驗到
「Kotlin 編譯通過」，而這一輪抓到的五個缺陷**每一個都只有跑起來才看得到**。

---

## 2026-09-17（下午）· S-74/S-75：紙張也要有內容、把重複的入口拿掉、以及協同那三個不會報錯的失敗

### 一、紙張樣板只有底紋，沒有內容

選「行動端線框」拿到的是兩個空的手機外框，選「黃金比例」拿到的是幾條參考線。
**會用的人不需要它，不會用的人看不懂它** —— 十三種紙裡有一半都是這樣。

所以每一種紙各配一份「實務範例」（照著改就能用的真實內容：受力分析、康乃爾
筆記、版面規格、零件圖標題欄、登入流程線框、使用者旅程…）與一份「空白大綱」
（只留標題與欄位）。走的是文件範本同一條管線（`templates/src/` → 排版 →
一份 JSON、兩個平台共用），所以座標仍然只算一次。

預設是**不套用**。最常用的動作仍然是「給我一張空白紙」，那件事不該因此多按一下。

### 二、排版估算短算，內容被靜靜裁掉

第一次把紙張範本鋪上畫布時，句子結尾出現了刪節號。查下去發現估算器有三處短算：

| 短算的東西 | 後果 |
|---|---|
| 段落間距（兩端都加 6pt，估算器沒算） | 五行清單短算 24pt |
| 一個字的實際寬度（用 1.0×字級，實際約 1.15×） | 行數算少 |
| 文字方塊自己的上下留白（估算器只留 10pt） | 兩行的框只放得下一行 |

三個加起來的症狀是：**第二段整段不見，第一行尾巴補上刪節號**。而它看起來像
「範本本來就只寫這麼多」—— 這是最難認的一種壞法。三個值都往上調，寧可高估：
估高了只是下一個區塊往下挪，估矮了是內容不見。39 種文件範本一併受惠。

### 三、首頁右上角那個頭像選單，四個項目全部是重複的

| 選單項目 | 首頁上已經有的入口 |
|---|---|
| 身分 | 身分卡片上的「編輯身分」 |
| 素材圖庫 | 主要動作卡片 |
| 錄音資料夾 | 「最近錄音與轉錄」區塊的按鈕 |
| 系統診斷 | 頁尾的版本號，以及「資料與同步」裡的卡片 |

整個拿掉。UI 測試原本要先開這個選單才點得到診斷，改指頁尾那顆版本號。

### 四、視窗版面：主要動作被切掉

`presentationDetents` 沒有給 selection 時，系統會挑**最小**的那一個 —— 於是
每一張表都開在半高，而這些面板的主要動作（「貼入畫布」、「建立」、「確認」）
都在最下面。使用者打開來看到的是一張按鈕被切掉的表，而且不會知道要往上拉。

改成預設 `.large`。另外把五張表上寫死的 `minWidth/minHeight` 拿掉（那是還在
當 Catalyst 視窗時的遺留），並把「算式計算」整張重做：內容自己捲、主要動作
釘在底部安全區、大標題改成 inline —— 原本大標題把「取消」擠到標題上面，又把
輸入框推到摺線以下。

### 五、線上協同：三個不會報錯的失敗

跑了一次真正的跨裝置驗證（iPad 開房、iPhone 用邀請連結加入，兩台都在成員清單
裡看到對方），然後在同一條路徑上抓到三件事：

**(1) 解密失敗時回傳的是信封本身。** `decryptPayload` 在「訊息是加密的、但這端
沒有金鑰」時回傳 `payload` —— 而加密訊息的 payload 是 `{"ciphertext": ...}`，
沒有 `page_index`、沒有 `item`。下游每一個 `guard let` 都靜靜地 return。
症狀：兩台都顯示已連線、成員清單也對，**但對方畫的東西永遠不會出現**。
改成回 nil，並補了四條單元測試釘住。Android 端一直是對的（`?: return`）——
這正是「同一段邏輯各寫一份就會漂移」的教科書案例。

**(2) 畫布還沒建立時，遠端筆畫連存都不會存。** 套用遠端筆跡那段整個包在
`if let canvas = canvasView` 裡。畫布是 SwiftUI 的 `@State`，而它是在
`makeUIView` 裡回填的 —— 那個時機的 @State 寫入會被丟掉（實測 `canvas=0`）。
落盤與畫面是兩件事：現在先無條件落盤，畫布有沒有在都不影響資料。
檔案層級驗證：修好前對方畫完那一頁的檔案仍是 64 bytes（空），修好後是 352。

**(3) 加入時只貼房號，不會有任何提示。** 金鑰在邀請連結 `#key=` 那一段。
只貼房號的人連得上、看得到成員，然後什麼都收不到。兩端都補上一則橘色提醒，
輸入框的提示字也從「請輸入房間識別代碼」改成「貼上邀請連結（或房號）」。

操作手冊的「線上協同」整節重寫成零基礎的八個步驟（六個語系），把「所有人要
同一個 Wi-Fi」「隊友要改伺服器位址、127.0.0.1 是自己」「只傳房號不夠」這三件
最容易卡住的事寫在該出現的位置，並補了一條對應的 FAQ。

---

## 2026-09-17 · S-70：四個回報 —— 範本沒落地、視窗拉不動、主題分類沒作用、簽長了兩支手機

### 一、範例筆記打開來一片白，而它的名字承諾的是說明

`seedDefaultNotebooks()` 只在筆記庫**空的時候**跑。範例的實際內容
（`SeedContent` / `SeedNotebooks`）是後來才加的 —— 在那之前裝過 App 的人，
庫裡早就有兩本只有標題與頁數的空殼，而空殼不是空的，所以補內容那個分支
一輩子不會執行。

實機的資料可以直接看：這台 Mac 上七個容器裡有四個是
`歡迎使用 Kairumo: 23 頁 / 0 個物件`。**功能一直在，只是永遠碰不到。**

補了一段回填：只補「標題是種子、而且整本一個字都沒有」的那幾本。
種子跑完那些字就是使用者自己的內容，所以只要有任何一筆筆跡或物件就一個
字都不動 —— 補內容補掉使用者的筆記，比一片空白嚴重得多。順手把舊版種子
留下的二十幾張空白頁收掉：整本已經驗證是空的，收掉不會動到任何東西。

Android 同一個洞（`seedIfEmpty`），加上一個一次性旗標 —— 沒有它，
使用者把範例刪掉之後它們會自己長回來。

### 二、彈出視窗調不了大小 —— 三條路試到第三條才會動

依序試過，每一條都是**編得過、看起來對、實際沒作用**：

1. 右下角自製拖曳把手 + `preferredContentSize`。手勢有進來（log 印得出
   位移）、值也套進去了，**外框紋風不動** ——
   iOS 18 的 `PresentationHostingController` 不理它。
2. `.presentationSizing(.fitted)` + `.frame`。整張表攤成全螢幕，
   導覽列還被畫了兩次。
3. `presentationDetents([.medium, .large])`。**會動**，而且 iPad 的
   form sheet 也吃這組值（模擬器上拖了驗過）。

另外一個一開始就踩到的坑：用 `horizontalSizeClass` 判斷「是不是手機」。
iPad 上的 form sheet 自己就是 compact 寬度，於是整台 iPad 都被判成手機。
要問的是裝置，不是那張卡片。

代價寫在 `ResizableSheet.swift` 裡：**只能調高度**，寬度由系統決定。
Android 對應的是對話框底部的拖曳把手（`ui/ResizableDialog.kt`），
同樣只調高度。

### 三、「主題分類」不是沒作用，是它管的東西在螢幕外

它一直有作用 —— 它換的是下面那份紙張清單。但那份清單被排在
**一整棵可展開的文件範本樹之後**，切換主題時會變的東西在畫面外，
使用者按下去看不到任何反應，合理的結論就是「這個壞了」。

沒有刪掉它，而是把清單移到切換器正下方，變成同一個區塊。順手把紙張目錄
下沉到核心（`ffi_paper.rs`）—— Android 的「新增筆記」原本**連紙張都不能選**，
各寫一份只會讓兩邊繼續分岔。

### 四、一份公文「簽」底下壓著兩個手機外框

「文件範本」（簽、契約、會議紀錄）與「紙張」是兩個獨立的選擇，而
`mobileWireframe` 這張紙的內容就是「內建雙手機螢幕輪廓框」。兩個都選了，
兩個都生效。

文件範本的 JSON **本來就帶著它要的 `pageStyle`**，兩端卻都只解析、不使用。
現在選了文件範本就以它為準，紙張的選擇讓位並在介面上講明白
（「紙張由文件範本決定。」）。對照表放在核心，兩端不會各給一個答案。

### 五、順手修掉的

`scripts/build-android-libs.sh` 在 `debug` 參數下直接中止：
macOS 內建的 bash 3.2 在 `set -u` 下，把空陣列的 `"${ARR[@]}"` 當成未設定
變數。release 路徑不會踩到，所以這個坑活了很久。

---

## 2026-09-16 · S-64：無障礙、搜尋、快捷鍵 —— 與三個安靜的失敗

### 一、`.help` 不是無障礙標籤

整個 App 只有 12 處 `accessibilityLabel`，而編輯器工具列一整排是純圖示。
VoiceOver 念出來不是「復原」，是系統從 SF Symbol 猜的
「arrow uturn backward」。`.help(...)` 給的是**提示**，不是標籤 ——
兩者很容易混淆，加了一個就會以為做完了。

補了 54 處，並留一條**掃原始碼**的測試：有 `.help` 沒 `accessibilityLabel`
就列出檔名行號。真正要驗的是「旁白念出來是什麼」，那需要真機；
退而求其次，守住最容易退步的那個模式。

Android 反而好得多（多數控制項本來就有可見文字），真正缺的是畫布上
可拖曳的把手 —— 標籤要放在容器上，不是裡面那顆裝飾用的圖示。

### 二、搜尋只搜得到標題

打在筆記裡的字一個都搜不到。S-61 的 39 種範本讓它更明顯：整份租賃契約的
條文都在筆記裡，搜「押金」是零結果。

兩個平台做法不同但行為一致：Android 的筆記本來就是 `.padnote` 套件，
直接用核心的 bigram 索引；Apple 的內容就在記憶體裡的附件上，直接比對。
實機驗證：搜「weekly」正好回一本，而**兩本的標題都沒有這個字**。

### 三、快捷鍵：兩次「編得過但沒作用」

1. SwiftUI 的 `keyboardShortcut` 掛在隱藏按鈕上 —— 零警告，按下去沒反應。
2. `UIKeyCommand` 放在包住內容的 controller 上 —— 理論上是祖先。加日誌
   實測，`keyCommands` **一次都沒有被查詢**。

第三版走 `buildMenu(with:)`（App 層級，不依賴響應鏈），第一次跑就把 App
打掉：九個 `UIKeyCommand` 共用同一個 selector，UIKit 算出重複的選單識別碼
而丟例外，堆疊只停在 UIKitCore。改成九個各自的 selector 就好。

**沒驗到的那一半要講清楚**：模擬器預設沒有連接硬體鍵盤，XCUITest 的
`typeKey` 走軟體鍵盤送不出 ⌘ 組合鍵。所以「命令收到後畫面有反應」有測試，
「⌘N 真的被送到命令上」沒有。那條會誤導人的 UI 測試刪掉了，
而不是留一條永遠紅的。

### 四、深色模式下看不見的色票

墨黑是 `#1C1F24`，深色卡片底是 `#1C1E1E` —— 差不到一個色階，那顆色票
**整個看不見**。兩個平台都一樣。

修法不是加個邊框了事：墨色本來就是「畫在紙上的顏色」，襯在深色介面上
看到的根本不是它在頁面上的樣子。改成襯一張白紙，可見度與正確性一起解決。

順帶把 Android 幾處寫死的介面色（錄音紅、選取框、工具列框線）換成語意色。
其餘寫死的 hex 留著 —— 它們是畫布內容（紙就是紙）、身分頭像色與設計師
色盤，本來就不該跟著深色模式變。

---

## 2026-09-16 · S-63：把三項跨平台落差補起來（其中一項是我看錯）

上一輪盤點兩個平台的差異時我列了三項。實際動手時發現**第一項不是落差**，
另外兩項是。

### 一、文字工作室的符號盤 —— 我看錯了

我說「Android 沒有數學符號、標點、羅馬數字、特殊符號四個插入盤」。
**Android 一直都有。** 我的比對方式有洞：用 `grep 'l("'` 抓語系鍵，但
Android 那邊是 `l(key)` 傳變數、清單用裸字串，所以整段被漏掉了。

同樣被我誤判的還有 `border_style` 與 `paragraph_style` —— 它們是**標籤鍵**，
對應的控制項（邊框開關、段落間距與縮排）Android 早就有。

不過查下去確實有問題，只是不是「缺功能」而是「兩份會漂移」：四組符號在
Apple 與 Android 各寫一份，靠 Android 那邊一句「與 Apple 端逐字相同」的
註解維持一致。**註解攔不住任何東西。** 兩份目前確實一樣，但加一個符號時
只改一邊的話，症狀會是「iPad 上插得到的符號在 Android 上找不到」，
而兩邊各自看起來都很完整。

所以下沉到核心（`ffi_symbols`），與 `ffi_link`、`ffi_math` 同一個判斷：
**只要兩邊要給出同一個答案，答案就放核心。**

### 二、3D 材質：Android 根本沒用到 metalness 與 roughness

核心的 `model3dMaterialLook()` 一直有回傳 `metalness` 與 `roughness`，
Apple 把它們餵給 SceneKit 的 PBR。**Android 只取了 hex**，兩個數字完全
沒用到 —— 於是黃金、塑膠、玻璃畫出來只是色相不同的平塗。

（更糟的是那個函式的註解寫著「金屬材質的暗部壓得更深一點」，但程式裡
根本沒有這件事。註解描述的是不存在的行為。）

Android 這條路是 2D 畫布，不可能真的做 PBR。但 PBR 之所以看起來像金屬，
主要是兩件在平面上也能近似的事：

1. **金屬的明暗反差大**（幾乎不漫射）→ metalness 越高，明暗曲線對比拉越開。
2. **越光滑高光越集中**，而且**金屬的高光帶自身顏色** → 金子的反光是金色的。
   這點畫錯的話金色會反出白光，看起來像塑膠 —— 那是材質畫錯時最明顯的破綻。

沒有引入 Filament／SceneView：為了材質質感背一個 3D 引擎，與這個專案在
reqwest、llama.cpp 上的判斷矛盾。

測試釘的是**看得見的性質**而不是像素值：金屬的反差要大於霧面、光滑面要有
高光而粗糙面沒有、金屬高光不泛白、色值不得超界。

### 三、畫布捲軸

Apple 有 `CanvasScrollbar`（理由是系統的捲動指示器純顯示、不接受互動，
Mac 上使用者會想拖它），Android 沒有。補上，理由多一條：**長筆記需要
位置感** —— 20 頁的筆記在連續模式下捲起來完全不知道自己在哪裡。

### 一個工具鏈的坑

Android 的 instrumented 測試連續三次報不同的測試 FAILED，而且每次只跑了
一兩條。查下去是 **Gradle 的誤報**：裝置上的 logcat 寫著
`run finished: 217 tests, 0 failed`，是 Gradle 端的串流被截斷，
把當下在跑的那一條記成失敗。真的當機的話 crash buffer 會有東西，那裡是空的。

**差點就去改根本沒壞的程式。** 判準：看裝置自己怎麼說，不要只看跑測試的
那個工具怎麼說。

---

## 2026-09-16 · S-61：39 種文件範本，與它們挖出來的兩個資料遺失

「新增筆記」要提供 39 種文件範本（商務、法律、公文、學術、個人），每種還有
「完整案例」與「空白範本」兩個版本 —— 78 份文件。

### 內容寫一次，版面算一次

照既有 `SeedContent.swift` / `SeedNotebooks.kt` 的做法，是兩個平台各手寫一份、
各自算 y 座標。78 份文件這樣做有兩個必然後果：兩邊會漂移（「同一份契約範本在
iPad 與 Android 上長得不一樣」），而且改一段文字就要重算它下面每個區塊。

所以內容寫在 `templates/src/`，**版面由 `scripts/doc_templates_tool.py` 算**，
輸出一份排好版的 JSON，兩個平台載入同一份。座標只算一次，兩邊必然相同。

Android 本來連挑選視窗都沒有（按「新增筆記」直接建一本空白的，連命名都不能），
一併補上，版面比照 Apple。

### 內文只做繁中與英文

公文、契約、訴狀綁的是特定法域的格式。逐字翻成泰文或韓文不會變成當地可用的
文件，只會讓使用者以為可以照著用 —— 那比沒有更危險。介面上的名稱與說明仍然
做滿六個語系，產生器會擋下漏掉的。法律類每一份都帶一段**可刪除**的提醒：
這是格式範本不是法律意見。

### 然後我把範本畫出來看 —— 三個缺陷

測試全綠，但我把三份範本算繪成 PNG 實際看過，一眼就發現測試問不到的事：

1. **文字不換行也不斷行。** PDF 匯出器把整個文字區塊丟一個 `Tj`：換行字元被
   吃掉（條列式的「一、…二、…」擠成一長串），太長的段落直接衝出紙張右緣被
   裁掉。畫布上好好的，一匯出或列印就少字，而且沒有任何提示。
2. **表格整張不見。** `NotebookPackageBridge` **完全沒有處理表格** —— 一個字
   都沒有。Apple 端畫的表格因此從來沒有進過 `.padnote`：匯出、列印、同步到
   Android 全部整張消失，而畫面上還在（它活在 Apple 自己的 JSON 裡）。
   **Android 一直都有寫進核心**（`TableStore.persist`），所以這不是「兩邊都還
   沒做」，是只有 Apple 漏了 —— 而且是會讓使用者在另一台裝置上才發現東西不見
   的那種漏。
3. `**粗體**` 的星號原樣印出來 —— 文字方塊不解讀 Markdown。42 處，已清掉。

前兩項都是**先前就存在**的缺陷，不是這一輪造成的；是這批範本（到處都是多行段落
與表格）讓它們無所遁形。兩項都修了，並各留一條回歸測試。

### 半形字在 CJK 字型裡不是半形

換行修好之後還是有一行衝出紙外。原因是 `format_pdf_text` 只要看到一個非 ASCII
字元，就把**整串**丟給 STSong 以 UTF-16BE 輸出 —— 裡面的數字與英文也照全形
前進。仍按 0.55 估寬的話，帶數字的中文句子會被低估，換行換得太晚。

核心的 `glyph_width` 與 Python 產生器的 `visual_width` 都改成：整段有非 ASCII
就整段按全形算。兩邊要一致，不然產生器估的高度會小於實際行數，下一個區塊
會壓上來。

### 驗證

Rust 82 個測試檔全綠（含 5 條換行測試）、clippy `-D warnings` 與 fmt 乾淨；
Apple 全套通過（含新增的表格 round-trip）；Android 206 條 instrumented 全過。
三份範本的實際算繪結果逐張看過。

---

## 2026-09-16 · S-60：真字形，而且一個位元組的字型都沒有內嵌

待辦上寫的是「要畫真字得內嵌一份中文字型，授權要另外拍板」。**那個前提是錯的**，
所以那個決策不必做。

### 先查清楚壞的是什麼

`export_page_png` 不只餵縮圖，**它也是使用者按下「匯出 PNG」時走的那條路**
（`Exporter.kt`、`ExportPrintManager.swift`）。灰條在縮圖上說得過去 ——
0.18 倍之下一個 15pt 的字只有 3 像素高，畫出來也是一團灰。在使用者匯出、
要拿給別人看的那張圖上，說不過去。

而且兩個平台不一致：

| | 縮圖 | PNG 匯出 |
|---|---|---|
| Apple | `PageThumbnailRenderer`（UIKit，真字） | 灰條 |
| Android | 灰條 | 灰條 |

同一本筆記在 iPad 的清單上看得到字、在 Android 上看不到 —— 那才是使用者真正
撞到的那件事。

### 不用內嵌字型：PDF 交給系統畫

核心的 PDF 匯出器本來就輸出**真正的文字物件**，中文走 Type 0 複合字型
（`/F_CJK`、UTF-16BE）。字型沒有內嵌，靠檢視器代換 —— 而兩個平台的系統
算繪器都做得到：

- **Apple**：PDFKit。實測核心產的 PDF，中文標題是完整的字。
- **Android**：系統內建的 `PdfRenderer`。

所以新增 `PageImageRenderer`（兩邊各一份），走「核心產 PDF → 系統算繪 → PNG」。
**內嵌 0 位元組**，也沒有多一個第三方相依。與 HTTP、LLM、PDF 讀取是同一個
分工：平台做得比我們好的事就交給平台。

### 怎麼確定畫出來的是字，不是豆腐框

這是這件事唯一容易自欺的地方。**灰條、豆腐框（▯）與真字形，三者都會有暗
像素** —— 只數「有沒有墨」三種都會過。

分得開的問法是**筆畫密度**：筆畫少的字與筆畫多的字，只有真字形才會差很多。
同樣八個字，在 Android 上「一」是 696 個暗像素、「鬱」是 4744 個，差 6.8 倍。
豆腐框畫出來兩者會一模一樣。

兩個平台各留一條這樣的回歸測試（`PageImageRendererTest.kt`、
`PageImageRendererTests.swift`），外加「只打字的頁面不可以匯出成空白」。

### 順手修掉一句寫錯的註解

`build-xcframework.sh` 裡當初寫「關掉 pdf feature 只是讓 Apple 也走純 Rust
那條，**功能不會少**」。少了 —— 少的就是字。Apple 不帶 PDFium 的判斷仍然成立，
但理由要算在 PDFKit 那裡，不是算在「fallback 一樣好」。

### 核心那條灰條保留

它仍然是系統算繪失敗時的後備，也是測試與命令列唯一的路。版面（位置、行寬、
行數、行高）照實算這件事因此還是要維持。

---

## 2026-09-16（收尾之二）· Android instrumented 的三條紅燈

CI 修完之後只剩這一個工作紅。本機用同一組映像（api-35 google_apis）
跑 `connectedDebugAndroidTest` 直接重現，三條：

### 1. 手冊裡不可以寫作業系統名稱

我在錄音那一節加了「**Android 版這一版只有錄音沒有轉錄**」。
`DocsAssetTest.documentsDoNotNameOperatingSystems` 擋下來，而它是對的 ——
理由有兩個：

- 同一份手冊六個語系、所有平台共用。指名某個平台，其他平台的使用者
  會誤判自己有沒有那個功能。
- **那句話遲早會變成謊話。** Android 一旦補上轉錄引擎，手冊就錯了，
  而不會有人記得回來改。

所以不是放寬規則，是改寫成用「你這台裝置有沒有」來描述，並給一個
使用者自己看得到的判準：「轉錄需要一個在裝置上跑的引擎，而不是每台
裝置都有 —— 沒有的話錄音照常運作，只是錄音列不會出現轉錄文字。」
六個語系都改。

**Apple 端的同名測試漏了 `manual.js`**（手冊的內容在那裡，不在 HTML
裡），所以這件事一路到 Android 才被擋下來。已補上，兩邊現在驗同一組檔案。

### 2. `theManualCoversBackupAndSync` 比對的是排版不是內容

與 Apple 端上一輪修過的是同一個問題：`manual.js` 重新產生時走
`JSON.stringify`，鍵名從 `id: "data"` 變成 `"id":"data"`。內容還在，
測試卻紅。改成兩種寫法都算。

### 3. 套索被當成「漏接核心筆刷」

`everyBrushMapsToACoreToolExceptTheEraser` 只放行橡皮擦。套索是 L-06 才
加的**選取**工具，和橡皮擦一樣沒有對應的核心筆刷 —— 那次沒有跟著改這條
測試，所以它一直把新工具誤判成漏接。這條是**先前就存在的紅燈**，不是
這一輪造成的。

本機 200 條 instrumented 測試現在全過。

### 結果：CI 八個工作全綠

`88996dc` 的這一輪八個工作**全部成功**，包含先前一直沒回報完的
`Test (windows-latest)`。CI 從 2026-09-15（S-20 那一筆）紅到現在，
這是這段期間第一次整排綠。

---

## 2026-09-16（收尾）· CI 整排紅燈：一條 lint 讓五個工作一起倒

### 看起來像五個問題，其實是一個

CI 從 9/15 的 S-20 那次提交起就是紅的，而且**每一個工作都紅**：
Android build、Android instrumented tests、FFI bindings、Format
compatibility、三個平台的 Test。

原因是 `.github/workflows/ci.yml` 設了 `RUSTFLAGS: -D warnings` ——
任何一條 clippy lint 都會讓 `cargo` 失敗，而上面那五個工作**全都要先編
核心**。所以它們不是五個獨立的故障，是同一個編譯步驟倒了五次。

這一次真正擋住全部的是 `padnote-shapes` 裡一個不需要的 `mut`（我自己在
`SpeechBubble` 寫下的閉包）。本機 `cargo check` 只把它當警告，而我當時
用 `grep -E "^error"` 過濾輸出 —— **警告被我自己的 grep 濾掉了**。

教訓寫進 STATE：本機要用 `RUSTFLAGS="-D warnings" cargo clippy
--workspace --all-targets` 對齊 CI，不要只看 `cargo check` 的錯誤行。

### 其餘的 clippy

`padnote-llm` 的 `match` 改 `?`、`padnote-ink` 的索引迴圈改
`iter().enumerate()`、`padnote-core` 的 `chunks_exact` 改 `as_chunks`、
三處 `collapsible_if`、`_point_marker` 搬到 `mod tests` 之前。

`padnote-llm-llama` 的 `n_cur` 是**模型上下文裡的絕對位置**，不是迴圈
次數；clippy 建議的 `for n_cur in (start..).take(n)` 讀起來更難懂，
所以在迴圈上標 `allow` 並寫明理由 —— 這種時候關掉 lint 是對的，
但關掉之前要說得出為什麼。

新加的縮圖繪圖函式參數過多（9/7）。把 `pixels/width/height/scale` 收成
一個 `Canvas` 型別：那四個永遠一起出現，而且拆開來傳很容易把 `width`
與 `height` 寫反 —— 那種錯畫出來是整張圖歪斜，不會有任何錯誤訊息。

### rustfmt

`cargo fmt --all`，34 個檔案。多數是這次之前就沒對齊的，最常見的一項是
edition 2024 的 import 排序（小寫排在大寫之後）。

### 這次是逐一跑過 CI 的每一個工作才推的

`cargo fmt --all --check`、`cargo clippy --workspace --all-targets`
（帶 `-D warnings`）、`cargo test --workspace`、`relay_ffi`、
format-compat、`generate-bindings.sh` + swiftc 語法檢查、
`build-android-libs.sh`、`gradlew assembleDebug`、
`android-release.sh --unsigned`、`cargo deny check`。

---

## 2026-09-16（深夜）· S-57 縮圖畫得出物件、S-59 Android 編輯器工具列分兩排

### S-57：縮圖不再是一張白紙

`padnote-export::to_png` 是**沒有 PDFium 時的 fallback**，而 Android 第一版
就走這裡（`pdf` feature 關閉）。它原本只畫底紋與筆畫 —— 一本只打字的筆記
在 Android 的清單上是一張全白的圖，而 Apple 那邊看得到整頁的版面。
「預覽跟畫布不一樣」那句抱怨，Android 這一半就是它造成的。

現在畫：**圖片**（解 PNG 後最近鄰取樣貼上）、**表格**（外框、格線、表頭底色
都照核心的版面算）、**形狀與連接線**（幾何走 `padnote-shapes`，與畫布同一份）、
**文字**（灰條，見下）。

順序與畫布一致：圖片 → 形狀 → 表格 → 文字。自己排一套的話，重疊的物件在
縮圖上的上下關係會與畫面相反。

形狀住在**物件樹**裡，不在 `page.blocks()` 中 —— `to_png` 因此多收一個
`objects: Option<&ObjectTree>`。不傳的話縮圖上會少掉整張流程圖。

`geom_kind` 那個 `match` 刻意不寫 `_ =>`：核心加了形狀而這裡忘了補時，
要在**編譯期**壞掉，不是在縮圖上默默少一個圖形。

#### 文字為什麼是灰條

畫真正的字需要一份內嵌字型。中文字型動輒好幾 MB，而且授權要另外拍板
（`deny.toml` 對授權的立場很明確）—— 那是一個決策，不是一個算繪問題，
所以留成 S-60。

灰條不是偷懶：位置、行寬、行數、行高全部照實算，只有字形是抽象的。
縮圖的尺度是 0.18–0.4 倍，一個 15pt 的字在上面只有 3–6 像素高，真的畫
出來也是一團灰。**版面對不對才是縮圖要回答的問題。**

實測：兩本範例筆記的縮圖現在看得到標題列、議程表的格線與表頭底色、
段落行條，以及歡迎頁下方那三顆圓角標籤 —— 與實際頁面一致。

### S-59：Android 編輯器的工具列分兩排

原本所有東西攤在同一個 `FlowRow`：返回、低延遲、掌拒、手寫／打字、模式提示、
框選、分頁導覽、顯示模式、新增頁、「⋯」。在 320dp 的手機上會換成四五行，
**佔掉半個螢幕**，而且手寫的筆刷列在打字模式下照樣列著 —— 使用者看到一整排
筆，點下去卻畫不出東西。

改成與 Apple 一致的兩層：

- **第一排是「這個畫面本身」**：返回 · 手寫／打字 · 分頁導覽 · 顯示模式 ·
  新增頁 · 「⋯」。
- **第二排是「目前模式的工具」**：手寫模式是低延遲、掌拒與筆刷列；
  打字模式是框選、新增文字方塊、插入連結。
- 模式提示獨立成一行細字，兩個模式都顯示。

低延遲與掌拒搬到第二排是有理由的：它們是**手寫的**設定，留在第一排的話，
打字模式下它們也一直在那裡，而那時候兩個都沒有意義。

實測：打字模式下畫布的起點從 y≈480 上移到 y≈275，等於在 640 高的螢幕上
拿回三分之一的版面。

---

## 2026-09-16（晚）· 未提交刪除的處置、S-40～42、Android 版面對齊 Apple

### 那批未提交的刪除：**維持還原**，並說明為什麼

擁有者把判斷交回來，結論是還原是對的：

- `sync_is_hidden` 修的是「在 A 刪掉、B 上還看得到」那個 bug，**帶著四條
  回歸測試**。刪掉它不只是少一個函式，是把那個 bug 放回去。
- `collab_check_server` 是一道安全控制：它擋掉指向公開網路的明文 `ws://`。
  Android 那邊連同呼叫端一起被拿掉，等於協同可以在公開網路上走明文。
- 三個函式都還有呼叫端，Android 因此**編譯不過**。
- repo 裡沒有任何紀錄（DEVLOG / TODO / ADR）說要移除它們。

四條測試已隨程式碼一起回來，`cargo test --workspace` 82 個 suite 全綠。
原本的工作區 diff 仍在 scratchpad 的 `pre-restore.patch`，要反悔隨時能拿回去。

### S-40：錄音卡片進得了匯出

縮圖與「匯出為圖片／列印」在上一輪已經補上。這一輪補完剩下的那條路：
`.padnote` 套件（PDF 匯出走它）原本只把 3D 模型與連結卡片算繪成衍生圖片
帶進去，錄音卡片會整張消失。加上 `renderObjectImage(NoteAudioAttachment)`
與 `objectKind: "audio"` 的標記，並把 `audioAttachments` 寫進筆記本中繼資料
—— 少了後者，同步到另一台裝置的是一張「有圖片、沒有卡片」的頁面。

兩個平台的 `derivedObjects` 都加上 `"audio"`：不加的話，匯入時那張 PNG 會被
當成普通圖片載入，同一張卡片變成兩份，而且各自能拖到不同的地方。

### S-42：錄音長度改由核心算

新增 `padnote_audio::ogg_opus_duration_us` 與 FFI 的 `audio_duration_seconds`。

**為什麼在核心**：Apple 可以問 AVFoundation、Android 可以問
MediaMetadataRetriever —— 但那是兩份實作，而卡片上寫的是**同一份資料**。
兩個平台顯示不同的秒數，使用者會以為同步壞了。而且格式是我們自己寫的，
granule 的語意與 pre-skip 的處理只有核心最清楚。

只讀最後一頁的 granule（它是累計值），並扣掉 pre-skip —— 不扣的話每段
錄音都會多出 80 ms，與播放器顯示的長度對不起來。讀不出來回 0 而不是丟錯：
長度是顯示用的，為了一個欄位讓插入失敗，代價完全不對等。

實測：5 秒的檔案在 Android 的卡片上顯示 `00:05`。

### S-41：Android 可以從首頁把錄音插進別本筆記

**音檔要複製，不能引用。** 錄音住在筆記本套件裡，而套件是同步的單位 ——
留一個指向別本筆記的引用，另一台裝置只會同步到目標那一本，打開之後拿到的
是一張播不出來的卡片，而畫面上看不出為什麼。

檔名撞到時加時間戳：直接沿用來源檔名的話，兩本筆記各有一個同名錄音時會
互相覆蓋。

順帶修掉一個自己寫出來的 Kotlin 陷阱：`created?.let { null } ?: 錯誤訊息`
在成功時的值是 `null`，而 `null ?: x` 就是 `x` —— 插入明明成功卻跳出
「插入失敗」。改成明確的 `if (created == null)`。

### Android 的首頁改成 Apple 的版面

原本兩個平台長得像兩個產品。最刺眼的是三張主要動作卡：Apple 是一排帶圖示
與副標的卡片，Android 是三顆 180dp 寬的純文字卡 —— 在手機上疊成三顆佔掉
半個螢幕的大按鈕。

改法：

- 加上畫面標題（原本第一眼看到的是一張身分卡，不知道自己在哪個畫面）。
- 三張動作卡改成 `weight(1f)` 等寬、帶圖示與副標，第三張從「新增子資料夾」
  換成「素材圖庫」（與 Apple 一致）。等寬在任何寬度下都是一排。
- 「繼續」改成橫向捲動的卡片列。
- 錄音列加上播放鈕與「⋯」選單（裡面就是 S-41 的入口）。原本只有一個
  麥克風符號，要聽得先開那本筆記再找到它。
- 「全部筆記」加上資料夾列，清單改成兩欄的縮圖格狀。
- 「資料與同步」改成帶圖示與右箭頭的列，並補上「選擇同步資料夾」——
  它原本只在編輯器的「⋯」裡，使用者要先開一本筆記才找得到。
- 補上「說明與條款」兩張卡與頁尾。操作說明原本也只在編輯器的選單裡。

縮圖那一格多了一道判斷：Android 的縮圖目前只畫得出筆畫與底紋（S-57），
所以一本只打字的筆記會是一張全白的圖。**一片空白就改畫「圖示 + 名字」的
佔位** —— 直接顯示的話，格狀清單上會是一排白方塊，看起來像縮圖壞了，
而其實是還沒實作。抽樣判斷而不是掃全圖：每次重組都全掃會卡住捲動。

---

## 2026-09-16（下午）· 七項回報：表格、形狀庫、模式分界、框選、縮圖、文件、圖示

### 1. macOS 上的表格面板只剩一格

一個 3×3 的表，編修面板上只畫得出**一個**輸入框，而下面的控制列（合併、
增刪欄列）完全正常 —— 所以看起來像「表格壞了」。

成因是 SwiftUI 的 `Grid` 放進雙向 `ScrollView` 時在 Mac Catalyst 上會塌成
一格。iPad 上同一段程式碼是好的，這也是為什麼它活到現在。換成
`VStack`＋`HStack` —— 兩個平台的行為一致，而且這裡本來就不需要 `Grid`
的對齊能力。

順帶補上**即時預覽**：欄寬、字級、合併與表頭底色原本都要「插進去才知道」，
而那時候面板已經關了。預覽與畫布走同一個 `NoteTableView` / `TablePreview`，
所以看到的就是會得到的。

### 2. 形狀庫從 20 種擴到 55 種

新增 35 種：直角三角形、平行四邊形、梯形、七／八邊形、十字、箭號、
四向箭頭方塊、雲朵、心形、閃電、月牙、水滴、L 形、四／六／八角星、太陽、
旗幟、對話框、匾額、扇形，以及 ISO 5807 的人工作業、延遲、已儲存資料、
彙整、抽取、跨頁連接、顯示、打孔紙帶、打孔卡、對照。

**持久化的號碼只能往後加。** `ShapeKind` 會以 u8 寫進 `.padnote`，
0–19 已經在使用者的檔案裡了 —— 動其中一個，就是把他既有的圖形換成別的
形狀。新的一律從 20 起算。`s46_canvas_objects` 那一行寫死的數量就是這道
防線，加形狀時要一起改它。

選單格子從 88pt 縮到 56pt（Android 84dp → 52dp）。五十幾個形狀用原本的
尺寸要捲四五屏，而使用者在找的那一個十之八九不在第一屏。名稱與 ISO 語意
改走輔助說明與無障礙標籤 —— 每格掛兩行字的話格子就小不下來。名稱同時
接上語系表：`NoteShapeAttachment.name(of:)` 是**持久化用的識別字**，
直接拿來顯示的話中文介面會出現「arrowblockright」。

### 3. 「打字模式」下筆還在畫線

`PKCanvasViewDrawingPolicy` 只能表達「誰可以畫」，表達不了「誰都不能畫」。
原本打字模式設 `.pencilOnly` —— 擋得掉手指，擋不掉 Apple Pencil。
拿筆的人在「打字模式」裡照樣在畫畫，而畫面上沒有任何東西告訴他模式換了。
Android 更直接：`InkCanvas` 根本沒有看模式，兩個模式都畫得出來（實測）。

兩邊都補上一道明確的開關（Apple 關 `drawingGestureRecognizer`，Android 在
`onTouchEvent` / `pointerInteropFilter` 最前面讓出事件），並且在畫布上加一張
**兩個模式都顯示**的徽章：這個模式下筆會不會畫線、物件動不動得了。
只在打字模式掛提示的話，切回手寫時畫面上沒有任何差別 —— 而兩邊「同一個
手勢會發生什麼事」完全不同。

### 4. 框選：一次搬一整組

在此之前物件只能一個一個碰。排好的一組圖文要整組往下移 20pt，就得重複做
七次，而且每次都會差一點點。套索只認手繪筆畫，碰不到任何物件。

框選刻意做成一個**明確的模式**，不是「在空白處拖曳就框選」—— 打字模式下
拖曳已經有兩個意思（拖物件＝搬它、拖空白＝捲畫布），再疊一個上去三者會
互相搶，而搶輸的那一次使用者看到的是「我明明在拖，畫面卻在捲」。

判定用**相交**不是「完全包住」：使用者掃過一排卡片時很少會把框拉得比那一排
還大，要求完全包住的話十次有九次什麼都沒選到。刪除時形狀連同它的連接線
一起刪 —— 只刪形狀的話，線會留在畫布上，兩端各指著一個不存在的東西。

### 5. 頁面縮圖與畫布不一致

縮圖的合成器**根本沒有畫表格、形狀與連接線**（現在再加上錄音卡片）。
所以一頁上有表格與流程圖時，側邊欄的縮圖與匯出的圖片裡它們全都不見 ——
使用者看到的是「預覽跟畫布不一樣」，而且會以為自己的內容掉了。

四種型別的幾何一律走核心（`layout()` / `outline()` / `ShapeGeometry`），
與畫布用的是同一份。自己再算一次的話，匯出的表格高度或連接線落點會和
畫面上差一點點 —— 而「差一點點」正是使用者說不出哪裡怪、只覺得怪的那種。

### 6. 手冊與隱私權政策對不上現況

逐條對過目前的實作，六個語系一起改：

- 打字模式那一節說「只認觸控筆，手指會捲動」→ 現在筆也不畫線。
- 說「點一下空白處新增文字方塊」→ 早就改成點兩下（單擊留給捲動與選取）。
- 說可以「向下延長此頁（+800pt）」→ 頁面高度早就固定了。
- 縮圖那句「與畫布等比例、位置一致」→ 在今天之前是錯的（少畫表格與形狀），
  現在對了，但仍然只畫上半部，所以改成講清楚。
- 新增：範例筆記、說明文件卡片、插入錄音、錄音插入頁面、框選、55 種形狀。
- 補一條常見問題：「為什麼打字模式下筆畫不出東西？」
- **Android 這一版只有錄音沒有轉錄** —— 原本的手冊對所有平台都說「裝置端
  轉錄」，那對 Android 使用者是假的。

隱私權政策補上**最重要的那一個缺口**：App 有 Google 登入與雲端硬碟同步，
而政策裡完全沒提。新增一節說明授權範圍只到 `appDataFolder`、憑證只存在
本機、以及怎麼撤銷。生效日一起更新。

兩份文件的算繪器都加上 `**粗體**` 的支援 —— 之前文字全走 `textContent`，
標記會原樣顯示成兩個星號。

### 7. Android 的圖示是預設的機器人

`res/` 底下根本沒有 `mipmap`，manifest 也沒有 `android:icon` —— 所以系統
用預設圖示。從 `assets/brand/kairumo-icon-v1.png` 產出五個密度的舊式圖示，
另外把筆尖與波形去底當成自適應圖示的前景、紙色當底色。

前景**不能**直接放整張原圖：系統遮罩（圓形、圓角方形、水滴…）會沿著畫面
裁一刀，原圖自己的圓角底板會被切出一圈缺角。去底時亮度門檻取 150 而不是
195 —— 底板有紙紋，接近門檻的中間調會在前景下緣留下一排鬼影（第一版看到了）。

### 順帶：`xcodegen` 會把 `Info.plist` 抹掉（第二次踩到）

今天稍早補的 `INFOPLIST_FILE` 證明是必要的。這一輪重跑 `xcodegen generate`
之後，`CFBundleURLTypes` 與多視窗支援都還在。

### 又一次：工作區裡未提交的刪除

重新產生綁定之後 Apple 端編不過 —— `collab_check_server` 不見了。那是同一批
未提交刪除的一部分（連同 Android 的明文 WebSocket 安全檢查）。已還原。
`canvas/ContinuousPages.kt` 的改動**沒有動** —— 那是另一件事，有自己的理由。

---

## 2026-09-16 · 四項使用者回報：文件視窗、範例內容、頁面錄音、插入子項

### 1. macOS 上「操作說明」與「隱私權政策」按了沒反應

不是找不到檔案，檔案一直都在 bundle 裡。原因是 `apple/Info.plist` **根本沒有被
打包進去**：`project.yml` 沒有宣告 `INFOPLIST_FILE`，而 pbxproj 裡那一行是
手改的 —— 任何人跑一次 `xcodegen generate` 就會把它抹掉。

抹掉的後果有兩個，而且都**沒有任何編譯錯誤**：

- `CFBundleURLTypes` 消失 → Google OAuth 的 redirect 回不到 App。
- 沒有 `UIApplicationSceneManifest` → Catalyst 的 `UIApplicationSupportsMultipleScenes`
  預設為 NO → `openWindow(id:)` **靜默失效**。按下卡片什麼也不會發生，
  Console 連一行訊息都沒有。

修法是兩件事一起：`project.yml` 補上 `INFOPLIST_FILE: Info.plist`（沒有它，
下一次 xcodegen 又會壞一次），`Info.plist` 補上多視窗的鍵。
實測：Catalyst 建置的 Info.plist 兩個鍵都在，兩份文件各自開出獨立視窗。

Android 這邊本來就是好的（gradle `copyUserDocs` 有把 `docs/manual`、`docs/legal`
複製進 assets），模擬器上實測打得開。

### 2. 範例筆記本是兩本空白頁

`seedDefaultNotebooks()` 只建立了**中繼資料** —— 標題、摘要、頁數，沒有任何
一個字。一本叫「歡迎使用 Kairumo」的筆記打開是一片白，比沒有範例還糟：
使用者會以為這個 App 只能手寫。

兩本各三頁，圖／文／表混排（`apple/Sources/SeedContent.swift`、
`android/.../library/SeedNotebooks.kt`）：

- 歡迎使用 Kairumo：三合一說明 + 三顆圓角標籤形狀 / 工具列對照表 / 備份與隱私
- 課堂與會議記錄：議程表 + 重點 / 可編修的長條圖 / 待辦表 + 三節點流程圖

**文案只在建立當下取一次語系字串**，不像標題那樣掛 `titleKey` 跟著語言變 ——
種子跑完那些字就是使用者自己的內容了，跟著變的話他改過的字會被翻譯蓋掉。

表格內容在 catalog 裡是**一條字串**（`\n` 分列、`|` 分欄）。拆成十五條的話
譯者看不到上下文，而且加一列要改六個語系的鍵名。

### 3. 錄音進不了頁面

在此之前錄音只有兩個位置：首頁清單，與筆記本層級的 `recordingAudioPath`。
兩者都**沒有座標**，所以「把這段錄音擺在對應的那一段筆記旁邊」做不到。

新增 `NoteAudioAttachment`／`AudioObject`：和圖片、表格同一層級的畫布物件，
有頁次、有位置、有尺寸，進得了堆疊順序與圖層面板。兩邊的 JSON 鍵名一致
（`audioAttachments`），所以跨平台同步得過去。

播放狀態以**附件 id** 為鍵而不是錄音 id —— 同一段錄音可以插在兩頁上，
按第 2 頁那張時第 1 頁不該一起變成暫停鈕。

卡片的選取用**長按**不是點擊：點擊要留給播放鈕，兩者都掛點擊的話
按播放會先被外層吃掉。

### 4. 「更多」插入子項目

實際壞掉的是**連結卡片**：`LinkPreviewSheet` 的插入路徑漏了 `pageIndex`，
所以在第 5 頁按「插入連結」，卡片會落在第 1 頁 —— 畫面上什麼也不會出現。
其他插入路徑（圖片、表格、形狀、3D）都有那一行，只有連結漏了。
Android 那邊本來就沒漏。

另外連結卡片兩邊都只能搬、不能改：補上旋轉、縮放、編修（網址／標題／說明）
與刪除（Android 在此之前**刪不掉**）。卡片高度也改成照模型走 ——
原本只綁寬度、位置用寫死的 `+50` 推算中心，卡片一變高選取框就對不上。

「插入物件」選單原本插不了表格、形狀、連結與錄音（只有「更多」裡有）。
一個叫「插入物件」的選單插不了其中四種，使用者找不到就會以為功能不存在。

### 順帶修掉的：Android 編譯不過

工作區裡有一批未提交的刪除，把核心的 `sync_is_hidden` / `sync_item` 與 Kotlin
包裝一起拿掉了，但 `NotebookLibrary` 與 `FolderTree` 還在呼叫 ——
`:app:compileDebugKotlin` 直接失敗。那批刪除連同**四條回歸測試**一起沒了
（「在 A 刪掉、B 上還看得到」那個 bug 的測試）。已還原，原本的工作區 diff
備份在 scratchpad。

---

## 2026-09-15 (32) · 版本號的唯一入口

### ASC 上的號碼為什麼會亂

    3.2.0  → build 24        2.10.0 → build 23, 24
    3.0.0  → build 22        2.9.0  → build 22
    2.10.1 → build 24        2.8.0  → build 20, 21   2.7.0 → build 20

build 24 被三個版本用過、22 被兩個用過。成因不是誰手滑，是結構問題：
Apple 與 Android 各自用不同指令打包、各自升號，而升號**只看 repo 裡的數字**。

**build 號的唯一性是 Apple 那邊的狀態，不是 repo 裡的狀態。** 這是這次的核心
認知。本機算出來的「下一個號碼」在 ASC 已經有更高的號碼時就是錯的，
而且錯誤要等到打包完十幾分鐘後上傳被拒才會知道。

### 三道結構性的改動

1. `asc-latest-build.py` 查 ASC 上已用過的最大 build 號。
   沒有 API 金鑰時回 0 而不是報錯 —— 它是防線不是必要條件。

2. `bump-version.sh` 取 `max(本機六個來源, ASC) + 1`。明確指定的號碼若不大於
   ASC 上已有的，**當場**擋下並告訴你該用哪個號碼。

3. `release.sh` 訂死「**一次發版只升一次版本號**」：
   `apple` / `android` 單獨執行時不升版，用的是 repo 目前的版本；
   要升版用 `all` 或 `bump`。單平台模式下給升版參數會被擋下 ——
   那正是兩邊號碼分家的起點，擋在入口比事後對帳便宜太多。

版本重設為 **3.6.0 (30)**：3.6.0 蓋過 ASC 上最高的 3.2.0，30 蓋過最高的 24。
兩個平台的產物都實際驗過帶著新號碼（Android aapt2 讀出 versionCode=30；
Apple 的 build settings 是 CURRENT_PROJECT_VERSION=30）。

---

## 2026-09-15 (31) · 不經商店的安裝路徑

### `.ipa` 不是「可以傳給別人的安裝檔」

新增 `scripts/dist.sh`，把當下的程式碼打包成能直接安裝的檔案（不升版、不
commit、不碰商店 —— 那些是 `release.sh` 的事）。mac 與 Android 很順：
Developer ID 簽章 + 公證 + 釘選的 DMG、apksigner 驗過的 universal APK，
兩者給誰都能裝。

iOS 不是。第一版交出一個通過所有驗證的 Ad Hoc `.ipa`，然後才發現
**iOS 根本沒有安裝 .ipa 的功能** —— AirDrop 過去、丟進「檔案」App 再點下去，
系統完全沒有反應。不是檔案壞了，是這個格式只有 Mac 上的 Apple Configurator /
Finder 送得進裝置，而且目標裝置的 UDID 還得事先登錄。

「打包成功、驗證全過」跟「對方裝得起來」是兩件事。這次的驗證只證明了前者，
卻用前者的口氣回報 —— 這正是 `STATE.md` 裡那條「誠實區分已驗證與待實機」
要擋的東西。

補上兩條真的能用的路：

- **自己的裝置** → `--ios-install`：development 簽章，解出 `.app` 交給
  `devicectl` 直接裝。Xcode 會順手把接上的裝置登錄進帳號，不必先查 UDID。
- **別人的裝置** → TestFlight（`release.sh`）。不收 UDID、對方不用 Mac。
  這是唯一不需要事先登錄裝置的路。

裝置偵測用 `hardwareProperties.reality == "physical"`。第一版拿 `platform`
判斷，結果九台模擬器全被撈進來，一路編譯完才在安裝時報
`capability not supported`。`pairingState` 也不行 —— 模擬器同樣是 `paired`。

### 順手補起來的洞

- `apksigner --print-certs` 的行首是 `V2 Signer:` 而不是 `Signer #1`。
  grep 抓不到就回傳 1，`set -e` 讓腳本在「其實驗證成功」的地方無聲中止。
  驗證用的 grep 一律接 `|| true`，結果另外判。
- Android 側載金鑰做成**固定**的一把（`android/.local/`），不是拋棄式。
  Android 認金鑰不認版本 —— 換金鑰的新版沒辦法覆蓋安裝，使用者得先移除舊版，
  資料一起沒了。

v2.10.1 (24) 的 iOS 與 Mac 組建已上傳 App Store Connect。

---

## 2026-09-14 (30) · 圖層面板、Mac 上傳被擋下的兩個設定

### 物件堆疊與群組 —— 最後一項「建了用不到」

核心的物件樹一直有完整的堆疊與群組操作，也有列舉出口，但兩個平台的介面
只做到選取與搬動 —— 使用者碰得到物件，卻**碰不到它們的順序**。兩個方塊疊在
一起時沒有任何辦法把下面那個拉上來。

兩平台各補一個圖層面板，規則放在純函式裡（`ObjectLayerOps` / `ObjectLayer`），
同一組案例兩邊各驗一次 —— 不然「同一個操作在兩個平台結果不同」只有使用者
會發現。面板由上到下＝由前到後，那是圖層面板的慣例；反過來的話使用者每按
一次「上移」都要在腦中翻譯一次。

核心補了兩支：

**`set_object_z_index`（絕對索引）。** 原本只有「上移一層／移到最上層」這類
相對操作。相對操作在併發下會疊加：兩台裝置各按一次「移到最上層」，合併後會
得出誰也沒預期的順序（`format-spec.md` §6.2 早就寫明要用絕對索引）。
整批重排時就是逐個帶索引進來。

**`object_node`。** `root_objects` 只回傳根層，而群組之後成員就不是根物件了 ——
平台若只看根層，**整組形狀會從畫面上消失**，而檔案裡其實好端端地存在。

### Mac 上傳被 altool 擋下

兩個錯誤，都是設定檔漏一行，而且**在本機完全看不出來**：App 建得起來、
跑得起來、測試也全過，直到上傳那一刻：

- **90296**：缺 `com.apple.security.app-sandbox`。Mac App Store 的硬性要求。
- **90242**：`Info.plist` 缺 `LSApplicationCategoryType`。

新增 `Kairumo-Mac.entitlements`，用 `[sdk=macosx*]` 條件掛 —— iOS 一律沙盒化，
帶這個鍵反而會讓佈建檔對不上。

**連帶要修的：沙盒會讓同步資料夾失效。** `CloudSyncFolder` 原本用
`bookmarkData()` 建一般書籤。沙盒裡的書籤必須是 security-scoped 的，
否則下次啟動時書籤解得開、卻沒有存取權 —— 而且不會有任何錯誤，
同步只是靜靜地什麼都沒搬。更新過期書籤前也要先取得 scope，
不然過期的書籤永遠不會被更新。

這幾件事寫成 7 條測試，直接讀專案設定檔驗 —— 發現它們的代價本來是
跑完一整輪封裝與上傳。

### 一份不是這個專案決定要有的東西

`crates/padnote-sync/src/gdrive.rs` 出現在工作區裡（未追蹤），`lib.rs` 多了
一行 `pub mod gdrive;`，**整個 workspace 因此編不過**：`async_trait` 不在相依裡，
而且它實作的方法名稱與 `CloudProvider` 完全對不上（真正的 trait 是
`list` / `get_range` / `append` / `put` / `supports_native_append`，且不是 async）。

它的第一個欄位是 `access_token` —— 那是 OAuth，也就是帳號系統，與決策 D-11
相衝（`docs/plans/account-sync-evaluation.md` 的結論正好相反）。

處理方式是最小且可還原的：`lib.rs` 還原成已提交的版本，`gdrive.rs` 移到
`docs/plans/gdrive-provider-draft.rs.txt`。**沒有刪除任何東西**，
要不要接回去是專案擁有者的決定。

### 測試

核心 942、Apple 306 + 5 UI、Android 200，全綠。

---

## 2026-09-13 (29) · 形狀跨得過平台了

v2.7.0 交付時我明講了一個還沒補的落差：Apple 的形狀存在筆記檔的 JSON 裡，
Android 走核心物件 —— 所以同一張流程圖傳到另一台裝置會**整個消失**，
而且不會有任何錯誤訊息：檔案同步成功、筆記打得開，就是那些方塊不見了。

### 第三個「寫得進去、讀不回來」

`insert_connection` 有 FFI，`connection_object` 沒有。與形狀、表格同一類漏洞，
這已經是第三個了：出口只開了一半，而且在單機測試時完全看不出來。
補上之後，Apple 的匯出改成把形狀與連接線寫進**核心的物件樹**。

連接線的端點用的是**核心給的物件 id**，不是平台原本那個 —— 用舊 id 的話，
線會連到不存在的形狀上，畫面上是一條從空氣連出來的線。匯出時兩端都找不到
對應物件的連線直接跳過，而不是寫一條斷掉的進去。

Android 這邊原本只有形狀存進核心，連接線只活在記憶體裡（重開就沒了），
一併補上；刪形狀時連著的線也一起移除。

### 兩件被 Android 測試抓到的事

**`DocsViewer` 的包裝層過時了。** 那條測試的註解自己寫著「哪天來源改成完整
文件，這裡會提醒去簡化 DocsViewer」—— 上一輪補 charset 之後它就觸發了。
包裝層移除，改成直接 `loadUrl`：少搬一份近 100 KB 的字串進記憶體，
相對路徑也自然生效。這是測試把自己的前提寫清楚換來的。

**上一輪清 OS 名稱漏了「iCloud」。** 我的搜尋清單沒包含它，四個精簡語系裡
還留著，連帶還有幾處「Mac 版視窗」。現在這個檢查在 Apple 與 Android 兩邊
各有一條測試守著。

### 測試

核心 938、Apple 280 + 5 UI、Android 183，全綠。

---

## 2026-09-13 (28) · 同步的環閉起來、表格與流程圖接上、文件亂碼

使用者列了五項落差，這一輪做完前四項，第五項（Android 應用層 22 項）進行中。

### 「A 裝置寫、B 裝置打開就有」—— 之前只有一半

資料夾同步早就在了，但它只搬 `Packages/` 底下的檔案，而使用者真正在編輯的
筆記活在 `NotebookStore` 裡，套件只是按一個按鈕才產生的衍生物，**而且沒有
回頭的路**。結果是：檔案同步得很成功，另一台裝置上什麼也沒出現。

補上 `NotebookPackageBridge.importDocument`，並把流程定成
**匯出 → 搬檔 → 匯入**（順序不能顛倒：先搬檔的話上傳的是上一輪的舊內容）。

過程中測試抓到四個 bug，沒有一個會跳錯誤訊息：

**連結卡片同步一次多一份。** 連結與 3D 模型匯出時會算繪成圖片（PDF 裡才看得
到），匯入時又被當成一般圖片收回來 —— 同一張卡片存在兩份，每同步一趟再多一份。
改成在外觀裡標記 `object`，匯入時跳過衍生圖片。

**頁面 id 不穩定，頁數翻倍。** 每次匯出都隨機生一批頁面 id，兩台裝置的頁因此
永遠不會收斂：合併之後不是一頁有兩邊的內容，而是變成兩頁。加了
`add_page_with_id`，頁面身分跟著筆記走。

**`AddPage` 沒有依 id 去重。** 雙方的 oplog 都帶著同一頁的 AddPage，合併後頁數
直接變兩倍。

**`create` 一定給你一頁。** 沿用既有 id 時那一頁是多餘的，而「先加再移」在多裝置
合併時**不保證互相抵銷** —— 交錯之後會留下一頁空白。加了 `create_empty`：
不產生它，就沒有需要抵銷的東西。

**匯出會洗掉自己的筆畫。** 只寫「這次新增的」而基準線記成「匯入當下的全部」，
這台裝置自己的內容在下一次匯出時會被扣成空的，而它的檔案又被整個重寫。
基準線改成記「別台裝置的部分」：`別人的 = 合併後的 − 自己的`。

Android 那邊原本只認一本固定檔名的 `notebook.padnote`，同步下來的筆記本永遠
不會被開啟。加了 `NotebookLibrary`（含舊資料搬家，測得比功能還細）。

### 表格與流程圖：缺的一直是「讀回來」

先前判斷「表格與 z-order 沒開 FFI」是錯的。寫入操作一直都在 —— 缺的是**讀取**：
平台拿不到「這一頁有哪些表格、裡面是什麼」「有哪些物件」「這個形狀長什麼樣」。
少了讀取，功能就是單向的：寫得進檔案，畫面上卻列不出來，也就選不到、編不了。
這與圖片區塊、筆記本中繼資料是同一類漏洞，而且在單機測試時完全看不出來。

補上 8 支讀取出口，兩個平台都接上表格（增刪列欄、合併、表頭）與形狀流程圖
（ISO 5807 九符號 + 範本）。表格的版面下沉到新的 `padnote-table` crate ——
欄寬、列高、斷行各算一份的話，表格的高度會不一樣，而高度會影響它底下的東西。

Android 的形狀搬動後重開會跳回原位：位移走的是**變換**，不改寫原始邊界
（ADR-0010），讀回來時要把變換套上去。

### Apple 的輸入診斷

Android 早就有延遲量測，iPad 出問題時卻沒有等價的資訊可看 —— 上一輪 Android
畫布全白那次就是靠那條線找到的。演算法與格式與 Android 逐字相同，否則兩邊的
數字沒有可比性，而那正是它唯一的用途。

### 隱私權政策整頁亂碼

`privacy.html` **完全沒有 `<head>`，也沒有 `<meta charset>`**，直接從 `<title>`
開始。WKWebView 載入本機檔案時沒有 HTTP 標頭可依據，只能猜編碼。手冊只有
`<script charset>`，HTML 本身同樣沒宣告 —— 它只是還沒踩到。

順帶發現 `apple/Resources/Docs` 是手動複製的。改了 `docs/` 卻忘了複製的話，
App 裡顯示的還是舊版，而且兩邊都看不出來 —— 與 Android 綁定那次同一類漂移。
加了 `sync-docs.sh`，裡面也會擋住沒有 charset 的檔案。

文件本身也過時了：還在教已經移除的「延長此頁」，而且完全沒有「備份與同步」
一節 —— 使用者問過「備份的功能在哪裡」，那本身就是問題的一部分。六語系補齊，
並移除所有作業系統名稱（同一份手冊要給所有平台的使用者看）。

### TestFlight 裝不起來是腳本的問題

`apple-release.sh` 只封裝 `generic/platform=iOS`、以 `-t ios` 上傳 —— 那是 iOS
專用組建。Mac 版 TestFlight 找不到 macOS 組建，就回報「要求的 App 無法使用或
不存在」。Mac Catalyst 必須用 `generic/platform=macOS,variant=Mac Catalyst`
另外封裝、以 `-t macos` 上傳。腳本改成預設兩個平台都做。

### 文件視窗

手冊與隱私權政策在桌機上改成**真正的視窗**：可以移動、拉大小、擺在旁邊一邊看
一邊操作。工作表這三件事都做不到。

### 測試

核心 935、Apple 270 + 5 UI、Android 177，全綠。

---

## 2026-09-13 (27) · 數字製圖：可重新編修、仿 Excel 的控制、兩個平台同一張圖

### 問題是「插進去就改不動了」

原本的 `ChartStudioView` 只有長條／折線／圓餅三種，而且交出去的是
`onInsertChart: (UIImage) -> Void` —— **一張點陣圖**。圖一插進畫布，
裡面的數字就再也拿不回來，想改一個值只能整張刪掉重做。

修法不是「加一個編輯按鈕」，而是換掉存的東西：存的是 `ChartSpec`（設定），
不是算繪好的圖。點陣圖仍然存進 blob（匯出 PDF 與尚未支援圖表的讀取器靠它），
但**設定跟著區塊走**，走的是既有的 `SetBlockAppearance`（op 32）。

### 版面引擎放進核心（新 crate `padnote-chart`）

圖表是「資料 + 樣式 → 幾何」，那是純邏輯。各平台自己畫一份的話，同一組數字
在 iPad 與 Android 上會長得不一樣 —— 軸的刻度不同、長條的寬度不同、圓餅的
起始角不同。使用者看到的是「我的圖變了」。

所以核心算出**所有幾何**（長條的矩形、折線的頂點、扇形的角度、刻度位置、
圖例與資料標籤的落點），平台層只負責把這些形狀畫出來。連文字寬度的估算都在
核心 —— 各自去問系統字型的話，同一張圖在兩邊的軸會退到不同位置。

幾何**不寫進檔案**：每次從設定即時算。把算好的座標存起來反而會讓兩邊有機會
不一致（一邊用舊座標、一邊重算）。

### 圖型從 3 種變 11 種

直條、堆疊直條、橫條、折線、平滑曲線、區域、堆疊區域、圓餅、環圈、散佈、雷達。
加上 Excel 使用者會預期的控制：軸標題與固定範圍／刻度間距、格線、圖例位置、
資料標籤（外側／內側／置中 + 小數位數）、每個數列的顏色、長條寬度、環圈孔徑。

幾個刻意的決定：

**長條圖的基線一定是 0，折線圖不是。** 長條不從 0 起算的話，長度比例會騙人 ——
那是統計圖表最常見的誤導。折線看的是趨勢，允許不從 0 起算。

**刻度間距一定是 1/2/5 的十次方倍。** 直接把範圍除以格數會得到 0.3333 這種刻度，
軸上就會出現一串沒人想讀的數字。

**圓餅只吃第一個數列**，而且介面會講出來 —— 默默把第二個數列畫上去會得到一個
誰也看不懂的圖，而使用者只會納悶第二欄的數字為什麼沒反應。

**平滑曲線用 Catmull-Rom。** 曲線必須**通過**每一個資料點；不通過資料點的
「平滑曲線」畫的是一組不存在的數字。

### 順手補上的核心漏洞：圖片區塊列不出來

寫這個功能時才發現 `text_block_ids` 有、`image_block_ids` 沒有。也就是說圖表
是**單向**的：寫得進 `.padnote`，另一台裝置卻列不出有哪些圖片區塊，也就找不到
它的設定 —— 使用者會看到一張改不動的圖，而設定好端端地躺在檔案裡。
補上 `image_block_ids` 與 `image_block_size`。

### 另一個漏洞：Android 綁定有兩份

`android/Generated/` 一份、`android/app/src/main/java/uniffi/` 一份，而 Gradle
只看後者。新增的 FFI 不會進 App，**而且編譯照樣會過**，因為舊的那份還在。
正確的產生方式一直是 `build-android-libs.sh`（它用 Android 的 feature 組合，
`generate-bindings.sh` 用的是預設組合，會少掉 `RelayServer`）。
在 `generate-bindings.sh` 裡把這件事寫成註解，免得下一個人又踩一次。

### FFI 的錯誤欄位不能叫 `message`

UniFFI 產生的 Kotlin 錯誤繼承自 `Throwable`，而 `Throwable` 已經有一個
`message` —— 撞名會讓整份 Kotlin 綁定編不過（Apple 端完全看不出來）。改成 `reason`。

### 測試

核心 54（`padnote-chart` 46 + FFI 8）、Apple 26、Android 27。
釘的是會誤導使用者的那幾件事：長條的基線、堆疊的總和、扇形的角度總和、
設定的無損往返、以及「每一種圖型都畫得出東西」——
新增類型時最容易發生的事，是忘了接上版面引擎，圖表會靜靜地空白。

---

## 2026-09-13 (26) · 筆刷畫不到物件上、備份與同步找不到、平台落差清查

### 筆刷在插入物件上沒作用 —— 是層級問題

插入物件（圖片、文字方塊、3D、連結、圖釘）是疊在 `PKCanvasView` **之上**的
SwiftUI 視圖，預設會吃掉觸控。於是使用者拿筆想在一張圖上圈重點，
筆畫根本到不了畫布 —— 在一個手寫筆記 App 裡，那是最該能做的事之一。

修法是**手寫模式下整層不攔截觸控**。代價是手寫模式不能直接拖動物件，
要搬動或編輯就切到打字模式。這個取捨是刻意的：手寫模式的主角是筆，
物件操作有它自己的模式。

### 「備份的功能在哪裡？」「同步的功能在哪裡？」

兩個都在，但都藏在「系統診斷」裡。**使用者找不到就是設計的問題** ——
而且備份與同步正是會在**出事之後**才想起來的功能，那時沒有人會去翻診斷頁。

首頁新增「資料與同步」區塊，三張卡片直接進去。標題旁邊直接寫
**「不需要帳號，也沒有我們的伺服器」** —— 使用者會找「登入」，
找不到會以為功能不存在，而那是決策 D-11 的結果不是缺漏。

### Android 只有一支固定的黑色鋼筆

清查時才看清楚：Android 連**橡皮擦都選不到**，顏色固定黑、筆寬固定 3pt。
核心的 `ToolKind` 與 `add_stroke` 從一開始就支援，缺的一直只是 UI。

補上筆刷（鋼筆／原子筆／螢光筆／鉛筆／橡皮擦）、六色色票、筆寬滑桿與預覽點。
兩個設計決定：

**橡皮擦不是一種 `ToolKind`。** 核心的 `ToolKind` 是筆刷種類，擦除走
`erase_stroke` —— 它是一個動作不是一種筆。所以它是引擎的一個狀態。

**擦除是整筆擦掉，不切斷筆畫。** 切斷需要把一筆拆成兩筆並改寫取樣點，
那會破壞「存原始取樣點」的不變式（ADR-0002）。整筆擦除是 append-only 的墓碑，
與同步、回溯都相容。

### 平台落差清查（`docs/PLATFORM-PARITY.md`）

逐層對帳，不用印象：

- **核心層沒有落差** —— 兩邊同一個 `padnote-core`。但核心開出 104 個 FFI 方法，
  Android 只用了 29 個。
- **值得注意的是兩邊都沒接的那幾項**：表格、形狀與流程圖、物件堆疊與群組 ——
  核心做好了、測試齊全，**兩個平台都碰不到**。那是「建了用不到等於沒建」，
  而且做一次兩邊都受益。
- **應用層 23 項 Android 完全沒有**，從多筆記本、資料夾、多頁一路到素材庫。
- **Android 有而 Apple 沒有的也記了**：輸入診斷列與延遲量測 —— Apple 出問題時
  沒有等價的資訊可看。

補齊順序不照清單由上而下，而是照「少了它就不算筆記 App」排。

### 驗證
- `cargo test --workspace` 826
- `KairumoTests` **138/138**
- Android instrumented **97/97**（新增筆刷與橡皮擦 8）
- Android 模擬器實測：工具列出現五種筆刷
- iOS 與 Mac Catalyst 建置成功

---

## 2026-09-13 (25) · Android 畫布文字方塊，以及外觀跨不過平台的缺口

### 動手前先發現：文字方塊的樣式**根本不會跨平台**

匯出到 `.padnote` 時，文字方塊只帶了「文字」與「位置」—— 顏色、邊框、
段落設定全部留在 Apple 自己的 JSON 裡。使用者在 iPad 上把方塊設成透明底、
加了行距，換到 Android 打開會變回白底無行距。

**那不是「還沒支援」，是資料遺失。** 而如果照著現況在 Android 再做一份，
兩邊就永遠對不起來 —— 這正是跨平台一致性規則要擋的事。

所以先補核心：新增 `SetBlockAppearance`（op 32），帶一段**核心不解讀的 JSON**。
核心不需要知道「淡黃色」或「圓角 12」是什麼意思 —— 那是平台的 UI 詞彙；
但那些值必須跨得過平台。鍵名寫進 `format-spec.md` §6.2，兩個平台共用同一組。

順帶補上 `text_block_ids(page)`：先前平台層拿得到某個區塊的內容與外觀，
卻**列不出有哪些區塊** —— 也就打不開別的裝置寫進來的文字方塊。

### 兩個平台共用的三條規則

1. **`"clear"` 是哨符不是顏色。** 它與 `#FFFFFF` 是兩回事，而且不能走顏色
   轉換 —— 多數 hex 轉換會丟掉 alpha，透明就變成黑色。Apple 端上一輪就是
   這樣出過 bug。
2. **`null`（沒設定）與 `0`（設成 0）是兩回事。** 編碼時 `null` 的欄位
   **不寫進 JSON**，否則接收端會把每一個從另一個平台來的方塊都當成「行距 0」。
3. **認不得的鍵一律忽略，不是報錯。** 另一個平台可能帶了我們還沒實作的欄位，
   那時該做的是保留其餘設定。

### Android 端

`TextBox` 的欄位與 Apple 的 `NoteTextAttachment` 一一對應；內距 14 也一樣 ——
差幾點就會讓同一段文字在兩個平台換行位置不同，版面就分家了。

一個 Compose 特有的換算要小心：行距在 Compose 是 `lineHeight`（字級 + 行距），
不是 Apple 的「額外間距」。直接把數字填過去，行數會不一樣。

編輯面板的控制項與 Apple 的「Word 文字編修」對齊，**改了就算數**沒有確認按鈕，
而且段落用「減／數值／加」而不是滑桿 —— 行距 0–24pt 這種範圍下滑桿很難精準，
也看不到目前是多少。同樣的取捨兩邊都做了一次。

### 驗證
- `cargo test --workspace` **826**（新增外觀與列舉 5）
- `KairumoTests` **138/138**（新增外觀編碼 6）
- Android instrumented **89/89**（新增文字方塊 12）
- Android 模擬器實測：選單「新增文字方塊」→ 方塊出現在畫布上 → 編輯面板
  可調字級／對齊／透明底／邊框／段落 → 落盤成 oplog
- iOS 與 Mac Catalyst 建置成功

---

## 2026-09-13 (24) · 文字方塊的四項修正

實機（macOS）回報四件事。其中兩件的根因都不是「沒做」，是「做了但被別的
東西蓋掉」。

### 1. 編輯面板蓋住正在改的方塊

這個專案**解過同一個問題**：圖片美化面板以前是 modal sheet，蓋住整個畫布，
調濾鏡時看不到自己在調什麼，後來改成浮在畫布上的 `FloatingPanel`。
文字編修還留在 sheet。

套用同樣的做法，但多做兩件事：浮動面板版**沒有取消／確認按鈕**（改了就算數，
畫布上同步看得到），而且**拿掉面板內的預覽區** —— 正在改的那個方塊就在畫布上，
面板裡再放一份預覽只是佔位置，而且兩份看起來不一樣時使用者不知道該信哪一個。

建立**新**方塊時仍然用 sheet：那時畫布上還沒有東西可看，蓋住沒關係。

### 2. 底色設不成透明 —— 選項一直都在，只是會被立刻覆蓋

「透明畫布」這個色票從一開始就在。問題在它旁邊的自訂顏色 `ColorPicker`：
它綁的是**衍生值**（從 hex 算出 `Color`，再寫回 hex），而

```swift
func toHex() -> String? { … String(format: "#%02X%02X%02X", …) }   // 丟掉 alpha
```

於是使用者剛選的 `"clear"` 會被轉成 `#000000` 寫回去。看起來就是「設不了透明」。

改成 `ColorPicker` 用自己的狀態、只在真的變動時寫回。順帶把透明色票畫上一條
斜線 —— 不畫的話它跟白色長得一模一樣，使用者根本認不出哪一個是透明。

### 3. 文字方塊不能拖曳縮放

圖片早就有右下角的縮放把手，文字方塊沒有 —— 要改大小得先開一個**蓋住它**的
面板，而且面板裡只有「方塊寬度」滑桿，**高度完全沒得改**。
（程式裡甚至有一個 `resizeBaseWidth` 狀態，宣告了卻從來沒被用過。）

補上把手，寬高一起拉。下限 120×60 不是隨手取的：比一行字還窄的方框，
每個字都會自己換一行，看起來像壞掉。

### 4. 沒有段落設定

原本只有對齊與清單 —— 那是「字」的設定，不是「段落」的設定。新增行距、
段距、首行縮排、整段縮排四項，畫布與匯出**套同一組值**（行距差幾點，
整塊文字的行數就不同，版面又會分家）。

全部是 `Optional`：舊檔沒有這些欄位，套上非零預設的話，升級之後每一則舊筆記
的排版都會變。

用「減／數值／加」而不是滑桿：這些值的合理範圍很小（行距 0–24pt），
滑桿在那種範圍下很難精準，而且看不到目前是多少。

### Android 的狀態

這四項在 Android 上**目前不適用** —— Android 版的畫布還沒有文字方塊
（WP5 做的是筆跡引擎）。等 Android 有文字方塊時，段落欄位與透明底色的
資料模型已經在 `NoteTextAttachment` 裡，行為要照這裡的規則實作。

### 驗證
- `KairumoTests` **121/121**（新增文字方塊 8）
- `cargo test --workspace` 821、Android 建置成功
- iOS 與 Mac Catalyst 建置成功

---

## 2026-09-13 (23) · S-44 可繼續編輯的 PDF 標註、D-05 拍板

### 核心早就會做，只是 App 沒在用

`padnote-export/src/pdf.rs` 從一開始就同時輸出**向量筆畫與標準 `/Subtype /Ink`
標註** —— 那正是「在 Goodnotes 裡繼續編輯我的筆畫」需要的東西。缺的不是能力，
是 Apple 的 App 走了自己的點陣路徑（把整頁算繪成圖再塞進 PDF）。

改成走核心的匯出器。Android 本來就走它，所以兩個平台匯出的 PDF 結構相同 ——
不是各寫一個「差不多」的產生器。

### 切換不能掉內容

核心的文件模型沒有 3D 模型與連結卡片這兩種型別，直接切過去會讓它們從匯出的
PDF 裡消失。把它們算繪成圖片再帶進去 —— 使用者看到的是同一個東西，
只是在 PDF 裡它是一張圖而不是可轉的模型。

核心拒絕這份資料時退回原本的點陣匯出：拿得到一份看得見內容的 PDF，
比拿到一個錯誤訊息好。

### 測試測得到什麼、測不到什麼

**測得到**：PDF 裡有 `/Subtype /Ink`、有 `/InkList`（實際座標）、有 `/Annots`
（掛到頁面上，否則檢視器根本不顯示）、有 `/BS`（筆寬，沒有的話別的 App 會用
自己的預設重畫，使用者看到的是「我的字變粗了」）、**三筆畫就是三個標註**
（合併成一個的話在別的 App 裡只能整組刪除）。

**測不到**：Goodnotes 認不認得它。那要真的把檔案丟進去 —— 已產出範例 PDF
交給使用者實測。

### D-05：先算錢再談模式

每年約 **US$99**，只有 Apple 開發者帳號；伺服器、模型託管、文件網址全部 US$0。
這不是需要商業模式的專案，是需要一百美金的專案。

已決議「不營收 + 捐贈連結」。`.github/FUNDING.yml` 與 `docs/SUPPORT.md` 備妥，
**帳號待使用者自己填** —— 我不會替他建立帳號。`SUPPORT.md` 也明確寫下
不會做的三件事（廣告、賣資料、關鍵功能閉源後收費），免得以後重新討論。

### 驗證
- `cargo test --workspace` 821
- `KairumoTests` **113/113**（新增 PDF 標註互通 9）
- Android instrumented **77/77**（新增 Ink 標註 1）
- iOS 與 Mac Catalyst 建置成功

---

## 2026-09-13 (22) · S-45 掌拒收回（Apple）與 S-54 錯誤訊息在地化

### 先更正清單

S-54 寫著「目前 44 個鍵」—— 那是舊的，實際已經 512 條。但**真正的缺口還在**，
只是位置不一樣：不是介面標籤，是**錯誤訊息**。它們全部是寫死的繁體中文。

英文或日文使用者出錯時看到中文，等於這個訊息對他完全沒有作用 ——
而錯誤訊息正是最需要看得懂的那一刻。

### S-45：Apple 的手寫模式原本沒有掌拒

畫布在手寫模式下是 `drawingPolicy = .anyInput` —— 手指能畫，代價是
**手掌也能畫**。Apple 端從來沒有用過核心的 `InkArbiter`。

解法分兩層：

1. **系統層**：偵測到觸控筆就切成 `.pencilOnly`。PencilKit 的掌拒做在系統層，
   比我們在應用層能做的任何判斷都可靠 —— 有現成的就不要自己寫。
   但不能一律 `.pencilOnly`：沒有 Apple Pencil 的人需要用手指寫字。
2. **收回**：切換是有延遲的。使用者的自然動作是手掌先碰螢幕、筆才落下，
   那一瞬間手掌已經畫出東西了。依核心的 `retract` 把筆落下前 500ms 內
   完成的筆畫收回 —— 時間窗與核心仲裁器用同一個值。

觸控觀察寫在 `AdaptiveCanvasView` 本體而不是 extension：在 extension 裡
override UIKit 方法雖然編得過，但那是靠 `@objc` 動態派發矇混，
不是 Swift 保證的行為。而且一律呼叫 `super`，PencilKit 的繪製路徑不受影響。

**沒有 `creationDate` 的筆畫一律保留** —— 收回一筆不確定的筆畫，
比留下一道誤觸的線嚴重得多。

### S-54：背景執行緒也要查得到字串表

錯誤訊息是在背景執行緒組成的（遷移、備份、同步都跑在背景），而
`LocalizationManager.localized` 是 `@MainActor` 隔離的。新增
`localizedUnsafe` 與一份目前語言的快照：字串表是唯讀的靜態資料，
從哪個執行緒讀都一樣。

另一個細節：一個訊息裡有好幾個 `%1@` / `%2@`。用 `replacingOccurrences`
一次全換的話，會變成「原稿 3 頁、套件 3 頁」這種永遠相等的句子。

### 三項需要你或硬體才能往下走

- **S-44**：我裝不了 Goodnotes / Notability / PDF Expert。但先知道一件事 ——
  目前匯出的 PDF 是**點陣合成**，不含 `/Ink` 標註。在別的 App 裡
  「開得起來、可以在上面加註」成立，但我們的筆畫不是可編輯的標註物件。
- **S-35**：目前沒有 Windows 版，這一條在有 Windows 版之前不成立。
  內容本身是結論不是待辦。
- **S-40**：需要實體數位板與藍牙筆。核心已區分 `Pen`/`Eraser`/`Mouse`，
  Android 已對應 `TOOL_TYPE_ERASER`；缺的是按鈕事件對應與實機校準。

### 驗證
- `cargo test --workspace` 821
- `KairumoTests` **104/104**（新增掌拒 10、錯誤在地化 4）
- Android instrumented **76/76**（新增錯誤訊息在地化 1）
- iOS 與 Mac Catalyst 建置成功

---

## 2026-09-13 (21) · 個人資料備份與一鍵復原

### 容器格式在核心

備份檔要能在任何一台裝置上還原 —— iPad 上做的備份換到 Android 也要開得起來。
兩個平台各寫一份格式的話，那件事第一天就不成立。所以容器、索引、校驗全部
在 `ffi_backup`，平台層只負責「哪些東西要進去」與「使用者按了什麼」。

佈局：魔術字 + 版本 + JSON 索引（逐檔路徑／大小／SHA-256，加上平台設定）
+ 連續的檔案內容 + 整體 SHA-256。

**為什麼逐檔都要自己的雜湊**：只有整體雜湊的話，備份檔壞掉時只知道「壞了」，
不知道壞在哪 —— 而使用者最想問的正是「我的筆記還在嗎」。逐檔雜湊讓還原時能
明確說出哪幾個檔案不可信，其餘照樣救回來。

### 三個「寧可不做」的決定

**壞掉的檔案不寫入。** 寧可少一個檔，也不要用壞掉的內容覆蓋掉使用者現有的資料。

**新版格式拒絕解析。** 猜著解析的後果是還原出一堆看起來正常、其實錯位的檔案。

**路徑不可信。** 備份檔可能來自別處，`../` 能寫到目錄之外就是一個可以覆蓋
使用者其他檔案的漏洞。有測試專門釘這一條。

### 復原之前一定先備份現況

使用者按下復原的那一刻，手上那份資料就要被覆蓋了。萬一備份檔本身有問題，
沒有安全網他會**同時失去兩份**。

### 測試抓到一個會毀掉備份的 bug

檔名原本只帶到秒。同一秒內建立的兩份備份會同名而互相覆蓋 —— 而「復原前先
備份現況」正好就發生在使用者按下復原的同一秒，結果是**安全備份把要復原的
那一份蓋掉**，復原出來的是剛才那份毀損的資料。

三條 Apple 測試同時紅了才看出來。檔名改成帶日期時間 + 一段亂碼，
安全備份另外標上 `safety` 讓使用者分得出來。

### 設定也要帶，但不是全部

Apple 只挑 `kairumo.` 開頭的鍵；整包 UserDefaults 倒出來會夾帶系統與其他框架
的東西，還原時寫回去可能造成很難追的行為。二進位值（security-scoped bookmark）
也不帶 —— 換一台裝置就失效，帶過去只會讓使用者以為同步資料夾還在。
Android 同理排除 `sync.treeUri`。

### 驗證
- `cargo test --workspace` **821**（新增備份容器 10）
- `KairumoTests` **90/90**（新增備份 12）
- Android instrumented **75/75**（新增備份 10，每條對應 Apple 端同名那條）
- Android 模擬器實測：選單按下「建立備份」產生
  `Kairumo-2026-09-13T16-40-22-1bea7b.kairumobackup` 並叫出分享面板
- iOS 與 Mac Catalyst 建置成功

---

## 2026-09-13 (20) · 問題 3＋5：固定頁面模型

### 根因不在繪圖程式碼

畫布的座標寬度原本就是「視窗寬度」。同一則筆記在 Mac 寬視窗寫的物件座標可能
在 x=1400，到了 iPhone 就在畫面外；匯出時再依當下的寬度取一次圖，
於是「畫布上看到的」與「匯出的」永遠對不起來。**那是座標空間的錯，
不是繪圖程式碼的錯** —— 先前只修繪圖是治標。

### 尺寸只有一個來源

`padnote_doc::{PAGE_WIDTH, PAGE_HEIGHT}` = 800 × 1132（A4 直式比例），
透過 `standard_page_size()` 開到 FFI，Apple 與 Android 各自的 `PageGeometry`
都從它取值。各寫一份常數的話遲早會差一點點，而差一點點的後果是同一則筆記
在 iPad 上是三頁、在 Android 上變成四頁。

維持 800 寬是為了讓既有內容的**水平位置完全不動**；改成真正的 A4 595×842
會讓所有東西水平縮放，動得比需要的多。

### 界線必須就是匯出邊界

畫布上畫出頁面邊緣（實線）與可列印區（虛線）。頁面之外畫成灰底 ——
寬螢幕上畫布比頁面寬，不畫的話使用者會以為整片都能寫。

**畫一條僅供參考的框線比不畫還糟**：他會相信它。所以匯出端一併改掉 ——
`PageThumbnailRenderer` 不再用傳進來的畫布寬度取圖，一律以頁面為準。

### 「延長本頁」移除，改成自動準備下一頁

四個入口的按鈕全部拿掉，自動延長改成：筆跡接近頁尾時把下一頁準備好。
刻意**不自動翻頁** —— 使用者可能只是把最後一行寫到很下面，畫面自己跳走比
繼續留在原地更糟。也只在目前是最後一頁時才新增，否則在中間的頁面寫到底
會憑空多出一堆空白頁。

### 既有過長頁面的重新分頁

`PageRepagination` 比照 WP4c 的紀律：先備份、備份失敗就停手、
**每一本都要驗過才算數**（重新分頁前後筆畫總數與物件總數必須相同，
少一個就還原這一本並記成失敗）、一本失敗不影響其餘、可重入、可回滾。

兩個設計決定值得記下來：

**頁次偏移先算成一張表**，不邊搬邊算。邊搬邊算的話後面的頁次會被前面剛插入的
頁影響，錯位一頁就整本亂掉，而且很難看出來。

**橫跨分頁線的筆畫整筆搬到它開始的那一頁**，不從中間切斷。切斷會把一個字
剖成兩半，那比讓它稍微超出界線更難接受。

五種附件各有 `pageIndex` 與 `y` 卻沒有共同型別，所以抽了一個
`MutablePagePositioned` 協定一視同仁地搬 —— 漏掉一種的後果是那種物件
全部留在原本的頁次上。

### 驗證
- `cargo test --workspace` 811
- `KairumoTests` **78/78**（新增重新分頁 13）
- Android instrumented **65/65**（新增頁面幾何 4）
- Android 模擬器實測：畫布上畫出可列印區的虛線界線
- iOS 與 Mac Catalyst 建置成功

---

## 2026-09-13 (19) · 問題 4：雲端同步（採建議 A）

### 動手前先發現一個會默默吃掉手寫的問題

架構不變式 1 是「每台裝置只寫自己 `device_id` 的檔案」—— 有了它，檔案層級的
衝突在數學上不可能發生，同步就只是把檔案湊在一起。`doc/ops/` 一直遵守這條。

**但筆畫檔不是。** 它原本叫 `ink/<page>.strokes` —— 每頁一個檔、與裝置無關。
單機用完全正常；一旦把套件放進共用資料夾，兩台裝置在同一頁上寫字就會寫同一個
檔名，後到的那份蓋掉先到的，使用者的手寫就這樣不見了，**而且沒有任何錯誤訊息**。

改成 `ink/<page>-<device>.strokes`，讀取時把該頁所有裝置的檔案串起來。
串接順序不影響結果：`materialize` 會先收齊墓碑再過濾。舊版命名的檔案繼續讀得到
—— 使用者手上已經有那種檔案，讓它們消失會是最嚴重的一種資料遺失。

四條測試釘住：兩台裝置的筆畫都要在、檔名確實分開、舊版命名仍讀得到、
在另一台擦掉的筆畫不會復活。

### 為什麼聯集複製就夠了

每個檔案都是 append-only 且只有一台裝置在寫，所以兩邊都有時，較長的那份必然是
較新的**超集**。同步因此退化成「把對方沒有的送過去、把自己沒有的拉回來」——
不需要合併，不需要仲裁，不需要伺服器。

這個推論成立的前提就是上面那個修正。沒有它，整個方案是錯的。

### 策略在核心，I/O 在平台層

Android 的雲端資料夾是 SAF 的 tree URI，**沒有 POSIX 路徑**，Rust 碰不到；
Apple 端則是可以直接讀寫的檔案系統路徑。兩邊的 I/O 天差地遠，但「複製哪些、
往哪個方向」必須完全一樣 —— 兩個平台對「同步」有不同的理解，在使用者眼中
就是資料遺失。所以 `plan_folder_sync` 放在核心，兩邊共用。

`manifest.json` 是唯一整份覆寫的檔案，「較長的是超集」對它不成立。
兩邊都改過時不挑邊，保留雲端那份並告訴使用者 —— 默默挑一個的後果是
另一台裝置改的標題無聲消失。

### 踩到的坑：SAF 會自己補副檔名

`createFile("application/octet-stream", "manifest.json")` 建出來的是
**`manifest.json.bin`**。那樣的資料夾兩個平台都讀不懂，而且建立本身回報「成功」。
修法是依副檔名給 MIME，建完再驗一次名字，對不上就改名，改不回來就當作失敗 ——
不要留下一個名字不對的檔案。

另一個相關的：SAF 的 `"w"` 不保證截斷。覆蓋較短的舊版時要用 `"wt"`，
否則尾端殘留上一版的位元組，壞掉的是雲端那一份。

### 為什麼不做建議 B（密語金鑰）

依你的決定只採 A。記在 `plans/account-sync-evaluation.md`：B 的代價是
「忘記密語 = 永久失去那本筆記」，那件事沒有伺服器就沒有補救。

### 驗證
- `cargo test --workspace` **811 通過**（新增多裝置筆畫 4、同步策略 8）
- `KairumoTests` **65/65**（新增雲端同步 10）
- Android instrumented **61/61**（新增資料夾同步 10，每一條都對應 Apple 端同名那條）
- iOS 與 Mac Catalyst 建置成功、Android 建置成功

### 待實機
「iCloud Drive / Google Drive 有沒有真的把檔案傳過去」是作業系統與雲端服務的事，
不是我們的程式碼。兩台真實裝置指到同一個資料夾的端到端驗證列在 `TODO.md`。

---

## 2026-09-13 (18) · WP8：出得了可安裝的檔案

### 文件的版本號原本沒人管

WP8 的驗收條件是「`bump-version.sh` 一次更新四處版本（Cargo、Apple、Android、
文件）且無漂移」。檢查的時候發現文件那一處從來沒被納入，而且**已經漂了**：
手冊的英文版停在 2.1.1、其他語言停在 2.3.0，程式是 2.3.4 —— 使用者拿到的
說明書標示的版本跟手上的 App 對不起來。

現在腳本會更新 `manual.js` 與 `privacy.html` 的「適用版本」，並在寫入後驗證
沒有殘留的舊版本號。替換刻意只針對那幾行與行文中明確的 `Kairumo vX.Y.Z`
範例 —— 全檔盲目換數字會動到日期、尺寸與快捷鍵。pre-push hook 也一起提交這兩份。

### 簽章：不做的事比做的事重要

`build.gradle.kts` 從 `keystore.properties`（已忽略）或 `KAIRUMO_*` 環境變數
讀簽章設定。沒設定時 release 版就是未簽章的 —— 這比「靜默用 debug 金鑰簽下去」
好：用 debug 金鑰簽的 AAB 上傳 Play Console 會被拒絕，而錯誤訊息不會告訴你原因。

這支腳本不會替使用者產生正式金鑰，也不把任何密碼寫進版控。上架金鑰弄丟等於
這個 applicationId 再也更新不了，那把鑰匙必須是他自己的。

`--test-sign` 會產一把**拋棄式**金鑰，只為了在本機驗證「簽章 → 安裝 → 啟動」
這條路是通的。它的密碼就寫在腳本裡，所以用完立刻刪掉設定檔 —— 留在原地的話，
之後任何一次 `./gradlew bundleRelease` 都會靜默用它簽，而簽出來的東西
看起來跟正式版一模一樣。

### 不開混淆，而且這是有意識的選擇

UniFFI 的 Kotlin 綁定透過 JNA 以**名稱**對應原生符號。被 R8 重新命名之後
會在執行期才炸開，而且是在使用者手上炸。體積的代價（約 2MB）換一個不會在
半夜出事的發佈版本，值得。

AAB 依 ABI 與密度切分，但**語言不切** —— 六國語系在同一份字串表裡，
切了會缺字。

### 踩到的坑

把 `docs/` 複製進 assets 的那個工作只掛在 `merge*Assets` 上，release 建置直接
被 Gradle 擋下來：lint 會在檔案還沒到位時去讀那個目錄。改成所有會碰到 assets
或 lint 的工作都依賴它。

另外：裝過 release 簽章的版本之後，`connectedDebugAndroidTest` 會以
`INSTALL_FAILED_UPDATE_INCOMPATIBLE` 失敗（簽章不同）。先移除再跑即可，
但錯誤訊息本身看起來像測試壞了。

### 驗證
- `./scripts/android-release.sh --test-sign` 產出 AAB 27MB、通用 APK 81MB
- **release 簽章的 APK 實際安裝到模擬器、啟動、寫得出字**，
  `versionCode=15 / versionName=2.3.4` 與其他所有來源一致
- `./scripts/bump-version.sh` 現在回報「Cargo.toml / project.yml /
  project.pbxproj / build.gradle.kts / 使用者文件 全數對齊」
- Android instrumented 51/51、`cargo test --workspace` 799、
  iOS 與 Mac Catalyst 建置成功

### CI
既有的 Android job 加上「組不簽章的 AAB」與 AAB 產物上傳；另外新增一個跑
instrumented 測試的 job（需要模擬器與 KVM，所以獨立）。**CI 設定沒辦法在本機
驗證**，第一次推上去可能要調。

### 下一步
剩下的都是人要做的：產生正式上傳金鑰、Play Console 建立應用程式、上傳 AAB 到
內部測試軌、隱私權政策填現成那份的網址。以及 `TODO.md` 裡那批實機待測。

---

## 2026-09-13 (17) · WP7：ML Kit 手寫辨識

### 辨識結果只進索引，不碰筆跡

辨識出來的文字寫進核心的搜尋索引（`index_handwriting`），**不會**取代或修改
任何一筆畫。手寫筆記的價值就在那個手寫，辨識只是讓它搜得到。

### 分組：逐筆會垮、整頁會糊

一個中文字往往是好幾筆，逐筆送去辨識會整個垮掉；整頁一次送則會把相隔很遠的
內容硬湊成一句。依**書寫停頓**切分是最接近人怎麼寫字的切法（門檻 700ms，
起點值，需實機以真實書寫節奏調整）。

一個容易寫錯的細節：停頓要從上一筆的**結束**時刻量，不是開始時刻。一筆長橫畫
寫了 600ms、下一筆 100ms 後落下，那是連著寫的；從開始時刻量會算成停了 700ms
而錯誤切開。有測試專門釘這一條。

每一組的文字只掛在該組的**第一筆**上。掛在每一筆的話，搜一個詞會回報好幾個
命中，使用者看到的是一堆重複結果。

### 沒有 Google Play 服務就要講清楚

架構決策 D2 原則上不引入 Google 服務相依，手寫辨識是已接受的例外 ——
裝置端沒有同等品質的替代。代價是沒有 Play 服務的裝置用不了，那種情況
必須明確講出來：靜默失敗的話，使用者會以為是自己的字太醜。

四種失敗各有自己的說法（沒有服務／沒有該語言的模型／下載失敗／辨識失敗），
有測試確認每一句都講得出使用者能據以行動的內容。模型只在 Wi-Fi 下下載 ——
數 MB 的模型用行動網路默默抓，不是我們該替使用者做的決定。

### 踩到的坑

`RecognitionContext.builder().build()` 看起來像個合理的預設值，實際上辨識時會丟
`Missing required properties: preContext`。我們本來也沒有前文可給（這是整頁
手寫，不是輸入法的接續輸入），改用不帶 context 的多載。

**這個錯是使用者自己看到的** —— 訊息直接出現在畫面上，而不是靜默什麼都沒發生。
那個設計當場就回本了。

### 驗證
- Android instrumented **51/51**（新增分組 4、Ink 轉換 2、語言模型 2、失敗訊息 1、
  索引與搜尋 1）
- 模擬器實機操作：寫三筆 → 選單「辨識手寫」→ 模型自動下載 → 回報
  `Indexed 3 group(s): 1l1`（三條斜線辨識成 1、l、1，合理）
- 「辨識結果進入索引且搜得到」用固定文字走同一條回填路徑做成確定性測試 ——
  自動化測試裡不去測 Google 的服務
- `cargo test --workspace` 799、iOS 與 Mac Catalyst 建置成功

### 待實機
中英文各 20 句的辨識率要在實機用真的筆寫才算數，已列入 `TODO.md`。

---

## 2026-09-13 (16) · WP6：Android 平台功能

### 先問清楚核心到底還給不給得出東西

Android 版關掉了 `asr` 與 `pdf` 兩個 feature。「綁定裡有這個函式」不等於
「它在這台裝置上會動」，所以先寫了 `CoreCapabilityTest` 實測而不是從函式名單
推論。結果：

| 能力 | 狀態 |
|---|---|
| 匯出 PDF／單頁 PDF | 可用（`%PDF` 檔頭、內容 > 500 位元組） |
| 匯出 PNG | 可用（合法 PNG 簽章） |
| 匯出 Markdown | 可用 |
| 列印資料 | 可用 |
| **錄音** | **可用** |
| 全文搜尋 | 可用 |

**關鍵發現：`asr` 關掉的是轉錄，不是錄音。** libopus 有交叉編譯進來，
`feed_audio` 確實寫得出音框。所以「第一版不含語音轉錄」不該連錄音都做不了 ——
差點就照字面把錄音一起砍掉。

`pdf` feature 關掉的是 PDFium（**讀** PDF），與匯出 PDF 不是同一件事 ——
匯出是 `padnote-export` 自己產的。

### 匯出全部走核心

PDF、PNG、Markdown、列印資料都是核心產生的，平台層只負責落檔與交給系統面板。
理由是 Apple 端剛踩過的坑：匯出若在平台層另外畫一次，畫出來的東西遲早會跟
畫布上的不一樣（那次是整頁空白）。

分享的兩個細節各有測試守著：URI 必須走 `FileProvider`（`file://` 丟給別的 App
在 Android 7 之後直接丟例外），以及 `FLAG_GRANT_READ_URI_PERMISSION` 不能漏
（漏了的話接收方拿到一個開不起來的連結，而寄件者這邊完全看不出問題）。
匯出檔只放 App 私有 cache，不落外部儲存 —— 那是使用者的筆記內容。

### 說明文件：一份來源，兩個平台

手冊與隱私權政策由 Gradle 在建置時從 repo 的 `docs/` 複製進 assets。

第一版直接把整個 `docs/` 掛成 assets 目錄 —— 結果 DEVLOG、TODO、STATE、ADR、
內部計畫全都進了 APK。改成只複製 `manual/` 與 `legal/` 兩個子目錄，
並加一條測試專門擋這次手滑。

另一件事：這兩份文件是為了網頁發佈而寫的**片段**（沒有 `<html>`，也沒有
viewport meta）。直接 `loadUrl` 的話 WebView 會用桌面寬度排版，手機上整頁縮成
看不清的小字。`DocsViewer` 補上最小外框與 viewport，並保留 asset 的 base URL
讓 `manual.js` 與 `img/` 的相對路徑還找得到。

### 工具列重做：功能點不到就等於沒做

第一版把所有動作排成一列再讓它水平捲動。在 320dp 寬的螢幕上，錄音、匯出、
列印、手冊全部被推到畫面外，而且捲不太動 —— 也就是 WP6 做的東西一個都碰不到。
改成兩個切換留在列上、其餘進溢位選單。過程中還發現「兩個切換 + 兩個按鈕」
就足以把最右邊那個擠出畫面，而被擠掉的永遠是最後加上去的那個。

說明文件也從 `AlertDialog` 改成全螢幕 `Dialog`：文件是要「讀」的，
塞進固定高度的小框裡，使用者看到的是一小條白色。

### 驗證
- Android instrumented **41/41** 通過（新增能力探測 7、匯出 7、文件打包 5）
- 模擬器實機操作：開選單 → 匯出 PDF，畫面回報
  `Exported: kairumo-….pdf`，檔案確實落在私有 cache（2,809 位元組）；
  手冊在全螢幕下正確排版，語言下拉與目錄都在
- `cargo test --workspace` 799、iOS 與 Mac Catalyst 建置成功

### 待實機
錄音在模擬器上沒有麥克風可測。程式路徑（權限請求 → `AudioRecord` 16kHz 單聲道
float → `feed_audio`）已就緒，核心端有實測憑據，實機驗證列在 `TODO.md` 的 A-07。

---

## 2026-09-13 (15) · 修正：實機上畫布全白

### 先更正一件事

上一則把「模擬器截圖是白的」解釋成「`screencap` 抓不到前緩衝圖層」。
**那個判斷是錯的。** 實機回報一模一樣的徵狀 —— 畫布全白 —— 證明前緩衝路徑
根本沒把墨跡顯示出來，而我當時把一個真實的失敗合理化掉了。

### 真正的原因：Kotlin 的名稱遮蔽

`drawStroke` 是寫在 `Canvas` 上的擴充函式。函式內的 `density` 解析到的是
**`Canvas.density`**（接收者的成員蓋過外層類別的屬性），而不是建構子傳進來的
螢幕密度。未設定的 `Canvas.density` 是 0，於是：

```
drawLine(a.x * density, …)   // 全部乘成 0 —— 整條筆畫塌到原點
paint.strokeWidth = … * density   // 線寬 0
```

編譯器不會有任何警告，畫面上就是一片空白。

找出來的方式不是讀程式碼，是把多重緩衝層的底色暫時改成淡黃色 ——
畫布變黃就代表表面看得見、問題在筆畫；還是白的就代表表面被蓋住。
一次建置就把可能性砍半。接著印出 `paint.strokeWidth`，`sw=0.0` 直接指到兇手。

欄位已改名為 `pxPerDp`，這個遮蔽陷阱不會再犯第二次。

### 另外兩個實機才看得到的問題

**SurfaceView 預設在視窗底下。** 靠在視窗上打洞才看得見，而 Compose 的
`Surface` 會畫一層不透明底色把洞蓋住。加上 `setZOrderOnTop(true)` 與
半透明格式才顯示得出來。（這是「白色」那一層；上面那個 bug 是「空白」那一層 ——
兩個問題疊在一起，所以單改一個都看不出進展。）

**「ⓘ」不是每個裝置的字型都有。** 沒有的話按鈕變成一塊看不見的區域 ——
實機上使用者就是「找不到右上角的按鈕」。改成文字 `Info`。

### 預設改回一般畫布

低延遲預設**關閉**。它還沒在實機上驗過，不能擋在使用者與「能不能寫字」之間；
已經驗過會動的那條路才該是預設值。要試的人自己打開，兩條路徑可以直接互比。

### 畫面上加了診斷列

`tool=1 r=0.0dp p=0.00 d=1.0 draw=1 rej=0 ges=0`

使用者手上的裝置我碰不到，所以一張截圖必須要能告訴我平台回報了什麼 ——
工具類型、換算後的接觸半徑、壓感、密度、仲裁結果。沒有這條線，遠端除錯只能用猜的。
順帶把「事件回報」改成每個事件都報，不是只有畫得出東西時 ——
被拒絕的那些才是要診斷的。

### 驗證
- 模擬器逐像素檢查：修正前畫布區只有一種顏色（底色），修正後出現 1,056 個
  純黑像素與抗鋸齒灰階 —— 兩條路徑都真的畫得出墨跡
- Android instrumented 22/22、`cargo test --workspace` 799、iOS 與 Mac Catalyst 建置成功

### 仍未解釋
實機回報「Stylus Only 按不動」。低延遲預設關閉之後畫面上不會有 SurfaceView，
若仍然按不動就是另一回事 —— 新增的診斷列會顯示觸控事件有沒有進到畫布。

---

## 2026-09-13 (14) · WP5c/d：前緩衝渲染與預測筆跡

### 預測筆跡只進畫面，永不進資料

`MotionEventPredictor` 猜的是「筆接下來大概會到哪」，用來補「手已經到了、
畫面還沒跟上」的視覺落差。它是**猜測**，不是使用者真的寫下的東西。所以
預測點只畫到前緩衝，絕不餵給 `InkEngine`、絕不進 `.padnote` —— 寫進去的話，
使用者的筆記裡會混進他沒寫過的線條。這是既有的架構不變式，不是這裡的臨時決定。

實作上還有一個容易漏的細節：畫完預測線**不更新**「最後一個點」。更新的話，
下一段真實軌跡會從猜測的位置接續，誤差一路累積下去。

### 延遲量測：先說清楚它量的是什麼

新的 `InkLatencyMeter` 量的是「取樣點在硬體上發生的時刻
（`getEventTimeNanos`）→ App 把它交給合成器」。**這不是筆尖到光子的完整延遲**，
面板本身的掃描與亮起時間量不到，那要靠高速攝影機。

說清楚比給一個好看的數字重要：拿它去跟別家 App 的「延遲」比較，只有在對方
用同一個定義時才有意義。它真正有用的是**同一台裝置上開關前緩衝的前後對比**。

量測器本身也有測試守著三件事：`percentile(100)` 不能越界（那正是最該看的
最差情況）、時鐘不同源算出的負延遲要丟掉而不是汙染統計、緩衝區不能無上限
成長（連續書寫十分鐘不該讓量測器自己變成記憶體問題）。

### 失敗時要退回去，不是黑畫面

各家 OEM 對前緩衝的支援不一致。建不起來就回報 `false`，UI 自動退回一般的
Compose 畫布並顯示一行說明。手寫可以比較鈍，但不能不能用。使用者也能手動
切換開關，直接比較兩條路徑。

### 踩到的坑

**按「清除」只清資料不夠。** 前緩衝與多重緩衝各自持有已經畫上去的像素，
不主動清的話，按了清除畫面還是舊的墨跡，要等下一次抬筆才更新。

**多重緩衝層每次 commit 都是全新的緩衝。** 只畫「這次新增的」的話，抬筆瞬間
先前的筆畫會整片消失 —— 要重畫全部內容。

**窄螢幕上工具列把畫布推下去。** 標題加了 `weight(1f)` 被擠成一欄一個字，
整個版面塌掉。工具列改成水平捲動，而資訊鈕留在捲動區外 —— 它被推出畫面的
時候，正好就是使用者要看延遲數字的時候。

### 驗證（模擬器能驗的部分）
- Android instrumented 測試 **22/22** 通過（新增 4 條延遲量測）
- 模擬器實測：前緩衝渲染器成功建立、線段與多重緩衝重畫都確實被呼叫、
  筆畫數正確、「清除」會清空
- iOS 與 Mac Catalyst 建置皆 **BUILD SUCCEEDED**

**模擬器驗不了的部分**：前緩衝圖層不會出現在 `screencap` 的畫面裡，而延遲數字
在模擬器上（p50 44ms）反映的是 `adb input` 的注入速率與模擬器的合成，
不是真實硬體的表現。實機加觸控筆的實測結果待補。

---

## 2026-09-13 (13) · WP5a/b：Android 筆跡輸入與畫布

### 掌拒不寫第二份

核心早就有 `padnote-input` 的 `PointerArbiter`，而且已經透過 `InkArbiter`
開到 FFI。Android 沒有理由再寫一套判定邏輯 —— 那會變成「同一支筆在兩個平台
行為不一樣」，而且是那種只有使用者會發現的不一樣。所以 Kotlin 這一層只做
**如實轉換**：`MotionEvent` → `FfiPointerEvent`。

### 轉換層有三個錯了不會當掉的地方

1. **只讀 `getX()` 會掉點。** S Pen 取樣率遠高於畫面更新率，一個
   `ACTION_MOVE` 裡通常塞著好幾個歷史取樣點。不讀 `getHistorical*`，
   快速書寫就會變成折線。測試裡塞了三個歷史樣本，斷言四個點一個都不能少。
2. **接觸半徑要換成 dp。** 核心的手掌門檻是 22「點」。直接餵 px，在 3x 螢幕上
   連筆尖都會被當成手掌 —— 而在 1x 的模擬器上完全正常。這種錯最難在開發機上
   發現，所以測試直接釘住 1x 與 3x 兩組數字。順帶：`touchMajor` 是**直徑**，
   核心要的是長半徑。
3. **`AXIS_ORIENTATION` 是 -π..π，核心要 0..2π。** 負值會被核心夾成 0，
   於是扁筆頭的方向在螢幕左半邊全部錯，而且不會有任何錯誤訊息。

### 測試抓到核心的一個真實缺口

原本寫的掌拒測試是「手掌碰一下、抬起、筆才落下」，結果失敗 ——
`retract_suspects` 只看 `active`，而已經抬起的指標在 `on_ended` 就被移除了，
收不回來。

這不是測試寫錯。使用者把手放上螢幕時，手掌常常是「碰一下、滑一小段、
隨著手安定下來抬起」，接著筆才落下 —— 那一小段在筆落下之前就結束了，
畫面上留下一道誰也不知道哪來的短線。模組註解自己寫著「只擋得住一半的情況」，
指的就是這個。

修法：`PointerArbiter` 多留一個 `recent_draws`，記住**剛結束**且被採納為墨跡的
非筆指標，收回時間窗內一併回報。配套四條測試：超出時間窗的舊筆畫不能被抹掉、
同一筆畫不能被收回兩次（平台層會去刪第二次，可能誤刪別的）、已經被擋掉的
不該回報（去刪一個不存在的東西）、換頁後要忘記上一頁。

### 畫布走原始事件，不走 Compose 手勢

`pointerInteropFilter` 而不是 Compose 的指標事件：Compose 的手勢層已經把歷史
取樣點合併成「這一幀的位置」了。手寫必須拿原始 `MotionEvent`。

寬度用核心 `half_width()` 的同一條公式 `base × (0.35 + 0.65 × pressure)`，
與 Apple 端的轉換器同源 —— 同一筆畫在兩個平台粗細才一樣。

### 驗證
- `cargo test --workspace`：799 通過（`padnote-input` 由 33 增為 38）
- Android instrumented 測試 **18/18** 在模擬器上通過，含：
  手掌（接觸半徑 45dp）不產生任何筆畫、筆畫寫進 `.padnote` 後重開讀得回來、
  被收回的筆畫重開後不會復活、歷史取樣點不掉點、dt 夾在 65535µs
- 模擬器實機操作：畫 6 筆有 6 筆；開啟「僅限觸控筆」後手指畫出 0 筆
- iOS 與 Mac Catalyst 建置皆 **BUILD SUCCEEDED**

instrumented 測試而非純 JVM 測試是刻意的：`MotionEvent` 的建構與歷史點機制
只有真的 Android runtime 才有，用 mock 測這一層等於在測自己寫的假物件。

### 還沒做
- **WP5c 前緩衝渲染**（`androidx.graphics.lowlatency`）與 **WP5d 預測筆跡**
  （`MotionEventPredictor`）。這兩項的效果是**延遲**，模擬器上量不出來，
  各家 OEM 對前緩衝的支援也不一致。沒有實機與觸控筆就驗不了，
  所以先不寫 —— 寫一段驗不了的程式碼，只是把風險藏起來。

---

## 2026-09-13 (12) · WP4c：儲存層遷移（先寫回滾，再動資料）

### 這一步的規則寫死，不留彈性

這是整個 WP4 裡唯一會碰到使用者真實資料的一步，所以 `NotebookMigration`
的每一條規則都是硬性的：

1. **原始檔案一律不覆蓋、不刪除。** 遷移只讀 `notebooks_v1.json` 與
   `Drawings/`，產物寫到 `Documents/Packages/`。有一條測試逐檔比對位元組，
   確認遷移前後原檔完全相同。
2. **動手前先備份。** 備份失敗就整個停手 —— 沒有退路的遷移不該開始。
3. **每一本都要驗過才算數。** 匯出完立刻讀回來，逐頁比筆畫數、逐筆比取樣點數、
   逐點比座標。驗不過的那一本把套件**刪掉**並記成失敗：留下一份看起來成功、
   其實內容不對的檔案，比明確失敗更危險 —— 使用者會信任它。
4. **一本失敗不影響其餘**，報告逐本交代結果與原因。
5. **可重入**：重跑不會產生重複，也不會白做已經遷好的。
6. **可回滾**：從備份還原並清掉產物。

### 指紋不能只看修改時間

可重入要靠內容指紋判斷「這本變了沒」。第一版想用 `lastModifiedDate` ——
錯的。手繪是存在獨立的 `.drawing` 檔，改手繪**不會**更新那個日期，
所以使用者畫了一整頁之後重跑會被判定成「沒變」而跳過。指紋改成
修改時間 + 每頁筆畫數 + 附件數，並有一條測試專門釘住這件事。

另一個相關的坑：核心是 append-only 的，舊套件沒清掉就重匯，內容會**疊起來**
（2 筆 + 3 筆變成 5 筆）。重跑前先刪舊套件，也有測試守著。

### 測試自己寫錯的那一條

`testRerunningSkipsUnchangedNotebooks` 第一次是紅的。原因是測試每次呼叫
`sampleDocuments()` 都產生新的 `lastModifiedDate`（預設 `Date()`），
兩次各叫一次等於資料真的變了 —— 那樣測到的是「有變就重做」，
不是這條要測的「沒變就跳過」。改成兩次餵同一批文件。

### 有按鈕，使用者才碰得到

能力建好卻沒開到使用者面前，等於沒建 —— 這個坑先前踩過。所以在
「系統診斷與版本資訊」裡加了「跨平台格式」區塊：狀態、轉換按鈕、
失敗逐本說明、還原備份、以及一句說明「原始筆記不會被更動」。
放診斷頁而不是主畫面：這是進階動作，不該是使用者第一天就會按到的東西。

遷移**不會在啟動時自動執行**。在啟動路徑上偷偷跑一次會動到使用者的資料，
而且出事時他們連自己做過什麼都不知道。

UI 測試從選單一路按到轉換完成，確認狀態列真的變成「已轉換 N 本」且無失敗。
過程中順手把首頁選單與轉換按鈕加上 accessibility identifier —— 原本靠
`label CONTAINS 'ellipsis'` 去猜，會先命中筆記卡片上的那個「⋯」。

### 型別名稱長度（iOS 閃退的那個舊帳）

新增的區塊會進 `AppDiagnosticsSheet.body` 的型別名稱，所以量了一次：
editor 7,102、home 6,523、diagnostics 1,988 字元。當初造成 iOS 閃退的是
89,768，距離很遠。

### 驗證
- `cargo test --workspace`：794 通過、0 失敗
- `KairumoTests`：40 條全過（遷移與回滾 16 條）
- `KairumoUITests`：含新增的遷移流程，全過
- iOS 與 Mac Catalyst 建置皆 **BUILD SUCCEEDED**
- Android `assembleDebug` 成功（字串表同步到 440 條）

### 還沒做的
App 的**讀取路徑**仍走既有的 JSON + `.drawing`。把讀取切到核心格式是另一件事，
而且要在使用者實際跑過遷移、確認產物無誤之後才值得談 —— 在同一次改動裡
既搬資料又換讀取來源，回滾就失去意義了。

---

## 2026-09-13 (11) · WP4b：iOS 匯出的 `.padnote`，在 Android 上打得開

### 驗收條件達成了

「iOS 建立的多頁筆記（含手寫、文字方塊）在 Android 開啟後，筆畫數、座標、
顏色與頁面高度與原稿一致。」這句話現在有實測憑據，不是推論：

| 項目 | iOS 原稿 | Android 讀到 |
|---|---|---|
| 標題 | 跨平台測試筆記 | 跨平台測試筆記 |
| 頁數 | 2 | 2 |
| 第 1 頁 | 2 筆畫、預設高 | 筆畫 2、高 1800pt |
| 第 1 頁首筆 | 紅、起點 (10, 200)、12 點 | RGBA(255,0,0,255)、(10.0, 200.0)、12 點 |
| 第 2 頁 | 1 筆畫、拉長到 3200pt | 筆畫 1、高 3200pt |
| 第 2 頁首筆 | 綠、起點 (40, 200)、12 點 | RGBA(0,255,0,255)、(40.0, 200.0)、12 點 |

流程是完整的那條：Swift 的 `NotebookDocument` → 核心 `.padnote` 套件 → 落到
磁碟 → 搬到 Android 模擬器 → Kotlin 用同一個核心開起來。

### 補上格式的兩個缺口

做到一半才發現核心格式根本存不下兩樣東西，而且兩樣都在驗收條件裡：

**頁面尺寸。** `Page.size` 存在，但沒有任何 `DocOp` 寫得進去。Kairumo 的畫布
可以向下延長，使用者拉到 3200pt 的那一頁若沒落盤，另一個平台會看到一頁被截短
的筆記。新增 `SetPageSize`（op 30）。

**區塊座標。** `Block.position` 同樣存在卻沒有對應操作 —— 文字方塊與圖片的擺放
位置進不了 op-log，跨平台打開會全部擠在一起。而且**在原本的平台上看不出來**，
因為位置是另外存在 JSON 裡的。新增 `SetBlockPosition`（op 31）。

還補了 `page_id_at(index)`：在此之前平台層只拿得到第一頁的 id，多頁筆記走不完。

`format-spec.md` 的操作清單同步更新（順帶補上先前漏記的六個表格操作，實際是
31 種而非文件寫的 21 種）。`covers_all_op_tags` 這條測試在我加完 op 卻沒更新
清單時立刻紅了 —— 它就是為此存在的。

### 匯出是新增的路徑，不是替換

`NotebookPackageBridge` 只讀 `NotebookDocument` 與 `PKDrawing`，另外寫一份套件。
現有的 JSON + `.drawing` 讀寫一行都沒改，使用者手上的資料不會被動到。
儲存層的正式遷移（含備份與回滾）排在下一步 —— 那才是會碰到真實資料的改動，
不該跟轉換器的對錯混在同一次。

### 踩到的坑

**adb push 進 App 專屬外部目錄，App 自己反而讀不到。** 推進去的檔案屬於 shell，
App 得到的錯誤訊息是「不是 .padnote 套件」—— 看起來像檔案壞了，其實是權限。
改用 `adb shell run-as` 解壓進內部 `filesDir`。

**讀不到某張圖不該讓整本筆記匯不出去。** 缺一張圖，跟整份匯出失敗，對使用者是
完全不同等級的損失。缺的附件略過，其餘照常匯出，並在摘要裡回報實際帶出去幾張。

**Swift 型別推導在內嵌算式上會直接放棄。** 測試資料裡一行 `CGSize(width: 6 -
CGFloat(i) * 0.25, …)` 就讓編譯器丟出 "unable to type-check this expression in
reasonable time"。把中間值先算出來即可。

### 驗證
- `cargo test --workspace`：794 通過、0 失敗
- `KairumoTests`：23 條全過（`InkInterop` 14 + `NotebookPackageBridge` 9）
- Android 模擬器實測：見上表
- iOS 與 Mac Catalyst 建置皆 **BUILD SUCCEEDED**

### 下一步
WP4c：儲存層遷移。先寫遷移與回滾的測試，再動 `NotebookStore` —— 這一步會碰到
使用者手上的真實資料，備份與可回滾是前提。

---

## 2026-09-13 (10) · WP4a：筆畫轉換器與往返保真度

### 這一步只做「證明得了」的部分

WP4 的驗收條件是「iOS 建立的筆記在 Android 打開後，筆畫數、座標、顏色與原稿
一致」。要能檢查這句話，得先有兩樣東西：核心能把筆畫**完整**吐回來，以及
Apple 端有一個 PKDrawing ⇄ 核心格式的轉換器。這一步先把這兩樣做出來並釘上
測試；儲存層的實際遷移（含備份與回滾）留到下一步，因為那會動到使用者手上的
真實資料，不該和轉換器的對錯混在同一次改動裡。

### 核心：`FullStroke` 與 `visible_stroke_details`

原本的 `visible_strokes` 刻意只回傳摘要（id、點數、外框）—— 一頁數萬個點過
FFI 很慢，渲染路徑本來就不該走那裡。但摘要驗不出座標錯位、壓感遺失、筆刷對應
寫反這些錯，所以另開一條互通專用的出口 `FullStroke`，把每個取樣點原樣帶出來。
兩者的分工寫進了註解，免得日後有人拿 `FullStroke` 去做渲染。

### 壓感不從 `PKStrokePoint.force` 取

直覺寫法是 force 夾到 0–1 當壓感。實際上 PencilKit 沒有公開 force 的數值範圍，
手指與 Apple Pencil 也不一致，硬夾的結果是重壓的筆畫整段飽和。改成從
PencilKit **實際畫出來的每點寬度**回推 —— 依核心 `half_width()` 的
`width = base × (0.35 + 0.65 × pressure)` 反解。這樣 Android 端用同一條公式
畫出來的粗細，才會跟 iPad 上看到的一樣。測試逐格掃過 0–1 確認兩個方向互為反函數。

### 踩到的坑

**tilt 與 altitude 是互補角，不是同一個東西。** PencilKit 的 altitude 是
「筆與螢幕平面的夾角」（π/2 為垂直握筆），核心的 tilt 是「偏離垂直的角度」。
直接對接的話，筆越立起來核心會以為越躺平 —— 而且畫面上完全看不出來。

**第一版 Rust 測試紅在測試自己身上。** 我把 pressure / tilt / azimuth 也寫成
位元組相等，結果失敗。格式規格 §5.4 明訂這三個欄位各用一個 u16 定點數儲存
（整點 16 bytes），本來就有量化誤差。把期望改成「誤差不超過一個量化格」，
這條測試才擋得住真正的錯（欄位接錯、角度用錯最大值）。同樣地，Swift 端的
altitude 容差要放到 1e-4：PKStrokePoint 存 0.6 讀回來是 0.60001…，那是
PencilKit 的儲存精度，收得比它還小只會紅在一個不能也不該修的地方。

### 新增 Apple 單元測試 target

轉換器是純邏輯，塞進 UI 測試裡跑很浪費。新開 `KairumoTests`（`apple/Tests`），
14 條測試在 0.02 秒內跑完。UI 測試維持原樣。

### 對現有 Apple 功能的影響：無

`InkInterop` 是一組純函式，沒有被畫布或 `NotebookStore` 呼叫 —— 現有的
iOS / iPadOS / macOS 路徑一行都沒改。核心那邊是新增兩個方法，既有 API 不動。

### 驗證
- `cargo test --workspace`：790 通過（新增 5 條 `s47_ink_interop`）、0 失敗
- `KairumoTests`：14 條全過
- iOS 與 Mac Catalyst 建置皆 **BUILD SUCCEEDED**
- Android `assembleDebug` 成功（`.so` 與 Kotlin 綁定已用新 FFI 重新產生）

### 下一步
WP4b：把轉換器接上實際的匯出／匯入 —— 從 `NotebookStore` 產生一份 `.padnote`
套件，並在 Android 端開起來比對。儲存層的正式遷移排在那之後。

---

## 2026-09-13 (9) · WP3c：介面字串收斂成單一來源

### 為什麼要動這 430 條字串

它們原本只活在 `LocalizationManager.swift` 的字典字面值裡。Android 一旦自己
再寫一份，同一句話就有兩個版本 —— 改了一邊忘了另一邊只是時間問題，而且是那種
上架後才被使用者發現的錯。所以收斂成 `i18n/ui-strings.json`，由
`scripts/i18n_tool.py` 同時產生 Swift 與 Kotlin 兩份表。

硬前提是不影響 Apple 現有行為，所以做法刻意保守：**產生出來的字與原本逐字相同**，
`LocalizationManager.localized(_:)` 的公開簽名與回退順序（目前語系 → en → 繁中 →
key 本身）一個字都沒動，只是字典改從產生檔取。

### 憑據不是「我覺得一樣」

`i18n_tool.py verify` 把產生的 Swift 檔與 Kotlin 檔各自**讀回來**再跟 catalog
逐條比對，三者必須完全相同才算過。這條檢查也進了 pre-push hook —— 有人手改產生檔
就會在推送前被擋下來。

### 踩到的坑：跳脫被做了兩次

第一次 verify 就抓到 4 條不一致。原因是從 Swift 抽出來時把 `\"` 當成兩個字元
原樣收進 catalog，輸出時 `swift_escape` 又跳脫一次，變成 `\\"` —— 畫面上會多出
反斜線。修法是讓「還原跳脫」只發生在讀取端、「跳脫」只發生在輸出端，各一次。
這正是為什麼要有 verify：肉眼看 430 條 JSON 不會看出這種差別。

### 順手補掉的漏洞：Android 沒進版本腳本

`bump-version.sh` 當初只顧 Cargo 與 Apple 專案檔。新加的 `build.gradle.kts`
沒被納入，就會重演 v1.4.0 那次的版本漂移，只是換個平台。現在版本來源、寫入與
寫入後驗證都包含 Android，pre-push hook 也會一起提交它。Android 畫面上的版本號
改讀 `BuildConfig`，不再是手寫的字串。

### 驗證
- `cargo test --workspace`：785 通過、0 失敗
- iOS（generic/platform=iOS）與 Mac Catalyst 建置皆 **BUILD SUCCEEDED**
- Android `assembleDebug` 成功，裝到模擬器實測畫面顯示「字串表：430 條」、
  示例字串正確取到 `about_app`
- `i18n_tool.py verify`：Swift 與 Kotlin 產生表都與 catalog 完全一致（430 條）
- XCUITest 冒煙測試：4 項全過（**TEST SUCCEEDED**）

順帶修掉一件會讓上面這條驗證形同虛設的事：`KairumoUITests` 少了
`GENERATE_INFOPLIST_FILE`，`xcodebuild test` 會在簽章階段就失敗 —— 那不是
「測試失敗」，是測試根本沒跑，而錯誤訊息長得很像單純一則 build 警告。

### 下一步
WP4 檔案互通：把 Apple 端的儲存層換成核心格式（含 PKDrawing → `add_stroke`
轉換器與資料遷移），兩邊的筆記檔才真的能互開。

---

## 2026-09-13 (8) · WP3b：協同中繼進核心 FFI，但**預設關閉**

### 為什麼是 feature 而不是直接換掉 Apple 的那份

Apple 版的 Swift 中繼（Network.framework）已經在使用者手上穩定運作。
把 tokio 拉進 iOS 的二進位不但沒有好處，還會動到已經穩定的功能 ——
硬前提就是不准這樣。所以 `relay` 這個 feature **預設關閉**：Android 建置時開，
Apple 不開。兩邊仍然共用 `padnote-relay` 的同一份 JSON 協定，
加密走 WP3a 的 `padnote_crypto::session`，所以 iOS 與 Android 能進同一個房間。

驗證這件事本身也要有憑據：`cargo tree -p padnote-core` 在未開 feature 時
**tokio 出現 0 次**，開了才是 3 次。

### bind 與 serve 拆開

`padnote-relay` 原本的 `run_server` 是「綁定完就進迴圈」，沒有機會回報實際埠號。
App 端常常用 port 0 讓系統挑，然後必須把**實際**綁到的埠顯示給使用者（邀請隊友
要填）。所以拆成 `bind()` / `serve(listener, hub, shutdown)`。

### 踩到的坑：不要在 FFI 裡 block_on

第一版 `RelayServer::start` 用 `runtime.block_on(bind(addr))`，測試立刻炸：
`Cannot start a runtime from within a runtime` —— 呼叫端本身在 async runtime 的
執行緒上。改成用同步的 `std::net::TcpListener` 綁定（順便就拿到埠號），
設成非阻塞後再交給 runtime。這樣不管呼叫端在什麼執行緒上都安全。

### 另一個坑：綁定要用相同的 feature 組合產生

Kotlin 端出現 `Unresolved reference: RelayServer` —— 因為產生綁定那一步用的是
預設 feature 的函式庫，裡面根本沒有 relay 的型別。建置腳本已改為用與 Android
相同的 feature 組合產生綁定。

### 驗收

- `crates/padnote-core/tests/relay_ffi.rs`：起中繼 → 兩個 WebSocket 客戶端加入
  同一房間 → 驗 joined / peers / peer_joined / oplog 轉發 → 停止。2 個測試通過。
- Android 模擬器實機：畫面顯示「協同中繼：已啟動於埠 36451（已停止）」。
- 硬前提回歸：`cargo test --workspace` 785 通過、iOS 建置 BUILD SUCCEEDED。

---

## 2026-09-13 (7) · WP3a：協同加密下沉到核心，並用跨語言測試證明相容

### 兩邊的加密其實是兩套演算法

盤點發現核心與 Apple 版用的根本不是同一個東西：

| | 演算法 | 用途 |
|---|---|---|
| `padnote-crypto::envelope` | XChaCha20-Poly1305 + Argon2id | **落盤**的同步檔案 |
| Swift `CollaborationManager` | AES-256-GCM（CryptoKit） | **即時協同訊息** |

也就是說協同訊息這條路，核心從來沒有實作過。Android 若自己再寫一份，
兩邊要在同一個房間裡互相解得開就得逐位元組對齊 —— 那是典型的安靜失敗。

### 做法：核心實作「與已上線 Apple 版相同」的那一套

新增 `padnote_crypto::session`，AES-256-GCM，位元組佈局照抄 CryptoKit 的
`AES.GCM.seal(...).combined`：

```text
combined = nonce(12) || ciphertext || tag(16)
```

**刻意不動 Swift。** 硬前提是不影響已穩定的 Apple 版，而且使用者手上已經有
在跑的 iOS 版 —— 讓核心去配合既有格式，Android 就能直接與它互通，Apple 端
一行都不用改。日後 iOS 要改走核心也不會有 wire 格式變動。

### 相容性不是用推論的，是測出來的

`crates/padnote-crypto/tests/cryptokit_interop.rs`：Rust 加密 → 真的叫 `swift`
用 CryptoKit 解；CryptoKit 加密 → Rust 解。雙向都通過。
沒有 Swift 工具鏈的機器（Linux CI）自動跳過，不會因此變紅。

FFI 曝露 `session_key_generate` / `session_seal` / `session_open` 三個函式，
Android 端在模擬器上實機跑過 round-trip：畫面顯示「AES-256-GCM round-trip 通過」。

新增相依 `aes-gcm`（連同 aes / ctr / ghash / polyval）皆為 MIT OR Apache-2.0，
符合 deny.toml 的白名單。

### 硬前提回歸

`cargo test --workspace` 785 通過、0 失敗；iOS 建置 BUILD SUCCEEDED。

---

## 2026-09-13 (6) · Android WP1–WP2：核心可編、APK 跑起來了

決策（專案擁有者拍板）：**路線 2（一次到位，Apple 儲存層改用核心格式）**、
**minSdk 29**、**第一版不含語音轉錄**。硬前提：不得影響現有 iOS / iPadOS / macOS。

### WP1 · 讓核心在 Android 編得出來

三個真實阻擋，逐一解掉：

**1. ONNX Runtime 沒有 Android 預編譯檔。** `ort-sys` 的 build.rs 直接 panic。
`padnote-core` 切出 `asr`（Silero VAD + 中文標點）與 `pdf`（PDFium）兩個 feature，
**預設全開 → Apple 端相依與行為完全不變**；Android 用 `--no-default-features` 建置。
VAD 在 feature 關閉時退回內建的能量式 VAD，錄音本身照常。

**2. libopus 缺席。** 這個最陰險：第一顆 APK 裝起來、跑起來，然後畫面顯示
`dlopen failed: cannot locate symbol "opus_encoder_destroy"` —— `.so` 帶著未定義
符號出貨了。Android 沒有系統 libopus，而 `audiopus-sys` 交叉編譯時不會自己建。
新增 `scripts/build-android-opus.sh`：用 NDK 的 CMake 工具鏈把 libopus 編成靜態庫
（下載後比對 xiph 官方 SHA256SUMS，符合 D4 的下載驗證規則），再由
`OPUS_LIB_DIR` 指給 audiopus-sys 靜態連結。

**3. audiopus-sys 沒有宣告 `rerun-if-env-changed`。** 換 ABI 時 cargo 會沿用上一個
ABI 的建置結果，於是 x86_64 的 .so 又帶著未定義符號。建置腳本改為逐 ABI
先 `cargo clean -p audiopus_sys` 再編。

驗收：`llvm-readelf --dyn-syms` 對兩個 ABI 的 `libpadnote_core.so` 查未定義 opus
符號，皆為 **0**；`NEEDED` 只剩 libc / libm / libdl。

### WP2 · Gradle 骨架與綁定整合

`android/`（Compose、minSdk 29、targetSdk 35、AGP 8.7.3 / Gradle 8.11.1），
UniFFI Kotlin 綁定經 JNA 呼叫 `.so`。CI 新增 android job：建 .so → 組 APK → 上傳產物。

驗收：模擬器（Pixel / Android 15）安裝執行，畫面顯示 **由 Rust 回傳的**
核心版本 2.3.0 與 `android/aarch64` —— Kotlin ⇄ UniFFI ⇄ Rust 這條路確認打通。

### 硬前提的回歸驗證

| 檢查 | 結果 |
|---|---|
| `cargo test --workspace` | 778 通過 · 0 失敗 |
| iOS 模擬器建置 | BUILD SUCCEEDED |
| Mac Catalyst 建置 | BUILD SUCCEEDED |

（clippy 有 4 個既有警告，位於 `ffi.rs` 的 insert_shape/insert_connection 與
`app.rs` 的表格迴圈，與本次改動無關；STATE.md 說的「零警告」已經過時。）

---

## 2026-09-13 (5) · 匯出是空白的，以及文字方塊的互動重做

### 匯出 PDF／圖片全是空白

`exportAsPdf` 寫死 `CGRect(0, 0, 612, 792)`（信紙尺寸）並且**只畫 PKDrawing**。
但畫布的座標系是「視圖寬度 × 頁面高度」，在 Mac 上常常是 1500×1800 以上 ——
所以那個框只截到左上角一小塊，使用者寫在中間的內容完全落在框外，
匯出檔看起來就是一片空白。文字方塊、圖片、3D 與圖釘也從來沒被畫進去。

這跟先前「側邊欄縮圖只畫 PKDrawing」是同一個錯誤，只是這次還多了尺寸寫死。

修法：把縮圖用的整頁合成抽成 `PageThumbnailRenderer.renderFullPage`
（不裁切、不取快取、可指定 scale），匯出 PDF／PNG／列印三條路徑共用它，
並且**每一頁用自己的尺寸開 PDF 頁** —— 頁面可以被「向下延長」，高度不一定相同。

驗證方式（`KAIRUMO_EXPORT_AUDIT=1`）：在模擬器裡合成一份帶手繪筆劃（刻意畫在
舊版取圖框之外的 y≈1000）、文字方塊（自訂底色＋紅色 3pt 邊框）與討論圖釘的頁面，
走匯出路徑輸出 PNG。結果三種內容都在圖裡，位置與畫布一致。

### 文字方塊：刪不掉、莫名的雙向箭頭、不能就地編輯

- 刪除鍵原本是三顆**無標示的小圓點**，壓在方塊右上角、還往外偏移 —— 難點也看不懂。
  改成帶文字的按鈕列（編輯／邊框／刪除）浮在方塊上方，並補上**右鍵／長按選單**
  （就地編輯、文字排版、邊框、刪除）—— 那是大家最先嘗試的操作。
- 移除右下角的「雙向箭頭」縮放把手：它只能改寬度、又小又會擋住文字。
  寬度改到「文字排版」面板裡用滑桿調。
- **點兩下＝就地編輯**：直接在畫布上改字，不必先開面板。
- 文字排版面板補上：自訂底色（ColorPicker）、邊框顏色（六色 + 自訂）、
  邊框粗細（三段）、圓角（直角／圓角／大圓角）、方塊寬度滑桿。
  `NoteTextAttachment` 新增 `borderColorHex` / `borderWidth` 兩個可選欄位
  （舊檔解碼時落到預設值，不影響相容性），匯出算繪也會套用同樣的邊框樣式。

---

## 2026-09-13 (4) · 畫布自適應、可拖曳捲軸、筆記拖放分類、套索工具列

### 收合側欄後畫布沒有變寬

`updateUIView` 是在 SwiftUI 狀態改變時呼叫，那個時間點 `bounds.width` 還是
**版面變動前**的舊值 —— 所以收合結構欄之後 contentSize 仍停在「扣掉側欄」的
寬度，右側空出一塊灰色。真正知道新寬度的時機是 `layoutSubviews`。

改用 `AdaptiveCanvasView`（PKCanvasView 子類）在 `layoutSubviews` 重算
contentSize 與樣板背景的尺寸。順帶拿掉寫死的 800pt 下限 —— 畫布內容寬度
就是視圖寬度，縮圖算繪也改用同一個基準。

UI 測試驗證：開啟筆記 → 收合側欄 → 斷言畫布寬度 > 視窗寬度的 85%。

### 游標拖不動捲軸

iOS 的捲動指示器是**純顯示**的，不接受互動。在 iPad 上沒差，但在 Mac 上跑時
使用者會很自然地想用游標去拖它。新增 `CanvasScrollbar`：自己畫一條捲軸，
接受拖曳與點擊軌道，直接設定 `contentOffset`；捲動狀態由
`scrollViewDidScroll` 回報給 SwiftUI。

### 未分類筆記可以拖進資料夾

側欄的筆記列加上 `.draggable(note.id)`，資料夾列與「未分類檔案」標題加上
`.dropDestination`，懸停時高亮。拖進資料夾即分類，拖回「未分類」即移出。

### Copy 與 Duplicate 的差別，以及缺少的「貼上」

畫布上長按出現的 Cut / Copy / Delete / Duplicate / Insert Space Above 是
**PencilKit 自己的系統選單**，不是我們畫的，也改不了它的文字。差別是：
Copy 進剪貼簿等你貼上，Duplicate 直接在旁邊多一份。

問題在於**那個選單只在有選取時出現**，複製完取消選取就沒有貼上的入口了。
所以在我們自己的套索工具列補上「貼上」與「再製」，並把純圖示按鈕改成
圖示＋文字＋說明提示（游標停留會顯示「複製到剪貼簿，之後用『貼上』放到
想要的位置」這類說明）。

---

## 2026-09-13 (3) · iOS 實機閃退：SwiftUI 型別名稱把主執行緒堆疊撐爆

### 症狀與當機記錄

實機（iPhone 17 Pro / iOS 27）一碰筆記或頁面就閃退，模擬器（iOS 18 與 27、
Debug 與 Release）與 Mac 上都正常。當機記錄：

```
Exception Type:  EXC_BAD_ACCESS (SIGSEGV)
Exception Subtype: KERN_PROTECTION_FAILURE ... Stack Guard
Thread 0 Crashed:
0..56  swift::Demangle::TypeDecoder::decodeMangledType / decodeGenericArgs  ← 遞迴 57 層
57     swift_getTypeByMangledName
58     swift_getTypeByMangledNameInContext
59-60  Kairumo
61     SwiftUICore ViewBodyAccessor.updateBody
```

不是我們的程式碼在崩，是 **Swift runtime 解析型別名稱時遞迴到爆堆疊**。

### 根因：body 的 mangled 型別名稱長達 89,768 字元

SwiftUI 把 `body` 整棵子樹的型別（包含每個 `.sheet` 內容的完整型別）編進
外層的 mangled 名稱。量測結果：

| | 修正前 | 修正後 |
|---|---|---|
| `NotebookEditorView.Body` | **89,768** 字元 | **6,478** |
| `HomeWorkbenchView.Body` | 31,977 | 6,002 |

**為什麼只在實機當：iOS 主執行緒堆疊只有 1MB，模擬器有 8MB。** 同一份程式碼
在模擬器上解析得完，在實機上遞迴到一半就撞上 Stack Guard。這也解釋了為什麼
使用者在 Mac 上跑同一個 iOS 二進位（Designed for iPad）不會當 —— 那裡的
主執行緒堆疊同樣寬裕。

### 修法：在大型子視圖插入型別邊界

把 20 個大型子視圖改成回傳 `AnyView`（`editorTopBar`、`drawingToolbar`、
`canvasWorkArea`、`allNotebooksSection` …），並用 `erasedView { }` 包住每個
`.sheet` / `.fullScreenCover` 的內容。父視圖的型別裡只剩 `AnyView` 三個字，
子樹的型別各自獨立解析。

代價是那些子樹失去結構化 diff，多一點重繪成本 —— 相對於實機必當，這個交換
很划算。

**量測方式留在程式裡**：`KAIRUMO_TYPE_AUDIT=1` 啟動時會印出兩個 body 的
型別名稱長度。這是唯一能在模擬器上看見這個問題的方法，改大型視圖後應該複查。

### 一併修掉

- 版本號：`MacWindowTitle` 原本包在 `#if targetEnvironment(macCatalyst)` 裡，
  但使用者在 Mac 上跑的是 **iOS 版**（`otool -l` → `platform 2`），整段程式
  沒被編譯進去 —— 這是版本號改三次都沒出現的真正原因。拿掉條件編譯，
  並在編輯器工具列直接畫上版本標籤，不再只依賴系統視窗標題。
- 討論串可以刪除單一則留言；已解決時多一條明顯的狀態列。
- 操作手冊與隱私權政策打包進 App，首頁「說明與條款」離線可讀（WKWebView）。
- 兩份文件的語言切換改成下拉式選單。

---

## 2026-09-13 (2) · 拖曳抖動、視窗標題守門員，與一個會覆蓋筆記的 Binding

### 圖釘浮層拖曳抖動：又是 .local 座標系的回授迴圈

拖曳手勢用預設的 `.local` 座標系，而手勢掛在被 `.offset` 位移的視圖上：
位移改變 → 手勢座標系跟著動 → translation 重算 → 位移再變，一個迴圈。
畫面就是劇烈抖動。改用 `DragGesture(coordinateSpace: .global)`，
全域座標不受自身位移影響。

這與 ADR 之外那條「畫布物件拖曳晃動」的成因是同一類 —— 值得記成通則：
**手勢若會改變自己所在視圖的位置，座標系就不能用 .local。**

順帶修好對話框標題列被擠壓（作者名稱直排、「標記為已解決」變成直書）：
寬度 320→360，名稱與按鈕文字加上 `lineLimit(1)` 與 `fixedSize()`。

### Mac 視窗標題：加上守門員計時器

上一版加了重試與場景通知，但使用者在**編輯器**裡仍看不到版本號 ——
`fullScreenCover` 推上來時 SwiftUI 會重設 `scene.title`，而那個時機
沒有對應的通知可掛。補上每秒一次、只在值不同時才寫入的守門員。
實測（Mac Catalyst 執行 + `CGWindowListCopyWindowInfo` 讀標題）：
首頁與編輯器都穩定顯示 `Kairumo v2.1.0`。

### ⚠️ 編輯器切換筆記會覆蓋掉目前這一則

測試途中在編輯器的資料夾目錄點另一則筆記，畫面整片空白。追下去發現：

```swift
// 舊的 switchToNotebook
self.notebook = updatedTarget   // notebook 是 $store.notebooks[index] 的 Binding
```

`notebook` 是 `$store.notebooks[index]`，index 在浮層呈現當下就算好了。
把另一份文件寫進這個 Binding，等於**把目前這則筆記在陣列裡的那一格
覆蓋成目標筆記** —— 原本那則的紀錄消失，陣列裡出現兩筆相同 id，
`ForEach` 的識別壞掉，畫面整片空白。

三層修正：
1. 編輯器不再自己寫 Binding，改為回呼 `onRequestSwitch`
2. 新增 `NotebookEditorHost` 持有「現在是哪一則」的 id，每次重算索引 ——
   切換只是換 id，不動任何一則筆記的內容，也不必關掉再重開浮層
3. `loadData` 載入時依 id 去重（保留最後修改的一筆），讓已經寫壞的檔案
   下次啟動自己痊癒，而不是一直卡在空白畫面

本機測試資料確認 5 則筆記都還在（記憶體中的覆蓋沒有落盤）。

---

## 2026-09-13 · UI 缺陷修正：結構側欄、圖釘浮層、內建協同中繼

三個實機回報的缺陷。

### 線上協同連不上，是因為後面根本沒有人在聽

預設位址 `ws://127.0.0.1:9002` 是 Rust 版 `padnote-relay` 監聽的埠，
但 App 從來不會去啟動那支程式，使用者手上也沒有可連的服務 ——
於是每次都停在「連線中斷，正在自動重新連線 (第 4/5 次)」。

改成由 App 自己提供中繼：`LocalRelayServer`（Network.framework 的
`NWListener` + `NWProtocolWebSocket`）在位址指向本機時自動啟動，
協定與 `crates/padnote-relay/src/hub.rs` 完全一致（第一位加入者為房主、
presence/oplog 轉發、最近 200 筆 oplog 供斷線追趕、房空自動回收）。

這不違反 D5。D5 說的是「我們不營運雲端後端」，不是「裝置不能開房間」——
發起協同的那台裝置當中繼點，資料仍然不經過任何我們的伺服器。
埠被占用時不當成錯誤：那通常代表已經有一個 `cargo run -p padnote-relay`
在聽，照常連過去就好。

順帶把失敗原因顯示出來。原本連不上只剩無聲的轉圈，使用者無從判斷是
位址錯、埠沒開、還是對方關機了。

### 圖釘對話框固定在圖釘旁邊，等於擋住要討論的東西

對話框原本用 `.position()` 釘在圖釘附近、完全不能移動 —— 而圖釘一定
就插在要討論的那塊內容上。加上標題列拖曳與「縮小成標題列」，
縮小後整條列仍可拖曳（縮小卻不能移動只是換個地方擋住畫面）。

### Mac 視窗標題看不到版本號

標題原本在 `onAppear` 裡 `DispatchQueue.main.async` 設一次。那個時間點
`connectedScenes` 可能還是空的（視窗場景尚未接上），設定就靜靜地掉了 ——
標題退回 App 名稱「Kairumo」。SwiftUI 的 `navigationTitle` 之後也可能覆蓋
`scene.title`。

改成 `MacWindowTitle`：場景還沒出現就短暫重試，並在場景／視窗／App
重新啟用時再確認一次，只有目前值不同才寫入（不會閃爍）。
順便把版本字串收斂成單一來源 `AppVersion` —— 先前三個檔案各自從
`infoDictionary` 撈，還各寫了不同的硬編碼 fallback（1.0.0 / 1.2.0 / 1.4.0），
版本一升級就互相對不起來。

驗證：實際建置 Mac Catalyst 版執行，用 `CGWindowListCopyWindowInfo`
讀回視窗標題為 `Kairumo v2.0.0`，等待數秒後仍未被覆蓋。

### 窄寬度的工具列：ViewThatFits 需要一個「一定塞得下」的候選

`ViewThatFits` 在所有候選都放不下時，會**無條件採用最後一個**，並以它的
理想寬度算繪 —— 於是工具列比視窗還寬、左右對稱被裁掉。iPad 直向與 iPhone
上「首頁」與「匯出與列印」直接消失，連退出筆記都做不到。

補上 `WrapLayout`（SwiftUI `Layout`）當最後一個候選：由左至右排、放不下就換行，
所以永遠有一個塞得下的版本。退讓順序刻意是「收文字標籤 → 收進『更多』選單 →
才換行」——換行會移動按鈕位置，是最後手段，不是第一選擇。

關鍵是**每顆按鈕都要是 Layout 的直接子視圖**：舊的 FlowLayout 版本把筆刷群組
包在 `HStack` 裡，那一整段變成單一子視圖，永遠不換行、照樣溢出。所以工具列
內容抽成 `@ViewBuilder` 的一串攤平項目，單行版與換行版共用同一份；
只有真的需要整組一起移動的（色盤、undo 群組）才刻意包成一個 `HStack`。

驗證：iPad 直向工具列換成兩行、iPhone 上主工具列兩行＋繪圖工具列四行，
所有按鈕都在畫面內；iPhone 維持側欄收合。

### 側邊欄縮圖的比例對不上畫布

縮圖固定用 800pt 寬算繪、卡片固定 130pt 高。但畫布的實際內容寬度是
`max(視圖寬度, 800)` —— 在 Mac 上是 1500 左右，物件的 x/y 存的又是畫布座標，
所以縮圖不只比例不對，位置也偏。再加上頁面預設 1800pt 高，`scaledToFit`
進 130pt 高的卡片後整頁只剩約 50pt 寬，物件小到看不出是什麼。

改成：縮圖用畫布的實際寬度算繪（由 GeometryReader 回報），卡片改為維持
同樣的長寬比、寬度撐滿側邊欄；頁面高度超過寬度 1.25 倍的部分裁掉，
底部畫一道漸層表示還有內容。物件在縮圖裡放大約 5 倍，且與畫布等比。

### 「打字」切不過去，是分段控制項把選項拆平了

模式切換用 `.pickerStyle(.segmented)`，每個選項裡卻放 `HStack { Image; Text }`。
UIKit 的分段控制項只吃單一 Text 或單一 Image —— HStack 會被拆成好幾段，
`.tag()` 跟著失效：畫面上看得到「打字」，點下去永遠回到手繪。
改成兩顆自己畫的按鈕，狀態由 `editorMode` 直接驅動。
切換前先 `saveCurrentPageDrawing()`，否則切模式時的畫布重建會吃掉未落盤的筆跡。

### 結構側欄預設展開

開啟筆記時看不到資料夾目錄，等於把「這則筆記放在哪裡」藏起來。
改為進入編輯器就展開，分頁選擇（頁面／資料夾）用 `@AppStorage` 記住。
iPhone（compact 寬度）維持收合 —— 280pt 側欄會把畫布擠到不能用。

### 驗證到哪裡
中繼服務有獨立的端到端煙霧測試（兩三個 WebSocket 客戶端實際加入同一
房間，驗證 joined / peer_joined / oplog 轉發 / catchup / peer_left），全數通過。
兩項純 UI 行為只驗證到可建置，實機互動待確認。

---

## 2026-09-12 (12) · 畫布物件、流程圖、工具列與六國語系（S-46 ～ S-50）

五項需求一次做完：物件自由排列、插入圖片與試算表圖形、流程圖範本、
工具列模組化、六國語系。

### 堆疊順序記「絕對索引」而不是「上移一層」

`SetZIndex` 存的是目標位置，不是相對動作。相對操作在無伺服器的 CRDT 下會疊加：
兩台裝置各按一次「移到最上層」，合併後誰也說不準結果。改成絕對索引後，
後到的 op 直接覆蓋，結果與套用順序無關。

移動**只在同層內**。跨層移動等於改變父子關係，那是 `Group`／`Ungroup` 的職責；
混在一起會讓「上移一層」偶爾把物件丟出群組，使用者完全無法預期。

### 連接線要「連」而不是「畫」

流程圖的價值在於線會跟著方塊走。把箭頭存成獨立線段，移動方塊時線就斷了 ——
那只是畫圖。因此 `Connection` 存的是「從哪個圖形的哪個連接點到哪個」，
路徑在渲染時才算。測試驗的也是這件事：移動後線的端點仍落在形狀邊緣上。

形狀用 ISO 5807 的標準語意（菱形＝判斷、平行四邊形＝輸入輸出），不自創圖形，
並把 `semantic()` 的說明帶到 UI —— 使用者看得懂符號才有意義。

### 試算表變成表格物件，而不是一段 Markdown 文字

原本 xlsx 匯入後變成兩個文字區塊（表名 + Markdown 表格）。那能看不能編：
改一格要去編輯一長串 `|` 分隔的文字。現在展開成 `BlockKind::Table`，
逐格編輯、整塊可搬動。

`SetTableCell` 逐格記錄而非整表覆寫 —— 整表覆寫會讓兩人同時編輯不同格時互相蓋掉。
`cells` 用扁平陣列而非巢狀 `Vec`：增刪欄只要 splice，巢狀結構容易出現長度不一致的列。

順帶把舊測試 `xlsx_becomes_a_markdown_table` 改寫成
`xlsx_becomes_an_editable_table_object` —— 它斷言的是舊契約，留著會擋住正確行為。

### 語系完整性交給型別系統

字串表若放在各平台的 `.strings` / `strings.xml`，就有五份要各自維護的表，
漏翻譯只有跑到那一頁才發現。放在 core 並以 `[&str; LOCALE_COUNT]` 表達，
**少一個語言就編不過**。

測試不只檢查「非空」，還檢查每個語言沒有偷懶照抄英文（重疊率 < 30%）、
日文有假名、韓文有諺文、泰文是泰文字母、繁簡至少有 15 個字不同。

### 抓到的兩個真缺口

1. `ToolGroup::Extras` 的標籤鍵指到 `Key::Record` —— UI 上整個「進階」分組
   會顯示成「錄音」。補了 `GroupExtras` 鍵。發現它的是 `ALL_KEYS.len()` 的
   數量守衛，這種看似笨拙的檢查真的會抓到東西。
2. `sibling_count` 一開始用 `draw_order().len()`，那是**整棵樹**的數量。
   群組內的物件「移到最上層」會算出錯誤上界。改成同層計數並補了對應測試。

### 平台層真的碰得到

上一輪的教訓是「能力建在 core 裡卻沒開 FFI，等於沒建」。這次同批完成：
`ffi_shapes.rs`（形狀／連接線／範本）、`ffi_ui.rs`（語系／工具列）、
session 上的 z 序與表格方法。`tests/s46_canvas_objects.rs` 刻意走平台層那條路，
其中 `z_order_survives_a_reopen` 驗的是**重開後順序還在** —— 只測記憶體內的狀態
會漏掉「沒落盤」這種最尷尬的缺陷。

Swift 綁定 178 個公開 API，`swiftc -parse` 通過。763 個測試、clippy 零警告。

### 尚未完成（已記入 TODO）

- **S-52**：形狀與連接線還沒有自己的 `DocOp` —— 幾何算得出來，但插入的流程圖
  還不會落盤，重開會消失。這是這批裡最明顯的缺口。
- **S-51**：表格只能建立固定尺寸並改格內文字，不能插入／刪除列欄或合併儲存格。
- **S-53**：工具列與語言切換的實際 UI 在平台層，Rust 端只是把資料準備好。

---

## 2026-09-12 (11) · 串流調查（S-32）

### 結論：改用段級串流，幀級串流需要重新匯出計算圖

原本以為要「重新匯出帶 cache 的 encoder」。查 FunASR 的原始碼才發現
**官方的串流 encoder 匯出本來就沒有 cache 輸入**：

```python
def export_encoder_input_names(self):
    return ["speech", "speech_lengths"]
```

串流是靠兩件事達成的：
1. 餵入帶重疊的特徵窗（`overlap_feats = cat(cache["feats"], feats)`，
   保留 `chunk_size[0] + chunk_size[2]` 幀）
2. decoder 的 16 個 cache 跨呼叫保留

我把兩者都實作了：`StreamingFrontend`（fbank 與 LFR 的跨塊邊界狀態）
與 `CifState`（跨塊的積分狀態，有測試證明分塊結果等同整段）。

**但幀級串流仍然不正確。** 實測不同的特徵上下文長度：

```text
整段處理      : 欢迎大家来体验达摩院推出的语音识别模型      ← 與 FunASR 吻合
幀級串流 ctx=5 : 嗯迎你迎来家来到看体验摩摩院推的的语音式模模式
幀級串流 ctx=15: 嗯迎你迎来家来到体验达摩院的的的音音频式识模式式
幀級串流 ctx=30: 嗯迎你迎大家来体验达摩院推的的语音式式模式
幀級串流 ctx=60: 嗯迎你迎大家来体验达摩院的的的音音音式模模式
```

加大上下文能緩解但無法解決。原因是 `online: True` 匯出的 encoder
把**分塊注意力遮罩的狀態封在計算圖內**，從外部餵特徵時它無從得知
自己處於哪一個分塊位置。

### 採取的作法
**預設段級串流**：`feed()` 只累積，`finish()` 才辨識。這與
`padnote-recorder` 天然契合 —— 它給的就是 VAD 切好的語音段。

延遲因此由 VAD 的 `MAX_SEGMENT_MS` 決定，已從 8 秒調到 **5 秒**。
這是取捨：太長延遲高、太短 ASR 上下文少。⚠️ 真正的甜蜜點要用 H2 的
中文測試集實測不同長度的 CER 後才能定。

幀級串流的基礎設施留著且有測試，等能匯出暴露分塊狀態的圖時可以直接接上。

### 為什麼不硬做
繼續加大上下文只是在猜，而且每次都要人工看輸出判斷好壞 ——
沒有 H2 的測試集，我無法量化「好一點」是真的好還是我在自我說服。

---

## 2026-09-12 (10) · 簡繁轉換與完整中文管線

### 做了什麼
463 → **487 個測試**。`padnote-text`（簡繁轉換）與
`padnote-core::transcript`（後處理管線），串起完整的中文鏈：

```
ASR 輸出 : 下周三下午三点在研讨室开会请大家准时参加
管線輸出 : 下週三下午三點在研討室開會，請大家準時參加。
```

### 為什麼這是必要的一步
Paraformer 與 ct-punc 都是**簡體模型**。在此之前整條管線的輸出是
沒有標點的簡體中文 —— 對台灣使用者等於不能用。

### 授權：差一點把 GPL 拉進來
`zhconv` 的 `Cargo.toml` 宣告 `license = "GPL-2.0-or-later"`。
直接 `cargo add zhconv` 會讓專案的 Apache-2.0 立場崩掉。

但讀 README 才發現那是因為**預設綁了 MediaWiki 的轉換表**：

> The library itself is licensed under MIT OR Apache-2.0 … BUT it may bundle
> conversion tables from MediaWiki … For MIT compatibility, disable the
> default `mediawiki` feature and enable `opencc`.

改用 `default-features = false, features = ["opencc"]` 後，綁的是 OpenCC 詞典
（Apache-2.0）。**而且品質取捨剛好相反**：OpenCC 表把「下周三」正確轉成
「下週三」，MediaWiki 表沒有。授權乾淨的選項同時也是比較好的選項。

### 兩個真實缺陷

**OpenCC 表在上下文中會誤判。**
「里面」單獨轉換是對的（→裡面），但在「代數里面」中，最長匹配先吃掉
「数里」，剩下的「面」被單獨轉成食物的「麵」。以專案維護的修正表補救，
**每一條修正都有測試**，且有反向測試確保不誤傷真正的「麵條」。

**逐詞轉換會失去詞組上下文 —— 這是測試抓到的。**
ASR 的詞多半是單字。「下」「周」「三」分開轉，「周」永遠不會變「週」，
因為那需要看到整個「下周三」。改成**整段轉換後再依字數分回各詞**；
字數改變時（少數詞彙替換會）退回逐詞轉換 ——
寧可少一點上下文，也不能讓時間戳錯位。

C1（點文字跳回錄音）完全依賴時間戳不變，這條不能妥協。

---

## 2026-09-12 (9) · 中文 ASR 與標點（S-24 / S-30）

### 做了什麼
411 → **463 個測試**。`padnote-punct-ct`（中文標點）與
`padnote-asr-paraformer`（fbank + LFR + CMVN + CIF + encoder/decoder）。

### 驗證方式：每一層都有獨立的真值來源
- **fbank**：用 FunASR 產生參考特徵，Rust 實作逐點比對（最大差異 <0.05）
- **CIF**：純數學，以 one-hot 幀追蹤每一幀的貢獻
- **整體**：用 FunASR 官方範例音檔比對 Python 與 Rust 的輸出

### 關鍵發現

**fp32 的輸出與 FunASR 完全吻合，證明管線正確。**
```
FunASR (fp32 PyTorch): 欢迎大家来体验达摩院推出的语音识别模型这ca
本實作 (fp32 ONNX)   : 欢迎大家来体验达摩院推出的语音识别模型
本實作 (int8 ONNX)   : 欢迎大家来体验达摩院推出的语音识别识别模型cia
```

**int8 量化明顯降低中文辨識品質。** 出現重複字（「识别识别」）與結尾雜訊。
進一步測試排除嵌入表（只量化 MatMul）得到**相同的劣化**，體積幾乎不變
（157.4 vs 157.6 MB）—— 問題出在量化 transformer 權重本身，不是嵌入表。
⚠️ 這是 **n=1 的觀察不是測量**，真正的取捨要等 H2 的測試集。

**匯出的 encoder 沒有 cache 輸入 —— 它不是真串流模型。**
一開始我按 0.6 秒切塊送進 encoder，輸出是
「嗯迎欢们大家来关注的来体验摩的的的的推一个语语音识模模一式型模型」——
每塊各自缺少上下文。改成整段處理後立刻正確。
幸好 `padnote-recorder` 給的就是 VAD 切好的語音段，天然符合這個模型的形態。

**FunASR 的 `dither=1.0` 讓特徵非確定性。**
同一段音訊兩次呼叫的特徵相差 0.27。轉錄結果必須可重現，因此前端關掉抖動。

**ct-punc 的詞表是簡體的。**
471,067 個 token 中僅 1.3% 是單字；繁體字（線、數、點）不在詞表內。
實測簡體 0/31 未知詞、繁體 3/18。因此管線順序必須是
**先標點、再轉繁體**，這不是實作細節而是正確性需求。

### 踩到的坑
- LFR 的輸出幀數要用**補齊前**的長度算，用補齊後會多一幀
- CIF 的測試期望算錯（alphas 總和 2.0 會發射兩次，我寫成一次）——
  實作是對的，測試是錯的
- `ort` 的混合型別輸入清單需要 `.into_dyn()`

---

## 2026-09-12 (8) · 模型授權稽核

### 做了什麼
在動手接 Paraformer-zh 之前先查授權（決策 D6 要求權重與程式碼分別確認）。
產出 `models/LICENSE-AUDIT.md`，排除一個模型，把另外兩個升級為待決策事項。

### 查到什麼

**Paraformer-zh 與 ct-punc 有雙通路授權衝突。**
同一份權重，兩個散布通路條款不一致：
- HF `funasr/*` 官方 repo：metadata 標 apache-2.0，且 **repo 內含完整的
  Apache-2.0 LICENSE 檔**，README 明文宣告以 Apache-2.0 發布權重
- FunASR GitHub 的 `MODEL_LICENSE`（v1.1）：授權「**僅供參考與學習使用**」，
  另含「不得詆毀」條款與違反即自動終止

著作權人對不同通路採不同授權是常見做法，HF 那份 LICENSE 檔是明確的授權讓與。
但通路 B 的措辭若被主張適用，產品使用就有疑義。**這是帶法律性質的判斷，
不該由我單方面決定** → 列為待決策 D-07。

**SenseVoice-Small 直接排除。**
權重走 FunASR Model License v1.1，不是 Apache-2.0。社群 2026-01 開的商用釐清
issue 至今**無維護者回覆**。「不得詆毀 + 自動終止」不是 OSI 開源授權，
與 `features.md` §4 賣點 3、4 宣稱的「開源、永遠免費且不可能反悔」直接衝突。
C9（中英夾雜）改由 whisper-large-v3-turbo 承擔 —— 它本來就支援 code-switching，
授權是乾淨的 MIT。

**官方 repo 沒有 ONNX。**
`funasr/*` 只有 PyTorch `model.pt`。目前 manifest 指的是第三方轉換版。
Apache-2.0 允許衍生作品，但轉換者的宣告不能高於上游授予的權利 ——
provenance 多了一層。

### 為什麼停在這裡而不直接接
C5（中文標點還原）是 **P0** —— 「中文轉錄沒有標點等於沒用」是市調寫進規格的
結論。若 D-07 選擇不採用，這個 P0 功能需要另找方案。
換句話說，D-07 不只是「要不要多一個 ASR 引擎」，而是**會不會少一個 P0 功能**。
這種等級的取捨應該由專案擁有者決定。

### 這次稽核的方法論
不只看 metadata 標籤，而是**實際抓 LICENSE 檔、讀 README 原文、查未結的
issue**。silero 那次的教訓（URL 指向需登入頁面）已經證明：
沒被實際驗證過的宣告等於沒有。

---

## 2026-09-12 (7) · Silero VAD（S-26）

### 做了什麼
376 → **395 個測試**。`padnote-vad-silero`（ort + Silero v4）、
遲滯開關邏輯、接進 `NotebookSession` 與 FFI、修正 manifest 的壞 URL。

### 為什麼換掉 EnergyVad —— 有數字
白噪音（振幅與語音相當）餵進兩個 VAD，100 個音框：

```
EnergyVad 誤判 100 個為語音
Silero    誤判   0 個
```

能量門檻法在真實教室（冷氣、投影機風扇）等於完全不可用。

### 選型過程

**先試純 Rust 的 tract，失敗了（ADR-0005）。**
`tract-onnx` 無法載入 Silero —— v4 與 v5 都含 8k/16k 的條件分支，
兩個分支輸出秩不一致，`ToTypedTranslator` 直接失敗。這是 tract 的能力邊界，
不是設定問題。

改用 `ort` 其實是**更好的架構決定**：同一個執行期之後還要服務
Paraformer-zh（S-24）與 PP-OCRv5（D4），一套推論庫服務三個用途。

**遲滯邏輯與模型推論分離。**
`SpeechGate` 是純邏輯、零相依、完整可測。用單門檻會在機率貼近臨界時
來回跳動，把一句話切成碎片 —— 有測試明確驗證遲滯區間內零狀態切換。

**內部緩衝而非要求對齊。**
Silero v4 只吃 512 樣本，但 `Segmenter` 用 20 ms（320 樣本）。
讓 VAD 內部緩衝，呼叫端不必為了模型改變粒度。

**模型缺失時降級而非失敗。**
錄音不該因為 VAD 用不了就停擺（S-25 的音檔優先原則）。
`uses_neural_vad()` 讓 UI 能提示使用者分段品質會下降。

### 踩到的坑

**manifest 裡的 URL 從來沒被驗證過。**
原本 silero 指向的 HuggingFace 路徑需要登入，`curl` 拿到的是 29 bytes 的
`Invalid username or password`，副檔名還是 `.onnx`。
**清單裡沒被實際下載過的 URL 等於沒有。** 已改用官方 repo 並補上真實雜湊，
同時加測試要求「已填雜湊的項目不得仍標記授權待審」。

**`ort` 2.0-rc 的 API 與舊版差異大**：`session.inputs` 變成 `inputs()`、
`name` 變成 `name()`、`to_array_view` 變成 `try_extract_tensor` 回傳
`(shape, slice)` 元組。

---

## 2026-09-12 (6) · 錄音編排（S-25）

### 做了什麼
348 → **376 個測試**。`padnote-recorder`（管線 + 轉錄 worker）、
接進 `NotebookSession`、FFI 曝露、端到端整合測試、可執行的示範程式。

### 為什麼這樣做

**把「音檔優先落地」做成型別層次的保證，而不是註解裡的承諾。**
`RecordingPipeline` **完全不持有 `AsrEngine`** —— 想在 `feed()` 裡呼叫模型？
沒有那個欄位可以呼叫。這讓「轉錄拖慢錄音」與「轉錄失敗丟音訊」在結構上
不可能發生，而不是靠紀律維持。這正是 Notability 最大差評的解法。

**`feed()` 內的順序不可調換。**
先把音訊寫進 Opus 檔，再做 VAD 分段。VAD 或分段出錯時，音訊已經安全了。

**開檔先於記錄 session。**
顛倒的話，當機會留下「有 session 記錄但沒有音檔」的孤兒。

**佇列溢位丟最舊的段，但音檔照寫。**
ASR 完全跟不上時，掉的只是待辨識的副本；錄音本身不受影響，
而且 `segments_dropped` 會回報給 UI 提示換小模型。

### 踩到的坑

**實際跑一次才發現的回報缺陷。**
示範程式印出「音檔時長: 0 ms」—— 因為 `recorded_audio_us()` 讀的是 pipeline，
而停止錄音後 pipeline 已被取走。單元測試沒抓到，因為它們都在錄音中查詢。
改成累計保存並補上測試。**這就是為什麼要真的跑一次，而不只是跑測試。**

**`#[cfg(test)] impl` 放在 `mod tests` 之後會觸發 clippy 的
`items_after_test_module`。**

### 外部驗證
`cargo run -p padnote-core --example record_session` 產出真實的 `.padnote` 套件，
`ffprobe` 確認音檔為 `Duration: 00:00:03.00, opus, mono` —— 完整流程的產出
是任何播放器都能開的標準檔案。

### 仍未驗證
- VAD 目前是**能量門檻法**，有背景噪音時會把冷氣聲當語音（S-26 待換 Silero）
- 背景 worker 的低優先權執行緒尚未實作（S-27，屬平台層）
- 真實 ASR 品質仍待 H2 的中文測試集

---

## 2026-09-12 (5) · 文件持久化與原生相依

### 做了什麼
325 → **348 個測試**。文件 op-log 持久化（S-23）、Opus 音訊編碼（S-13）、
whisper.cpp ASR 引擎（S-15）、PDFium 適配層（S-21）。

### 為什麼這樣做

**S-23 的範圍比原本大。**
原本只要把文字 op 落盤，但一查才發現**頁面、區塊、錄音 session 從來沒有
持久化過** —— 只有筆畫會落盤。所以做成完整的 12 種 `DocOp` 與 `open()` 重播，
而不是只補文字那一塊。

**record() 先落盤再套用。**
反過來的話，中途當機會讓記憶體與磁碟不一致，使用者看到的是
「有效果但沒存到」—— 那比直接失敗更糟。

**先實測哪些原生相依能編譯，再決定要不要寫。**
pdfium-render、audiopus、whisper-rs 都先用 probe 專案試編。能編譯才動手，
避免寫完一堆無法驗證的程式碼。

**Opus 用 ffprobe 做外部驗證。**
自己寫的測試只能證明「跟我想的一樣」。`ffprobe` 回報
`Duration: 00:00:02.00, 32 kb/s, mono` —— granule 算錯的話時長會顯示成 1/3，
這是真正獨立的驗證。

### 踩到的坑

**UniFFI 會靜默略過名為 `open` 的建構子。**
`open` 是 Swift 的存取修飾關鍵字。Rust 端編譯正常、綁定產生正常，
但 Swift 端就是看不到那個方法。改名為 `open_existing` 並加測試釘住。
**這類無聲失敗是 FFI 最危險的地方。**

**PDFium 不是執行緒安全的。**
`Pdfium` 持有的綁定既非 `Send` 也非 `Sync`，連 `thread_safe` feature 也不足以
滿足我們 trait 的約束。改成每次操作重新綁定（dlopen 有快取，成本遠低於渲染）。
更重要的影響是：**多頁渲染無法平行化**，J2 的效能預算只能靠 `PageCache`
的預抓，不能靠開執行緒。

### 誠實標記
- Opus：**已驗證**（ffprobe 外部確認）
- 文件持久化：**已驗證**（往返測試涵蓋頁面/文字/錄音/索引/墓碑）
- whisper：**編譯與錯誤路徑已測，轉錄品質未驗證**（需模型 + 中文測試集）
- PDFium：**介面與邊界檢查已測，實際解析未驗證**（需 libpdfium 執行期庫）

---

## 2026-09-12 (4) · 文字 CRDT 與 UniFFI 綁定

### 做了什麼
265 → **299 個測試**。`padnote-doc::text`（CRDT + 二進位編碼）、
文字同步端到端驗證（S-16）、`padnote-core::ffi`（UniFFI 門面）、
Swift/Kotlin 綁定產生腳本、XCFramework 建置腳本、CI 綁定閘門。

### 為什麼這樣做

**先做 CRDT 再做 UniFFI。**
CRDT 會改變 core 的 API 表面，反過來做等於要重弄一次綁定。

**自行實作 CRDT 而非引入 yrs（ADR-0004）。**
需求是純文字 —— 粗體、標題、待辦這些屬性住在 `Block` 層級，不在字元流裡。
換到的是與既有 oplog／Lamport 一致、零額外相依、每條收斂性質都有測試。

**FFI 獨立一層，不直接匯出內部型別。**
`Uuid`、`NotebookTime`、trait 物件都過不了 FFI 邊界；更重要的是，
內部重構不該逼著兩個平台的 UI 一起改。這一層是穩定契約。

**`visible_strokes` 只回傳摘要，不回傳取樣點。**
一頁數萬個點跨 FFI 邊界會很慢。渲染資料由平台層直接讀 `.strokes` 檔。

**綁定不進版控。**
與 core API 不同步的綁定比沒有更危險。改 `ffi.rs` 後跑
`scripts/generate-bindings.sh`，CI 也會驗證產得出來。

### 踩到的坑
- **併發插入的測試一開始就寫錯**：忘記讓兩端先套用自己的操作，
  結果是「兩邊各缺一半」而非不收斂。**測試本身錯了比實作錯更難發現** ——
  修正後補上「各自本地結果」的中間斷言，這樣下次失敗會指向正確的地方
- `uniffi` 的 bindgen 需要 `features = ["cli"]`，否則 `uniffi_bindgen_main` 找不到
- `#[derive(uniffi::Object)]` 的結構仍受 `missing_debug_implementations` 檢查

### 驗證結果
S-16 端到端：三站 300 次併發編輯（插入 430 字、刪除 105 字），
經二進位編碼 → 加密 → SyncEngine → 本機資料夾 → 拉取 → 解密 → 解碼 → 套用，
**三台文字完全一致、緩衝區歸零**。另含「刪除與插入交錯」「離線追平」
「同位置併發輸入兩邊都保留」「磁碟上為密文」等迴歸測試。

Swift 綁定已產生並通過 iOS SDK 語法檢查，API 為慣用的 camelCase + throws。

### 下一步
剩下的幾乎都需要原生相依或實機：PDFium、Opus、sherpa-onnx、
Apple Vision 的 `HwrEngine` 實作。

---

## 2026-09-12 (3) · 引擎、模型與 PDF 層

### 做了什麼
205 → **265 個測試**。新增 `recognize::registry`（HWR fallback 鏈）、
`core::setup`（引擎與權限中心）、`padnote-models`（下載器）、
`padnote-pdf`（介面 + LRU 頁面快取），以及機器可讀的 `models/manifest.json`。

### 為什麼這樣做

**權限與引擎狀態機放在 core 而非各平台 UI。**
「錄音要不要麥克風權限」「轉錄缺哪個模型」若寫在 Swift，Android 版要重寫一遍，
兩邊必然長歪。core 判斷、UI 只負責畫。

**`Feature::CoreNotes` 的需求清單刻意是空的。**
手寫與打字不依賴任何權限或模型 —— 全新安裝、零權限的狀態下就該能用。
這是產品定位的基石，用測試把它釘住。

**未回報過的能力預設為 `Unsupported` 而非 `Ready`。**
樂觀假設會讓功能在執行期才爆炸，那時已經來不及給使用者好的錯誤訊息。

**模型清單用 `"sha256": "pending"` 而不是填假雜湊。**
填佔位雜湊看起來「完成度高」，但有人會誤以為可以出貨。現在的行為是
**下載器直接拒絕**，而且有測試確保它連網路請求都不發出。

**網路 IO 抽象成 `Fetcher` trait。**
續傳與驗證邏輯是最容易寫錯、又最難在真機上重現的部分。抽象掉之後，
「伺服器不支援 Range」「回傳空資料」「本機半檔比宣告還大」這些情境
全部可以離線測。

### 踩到的坑
- `OpenOptions::create(true).write(true)` 未指定 `truncate` 會觸發 clippy 警告；
  續傳情境必須明示 `truncate(false)`，否則會毀掉既有進度
- `RenderKey` 用 f32 當 HashMap key 不可行，改存 ×100 的整數

### 下一步
剩下的幾乎都需要原生相依或實機：UniFFI 綁定（S-12）可以做，
PDFium / Opus / sherpa-onnx（S-21/S-13/S-15）能編譯但**無法在此驗證正確性**。

---

## 2026-09-12 (2) · 核心邏輯層

### 做了什麼
新增 6 個實作完整的模組，workspace 從 55 → **205 個測試**：
storage（套件/blob）、ink::geometry、search（CJK 索引）、asr::pipeline、
crypto（信封加密 + 復原碼）、sync::engine、export（MD/SVG）、
core::app（NotebookSession 門面）。

同時建立 `STATE.md` / `DEVLOG.md` / `TODO.md` 交接文件。

### 為什麼這樣做

**先把純邏輯層做完，UI 與模型整合往後放。**
使用者的方針是「先把所有功能建起來再 debug」。純 Rust 邏輯當場可驗證，
而 UI 與 ASR 模型需要實機迭代 —— 先做能驗證的，把不能驗證的變成明確的接手點。

**搜尋自己實作而不直接上 Tantivy。**
中文斷詞才是難點，不是倒排索引。bigram 免詞典、免模型、對未登錄詞（「特徵值」）
不會切錯，代價只是索引較大 —— 本機單機搜尋完全可接受。
`SearchIndex` 介面不變，P2 想換 Tantivy 隨時可以。

**加密用信封而非直接以密語加密內容。**
換密語時只需重新包裹 DEK，不必重新加密整本筆記。測試明確驗證了這點。

### 踩到的坑

**`CloudProvider::list` 的語意錯了。**
原本實作成「列單層目錄」，導致 `SyncEngine::pull` 永遠看不到其他裝置的目錄，
5 個測試同時失敗。正確語意是**依前綴遞迴列舉**（物件儲存語意）。已修正並補測試。

**加密 chunk 少了框架邊界 —— 這是真實缺陷。**
兩次 push 之間只 pull 一次時，讀取範圍會橫跨兩個獨立密文塊，解密必然失敗。
加上 `u32` 長度前綴解決，並讓讀取端只消耗**完整框架**，
寫到一半當機的尾巴留待下次補齊而非當成損毀丟棄。`format-spec.md` §7.1 已更新。

**`b"中文"` 不是合法的 byte string。** 要用 `"中文".as_bytes()`。

### 下一步
S-06/S-10/S-12（辨識註冊表、PDFium、UniFFI 綁定），
以及 S-13/S-15（Opus、ASR 引擎）—— 後者需引入原生相依。

---

## 2026-09-12 · M0 工程地基

### 做了什麼
- `docs/format-spec.md` v0.1 定稿
- Rust workspace 11 crates，55 測試通過、clippy 零警告
- CI：三平台 fmt/clippy/test + cargo-deny + 格式相容性
- S3 同步收斂驗證 **Go**；S4 授權稽核完成
- S1（Swift 墨跡 spike）與 S2（CER 評測工具）程式就緒，待硬體與資料

### 為什麼這樣做

**先做 format-spec 而非先寫 UI。**
統一時間軸是 C1/C4/A10/B6 四個功能的共同地基，事後更動等於重寫文件層。
在還沒有任何程式依賴它之前定稿，成本最低。

**S3 排在 S1 之前完成。**
風險登記簿上 S3 是唯一標「影響極高」的項目 —— 它決定整個「用雲端硬碟同步」
的架構是否成立。而且它純軟體、當場可驗證，不像 S1 要等硬體。先殲滅可驗證的最大風險。

**筆畫不進 CRDT（ADR-0002）。**
一筆畫數百個點，全進 CRDT 會讓 op 爆炸。改用 append-only + 墓碑 —— 新增與刪除
天生可交換冪等，這正是收斂所需的全部性質，不需要完整 CRDT 的機制。

**刻意先不引入外部相依。**
M0 全程零 crates.io 相依，確保任何環境都能 build。等真正需要（serde/sha2）
才引入，且每個都要過 `deny.toml` 白名單。

### 踩到的坑
- 自訂 `Display` 用 `f.write_str` 會忽略 `{:<12}` 寬度 → 必須用 `f.pad()`
- doc comment 裡放真實 Tab 會觸發 clippy 警告
- `cargo` workspace 成員缺 `src/lib.rs` 會讓整個 workspace 無法載入

### 下一步
依「先把功能全部建起來、之後再 debug」的方針，推進 storage / doc / search /
asr 編排 / export 等純邏輯層。需要實機的一律記進 `TODO.md`。
