...
window.KAIRUMO_MANUAL = {
  ...
  "zh-Hant": {
    ...
    ui: {
      ...
      version: "適用版本 v3.8.0（bundle 32）· 2026 年 9 月 13 日",
      tocTitle: "目錄",
    ...
    sections: [
      {
        ...
        steps: [
          ...
          "桌機版視窗左上角會顯示版本號（例如 Kairumo v3.8.0），回報問題時請附上它。"
        ],
      ...
      },
      {
        ...
        faq: [
          ...
          ["怎麼回報問題？", "請附上桌機版視窗左上角顯示的版本號（例如 Kairumo v3.8.0）與操作步驟。"]
        ],
        fig: null
      }
    ]
  ...
  "en": {
    ...
    ui: {
      ...
      version: "For version 3.8.0 (bundle 32) · 13 September 2026",
      tocTitle: "Contents",
    ...
    sections: [
      {
        ...
        steps: [
          ...
          "On desktop the window title shows the version (for example Kairumo v3.8.0) — include it when you report a problem."
        ],
      ...
      },
      {
        ...
        faq: [
          ...
          ["How do I report a problem?", "Include the version shown in the desktop window title (for example Kairumo v3.8.0) and the steps you took."]
        ],
        fig: null
      }
    ]
  ...
  "zh-Hans": {
    ...
    ui: {
      ...
      version: "适用版本 v3.8.0（bundle 32）· 2026 年 9 月 13 日",
      tocTitle: "目录",
    ...
    sections: [
      { id: "start", title: "开始之前", lead: "Kairumo 是一本把手写、打字与录音放在同一条时间轴上的笔记本。完全免费、开放源代码，数据留在你自己的设备上。", buttons: [], steps: ["平板、手机与台式机都能用；手写用触控笔最顺手，用手指或鼠标同样可以写。", "第一次打开不需要注册账号，也不需要联网。", "你写下的每一笔都存在这台设备里。要在多台设备之间同步，就挑一个自己的云端硬盘文件夹（见「数据备份与同步」）。", "台式机版窗口左上角会显示版本号（例如 Kairumo v3.8.0），反馈问题时请附上它。"], tip: "没有服务器、没有账号，就没有“忘记密码”这回事 —— 但也意味着设备丢失时没有云端副本，请自己做备份。", fig: null },
      ...
      { id: "faq", title: "常见问题", lead: "遇到状况时先看这里。", buttons: [], faq: [["在线协作连不上怎么办？", "先确认两台设备在同一个 Wi-Fi。队友的“协作服务器地址”必须填房主画面上显示的 ws://…:9002，而不是默认的 127.0.0.1。首次连接时系统会询问局域网权限，要选允许。"], ["笔记会不会丢失？", "每一笔都实时写入本机文件。同步文件只新增不修改，每台设备只写自己的文件，所以不会互相覆盖。"], ["需要注册账号吗？", "不需要。Kairumo 没有账号系统，也没有后端服务器。"], ["手写可以转成文字吗？", "可以。使用系统内置的设备端识别，不会把笔迹传上网。"], ["怎么反馈问题？", "请附上台式机版窗口左上角显示的版本号（例如 Kairumo v3.8.0）与操作步骤。"]], fig: null }
    ]
  },
  ...
  "ja": {
    ...
    ui: {
      ...
      version: "対象バージョン v3.8.0（bundle 32）· 2026年9月13日",
      tocTitle: "目次",
    ...
    sections: [
      ...
      { id: "faq", title: "困ったときは", lead: "まずここを確認してください。", buttons: [], faq: [["共同編集に接続できない", "両方の端末が同じ Wi-Fi にあるか確認します。参加側の「Relay Server Address」には、ホスト画面に表示された ws://…:9002 を入力します（既定の 127.0.0.1 ではありません）。初回接続時、システムがローカルネットワークの許可を求めるので許可してください。"], ["ノートが消えることは？", "書いたそばからローカルファイルに保存されます。同期ファイルは追記のみで、各端末は自分のファイルしか書かないため上書きは起こりません。"], ["アカウントは必要？", "不要です。Kairumo にはアカウントもバックエンドサーバーもありません。"], ["手書きを文字にできますか？", "できます。端末内蔵の認識機能を使うため、筆跡が外部に出ることはありません。"], ["不具合はどう報告しますか？", "デスクトップ版のウインドウタイトルに表示されるバージョン（例：Kairumo v3.8.0）と、操作手順を添えてください。"]], fig: null }
    ]
  },
  ...
  "ko": {
    ...
    ui: {
      ...
      version: "대상 버전 v3.8.0 (bundle 32) · 2026년 9월 13일",
      tocTitle: "목차",
    ...
    sections: [
      { id: "start", title: "시작하기 전에", lead: "Kairumo는 손글씨·타이핑·녹음을 하나의 타임라인에 올려 두는 노트입니다. 무료이며 오픈 소스이고, 데이터는 기기에 남습니다.", buttons: [], steps: ["태블릿, 스마트폰, 데스크톱에서 사용할 수 있습니다. 스타일러스가 가장 자연스럽지만 손가락이나 마우스로도 됩니다.", "계정 가입도, 인터넷 연결도 필요 없습니다.", "작성한 내용은 이 기기에 저장됩니다. 여러 기기에서 동기화하려면 본인 클라우드의 폴더를 지정하세요(「백업 및 동기화」 참고).", "데스크톱에서는 창 제목에 버전(예: Kairumo v3.8.0)이 표시됩니다. 문제를 알릴 때 함께 적어 주세요."], tip: "서버도 계정도 없으므로 비밀번호를 잊을 일이 없습니다. 대신 클라우드 사본도 없으니 백업은 직접 해 두세요.", fig: null },
  ...
  "th": {
    ...
    ui: {
      ...
      version: "สำหรับเวอร์ชัน v3.8.0 (bundle 32) · 13 กันยายน 2026",
      tocTitle: "สารบัญ",
