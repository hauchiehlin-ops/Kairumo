...
//!
//! ## 這條路現在是後備，不是兩個平台的主線（工作項 S-60）
//!
//! 灰條在縮圖上說得過去，在**使用者匯出的 PNG** 上說不過去 —— 那張圖是要
//! 給人看的，字不能是灰條。兩個平台因此都改走「核心產 PDF、系統算繪」：
//! Apple 用 PDFKit（`PageImageRenderer.swift`），Android 用系統的
//! `PdfRenderer`（`PageImageRenderer.kt`）。核心的 PDF 匯出器本來就輸出真正的
//! 文字物件，而兩邊的系統算繪器都會替非內嵌字型代換系統字型 ——
//! **一個位元組的字型都不用內嵌就有真字形**，那個授權決策因此不必做。
//!
//! 這裡仍然是兩邊的後備（系統算繪失敗時），也是測試與命令列唯一的路，
//! 所以灰條要繼續把版面算對。
