...
fn cells_tile_without_gaps_or_overlap() {
    ...
    assert!(
        (first.x + first.width - second.x).abs() < 1e-9,
        "兩格之間有縫或重疊"
    );
}
...
fn a_taller_cell_makes_the_whole_row_taller() {
    ...
    assert!(
        (l.cells[0].height - l.cells[1].height).abs() < 1e-9,
        "同一列的格子高度不一致"
    );
...
fn covered_cells_are_not_emitted() {
    ...
    assert!(
        !l.cells.iter().any(|c| c.row == 0 && c.col == 1),
        "被合併蓋住的格子不該畫出來"
    );
...
fn a_zero_sized_table_produces_nothing_instead_of_panicking() {
    assert!(
        layout(0, 3, &[], false, &[], 400.0, TableStyle::default())
            .cells
            .is_empty()
    );
    assert!(
        layout(3, 0, &[], false, &[], 400.0, TableStyle::default())
            .cells
            .is_empty()
    );
    assert!(
        layout(2, 2, &[], false, &[], 0.0, TableStyle::default())
            .cells
            .is_empty()
    );
}
