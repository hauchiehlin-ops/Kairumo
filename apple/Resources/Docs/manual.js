...
window.KAIRUMO_MANUAL = {
  ...
  "zh-Hant": {
    ...
    ui: {
      ...
      version: "適用版本 v2.10.0（bundle 23）· 2026 年 9 月 13 日",
      tocTitle: "目錄",
    ...
    sections: [
      {
        ...
        steps: [
          ...
          "桌機版視窗左上角會顯示版本號（例如 Kairumo v2.10.0），回報問題時請附上它。"
        ],
      ...
      },
      {
        ...
        tip: "用觸控筆書寫時可以直接把手掌放在螢幕上 —— 掌拒會忽略手掌，只認筆尖。",
        fig: "editor",
      ...
      },
      {
        ...
        tip: "打字模式下畫布只認觸控筆，手指的滑動會當成捲動 —— 不會誤畫到線。",
        fig: "typing",
      ...
      },
      {
        ...
        steps: [
          ...
          "「形狀與流程圖」含基本形狀與 ISO 5807 的流程圖符號，另有內建範本會一次放好節點與連接線。",
          "「算式計算」把算式變成卡片放進筆記。",
      ...
      },
      {
        ...
        buttons: ["建立備份", "從備份復原", "選擇資料夾", "立即同步"],
        steps: [
          "首頁的「資料與同步」區塊裡有三張卡片，全部的動作都在那裡。",
          "「建立備份」把所有筆記、圖片與錄音打包成一個檔案，存到你指定的位置。",
          "「從備份復原」挑一個備份檔還原。復原前會先自動備份目前的資料，選錯檔案也救得回來。",
          "同步：先在自己的雲端硬碟裡建一個資料夾（例如叫 Kairumo），按「選擇資料夾」指到它。",
          "在另一台裝置上做同樣的事，指到**同一個**資料夾。",
          "兩邊各按一次「立即同步」。A 裝置寫的東西，B 裝置按下同步之後就會出現。"
        ],
        tip: "檔案的搬運交給你原本就在用的雲端硬碟。我們不碰網路，也沒有你的資料 —— 這也是為什麼找不到「登入」。同步只會新增檔案，不會覆蓋別台裝置寫的內容。",
        fig: null
      },
      {
        id: "comment",
      ...
      },
      {
        ...
        buttons: [],
        faq: [
          ["線上協同連不上怎麼辦？", "先確認兩台裝置在同一個 Wi-Fi。隊友的「協同伺服器位址」必須填房主畫面上顯示的 ws://…:9002，而不是預設的 127.0.0.1。首次連線時系統會詢問區域網路權限，要選允許。"],
          ...
          ["怎麼回報問題？", "請附上桌機版視窗左上角顯示的版本號（例如 Kairumo v2.10.0）與操作步驟。"]
        ],
        fig: null
      }
    ]
  ...
  "en": {
    ...
    ui: {
      ...
      version: "For version 2.10.0 (bundle 23) · 13 September 2026",
      tocTitle: "Contents",
    ...
    sections: [
      {
        ...
        steps: [
          ...
          "On desktop the window title shows the version (for example Kairumo v2.10.0) — include it when you report a problem."
        ],
      ...
      },
      {
        ...
        buttons: ["Fountain Pen", "Eraser", "Lasso", "Stroke Width"],
        steps: [
        ...
        tip: "With a stylus you can rest your palm on the screen — palm rejection ignores it and follows only the tip.",
        fig: "editor",
      ...
      },
      {
        ...
        tip: "In typing mode the canvas only accepts stylus strokes, so a finger swipe scrolls instead of drawing a stray line.",
        fig: "typing",
      ...
      },
      {
        ...
        steps: [
          ...
          "“Shapes & Flowcharts” includes basic shapes and the ISO 5807 flowchart symbols, plus templates that place the nodes and connectors for you.",
      ...
      },
      {
        ...
        buttons: ["Create Backup", "Restore from Backup", "Choose Folder", "Sync Now"],
        steps: [
          "The “Data & Sync” block on the home screen holds all three cards.",
          ...
          "To sync: create a folder in your own cloud drive (call it Kairumo, say) and point “Choose Folder” at it.",
        ...
        tip: "Moving the files is your cloud drive's job — the one you already use. We never touch the network and never hold your data, which is why there is no “Sign in”. Syncing only adds files; it never overwrites what another device wrote.",
        fig: null
      },
      {
        id: "comment",
      ...
      },
      {
        ...
        buttons: [],
        faq: [
          ["Collaboration won't connect", "Make sure both devices are on the same Wi-Fi. The guest's “Relay Server Address” must be the ws://…:9002 address shown on the host's screen, not the default 127.0.0.1. On the first connection the system asks for local network permission — allow it."],
          ...
          ["How do I report a problem?", "Include the version shown in the desktop window title (for example Kairumo v2.10.0) and the steps you took."]
        ],
        fig: null
      }
    ]
  ...
  "zh-Hans": {
    ...
    ui: {
      ...
      version: "适用版本 v2.10.0（bundle 23）· 2026 年 9 月 13 日",
      tocTitle: "目录",
    ...
    sections: [
      { id: "start", title: "开始之前", lead: "Kairumo 是一本把手写、打字与录音放在同一条时间轴上的笔记本。完全免费、开放源代码，数据留在你自己的设备上。", buttons: [], steps: ["平板、手机与台式机都能用；手写用触控笔最顺手，用手指或鼠标同样可以写。", "第一次打开不需要注册账号，也不需要联网。", "你写下的每一笔都存在这台设备里。要在多台设备之间同步，就挑一个自己的云端硬盘文件夹（见「数据备份与同步」）。", "台式机版窗口左上角会显示版本号（例如 Kairumo v2.10.0），反馈问题时请附上它。"], tip: "没有服务器、没有账号，就没有“忘记密码”这回事 —— 但也意味着设备丢失时没有云端副本，请自己做备份。", fig: null },
      ...
      { id: "write", title: "手写", lead: "七种笔、四挡粗细、可自定义颜色。写错可以擦，也可以整段圈起来搬家。", buttons: ["钢笔", "橡皮擦", "套索选取", "笔迹粗细"], steps: ["确认上排停在“手绘模式”。", "从工具栏挑一支笔：钢笔、圆珠笔、毛笔、马克笔、荧光笔、铅笔、水彩笔。", "工具栏中间的四颗圆点选粗细，右边的色盘选颜色；点色盘旁的滑杆图标可自定义任何颜色。", "写错时选“橡皮擦”擦掉；想搬动或复制一整段笔迹，用“套索选取”圈起来再操作。", "最右边是撤销、重做与清除本页。", "写到页面底部时会自动新增下一页，不必手动延长。"], tip: "用触控笔书写时可以直接把手掌放在屏幕上 —— 掌拒会忽略手掌，只认笔尖。", fig: "editor", cap: "手绘工具栏：笔刷、粗细、色盘与更多" },
      { id: "type", title: "打字", lead: "在画布上任何位置放文本框，和手写混排。", buttons: ["打字模式", "文字排版", "特殊符号", "插入链接"], steps: ["点上排的键盘图标，切换到“打字模式”。", "在画布上任何一处点一下，该位置就会出现文本框并弹出键盘。", "点“文字排版”可以调字号、粗体、斜体、下划线、对齐与文字颜色。", "“特殊符号”可插入数学与常用符号；“插入链接”粘贴网址后会变成可点击的预览卡片。", "文本框可以直接拖动，拖右下角的把手可以改变大小。"], tip: "打字模式下画布只认触控笔，手指滑动会当成滚动 —— 不会误画出线条。", fig: "typing", cap: "打字模式工具栏：文字排版、特殊符号、插入链接与更多" },
      ...
      { id: "data", title: "数据备份与同步", lead: "不需要账号，也没有我们的服务器。备份是一个文件，同步是一个你自己的云端文件夹。", buttons: ["创建备份", "从备份恢复", "选择文件夹", "立即同步"], steps: ["首页的“数据与同步”区块里有三张卡片。", "“创建备份”把所有笔记、图片与录音打包成一个文件。", "“从备份恢复”会先自动备份当前数据，选错文件也救得回来。", "同步：在自己的云端硬盘建一个文件夹，按“选择文件夹”指到它；另一台设备指到同一个文件夹。", "两边各按一次“立即同步”。"], tip: "文件搬运交给你原本就在用的云端硬盘。我们不碰网络，也没有你的数据 —— 这也是为什么找不到“登录”。", fig: null },
      ...
      { id: "faq", title: "常见问题", lead: "遇到状况时先看这里。", buttons: [], faq: [["在线协作连不上怎么办？", "先确认两台设备在同一个 Wi-Fi。队友的“协作服务器地址”必须填房主画面上显示的 ws://…:9002，而不是默认的 127.0.0.1。首次连接时系统会询问局域网权限，要选允许。"], ["笔记会不会丢失？", "每一笔都实时写入本机文件。同步文件只新增不修改，每台设备只写自己的文件，所以不会互相覆盖。"], ["需要注册账号吗？", "不需要。Kairumo 没有账号系统，也没有后端服务器。"], ["手写可以转成文字吗？", "可以。使用系统内置的设备端识别，不会把笔迹传上网。"], ["怎么反馈问题？", "请附上台式机版窗口左上角显示的版本号（例如 Kairumo v2.10.0）与操作步骤。"]], fig: null }
    ]
  },
  ...
  "ja": {
    ...
    ui: {
      ...
      version: "対象バージョン v2.10.0（bundle 23）· 2026年9月13日",
      tocTitle: "目次",
    ...
    sections: [
      { id: "start", title: "はじめる前に", lead: "Kairumo は手書き・タイピング・録音を同じタイムライン上に置くノートです。完全無料、オープンソース、データは端末内に留まります。", buttons: [], steps: ["タブレット・スマートフォン・デスクトップに対応。スタイラスが最適ですが、指やマウスでも書けます。", "アカウント登録もインターネット接続も不要です。", "書いた内容はこの端末に保存されます。複数の端末で同期するには、ご自身のクラウドのフォルダを指定してください（「バックアップと同期」参照）。", "デスクトップ版ではウインドウのタイトルにバージョン（例：Kairumo v2.10.0）が表示されます。不具合報告の際は添えてください。"], tip: "サーバーもアカウントもないためパスワードを忘れる心配はありません。その代わりクラウド上の控えもないので、バックアップはご自身で。", fig: null },
      ...
      { id: "write", title: "手書き", lead: "7種類のペン、4段階の太さ、自由な色。消すことも、まとめて囲んで動かすこともできます。", buttons: ["Fountain Pen", "Eraser", "Lasso", "Stroke Width"], steps: ["上部が手書きモードになっていることを確認します。", "ペンを選びます：万年筆、ボールペン、筆、マーカー、蛍光ペン、鉛筆、水彩。", "中央の4つの丸が太さ、右側が色です。スライダーのアイコンで自由に色を作れます。", "消すときは消しゴム。まとめて移動・複製するときは投げ縄で囲みます。", "右端に取り消し・やり直し・ページ消去があります。", "ページの下端まで書くと自動的に次のページが追加されます。"], tip: "スタイラスなら手のひらを画面に置いたままで大丈夫です。パームリジェクションがペン先だけを拾います。", fig: "editor", cap: "手書きツールバー：ブラシ、太さ、色、ページ延長、その他" },
      { id: "type", title: "タイピング", lead: "キャンバスの好きな位置にテキストボックスを置き、手書きと混在させられます。", buttons: ["Typing", "Text Studio", "Special Symbols", "Insert Link"], steps: ["上部のキーボードアイコンで入力モードに切り替えます。", "キャンバスをタップすると、その位置にテキストボックスが現れてキーボードが開きます。", "「Text Studio」で文字サイズ・太字・斜体・下線・揃え・色を設定します。", "「Special Symbols」は数式や記号、「Insert Link」は貼り付けた URL をプレビューカードにします。", "テキストボックスはドラッグで移動、右下のハンドルでサイズ変更できます。"], tip: "入力モードではキャンバスはスタイラスのみを筆記として扱うため、指のスワイプは線にならずスクロールになります。", fig: "typing", cap: "入力モードのツールバー：Text Studio、記号、リンク挿入、その他、ページ延長" },
      ...
      { id: "data", title: "バックアップと同期", lead: "アカウントも当方のサーバーも不要です。バックアップは 1 つのファイル、同期はご自身のクラウドフォルダです。", buttons: ["Create Backup", "Restore from Backup", "Choose Folder", "Sync Now"], steps: ["ホーム画面の「データと同期」に 3 枚のカードがあります。", "「Create Backup」はノート・画像・録音を 1 つのファイルにまとめます。", "「Restore from Backup」は復元前に現在のデータを自動でバックアップします。", "同期：ご自身のクラウドに作ったフォルダを「Choose Folder」で指定し、もう一方の端末でも同じフォルダを指定します。", "両方で「Sync Now」を押します。"], tip: "ファイルの運搬は普段お使いのクラウドが担当します。当方はネットワークに触れず、データも保持しません。だから「サインイン」がありません。", fig: null },
      ...
      { id: "faq", title: "困ったときは", lead: "まずここを確認してください。", buttons: [], faq: [["共同編集に接続できない", "両方の端末が同じ Wi-Fi にあるか確認します。参加側の「Relay Server Address」には、ホスト画面に表示された ws://…:9002 を入力します（既定の 127.0.0.1 ではありません）。初回接続時、システムがローカルネットワークの許可を求めるので許可してください。"], ["ノートが消えることは？", "書いたそばからローカルファイルに保存されます。同期ファイルは追記のみで、各端末は自分のファイルしか書かないため上書きは起こりません。"], ["アカウントは必要？", "不要です。Kairumo にはアカウントもバックエンドサーバーもありません。"], ["手書きを文字にできますか？", "できます。端末内蔵の認識機能を使うため、筆跡が外部に出ることはありません。"], ["不具合はどう報告しますか？", "デスクトップ版のウインドウタイトルに表示されるバージョン（例：Kairumo v2.10.0）と、操作手順を添えてください。"]], fig: null }
    ]
  },
  ...
  "ko": {
    ...
    ui: {
      ...
      version: "대상 버전 v2.10.0 (bundle 23) · 2026년 9월 13일",
      tocTitle: "목차",
    ...
    sections: [
      { id: "start", title: "시작하기 전에", lead: "Kairumo는 손글씨·타이핑·녹음을 하나의 타임라인에 올려 두는 노트입니다. 무료이며 오픈 소스이고, 데이터는 기기에 남습니다.", buttons: [], steps: ["태블릿, 스마트폰, 데스크톱에서 사용할 수 있습니다. 스타일러스가 가장 자연스럽지만 손가락이나 마우스로도 됩니다.", "계정 가입도, 인터넷 연결도 필요 없습니다.", "작성한 내용은 이 기기에 저장됩니다. 여러 기기에서 동기화하려면 본인 클라우드의 폴더를 지정하세요(「백업 및 동기화」 참고).", "데스크톱에서는 창 제목에 버전(예: Kairumo v2.10.0)이 표시됩니다. 문제를 알릴 때 함께 적어 주세요."], tip: "서버도 계정도 없으므로 비밀번호를 잊을 일이 없습니다. 대신 클라우드 사본도 없으니 백업은 직접 해 두세요.", fig: null },
      ...
      { id: "data", title: "백업 및 동기화", lead: "계정도, 저희 서버도 필요 없습니다. 백업은 파일 하나, 동기화는 본인의 클라우드 폴더입니다.", buttons: ["Create Backup", "Restore from Backup", "Choose Folder", "Sync Now"], steps: ["홈 화면의 “데이터 및 동기화”에 카드 세 개가 있습니다.", "“Create Backup”은 노트·이미지·녹음을 파일 하나로 묶습니다.", "“Restore from Backup”은 복원 전에 현재 데이터를 자동으로 백업합니다.", "동기화: 본인 클라우드에 폴더를 만들고 “Choose Folder”로 지정한 뒤, 다른 기기에서도 같은 폴더를 지정합니다.", "양쪽에서 “Sync Now”를 누릅니다."], tip: "파일 이동은 이미 쓰고 계신 클라우드가 맡습니다. 저희는 네트워크를 건드리지 않고 데이터도 보관하지 않습니다. 그래서 “로그인”이 없습니다.", fig: null },
      ...
      { id: "faq", title: "문제 해결", lead: "먼저 여기를 확인하세요.", buttons: [], faq: [["협업이 연결되지 않아요", "두 기기가 같은 Wi-Fi에 있는지 확인하세요. 참여자의 “Relay Server Address”에는 호스트 화면에 표시된 ws://…:9002를 넣어야 하며 기본값 127.0.0.1이 아닙니다. 첫 연결 때 시스템이 로컬 네트워크 권한을 물으면 허용하세요."], ["노트가 사라질 수 있나요?", "쓰는 즉시 로컬 파일에 저장됩니다. 동기화 파일은 추가만 하며 각 기기는 자기 파일만 쓰기 때문에 덮어쓰기가 일어나지 않습니다."], ["계정이 필요한가요?", "아니요. Kairumo에는 계정도, 백엔드 서버도 없습니다."], ["손글씨를 글자로 바꿀 수 있나요?", "가능합니다. 기기 내장 인식 기능을 사용하므로 필기가 외부로 나가지 않습니다."], ["문제는 어떻게 알리나요?", "데스크톱 창 제목에 보이는 버전(예: Kairumo v2.10.0)과 진행한 단계를 함께 알려 주세요."]], fig: null }
    ]
  },
  ...
  "th": {
    ...
    ui: {
      ...
      version: "สำหรับเวอร์ชัน v2.10.0 (bundle 23) · 13 กันยายน 2026",
      tocTitle: "สารบัญ",
    ...
    sections: [
      { id: "start", title: "ก่อนเริ่มต้น", lead: "Kairumo คือสมุดจดที่วางลายมือ ข้อความ และเสียงไว้บนไทม์ไลน์เดียวกัน ใช้ฟรี เป็นโอเพนซอร์ส และข้อมูลอยู่ในเครื่องของคุณ", buttons: [], steps: ["ใช้ได้บนแท็บเล็ต โทรศัพท์ และเดสก์ท็อป เขียนด้วยสไตลัสลื่นที่สุด แต่ใช้นิ้วหรือเมาส์ก็ได้", "ไม่ต้องสมัครบัญชีและไม่ต้องต่ออินเทอร์เน็ต", "ทุกอย่างที่คุณเขียนถูกเก็บไว้ในเครื่องนี้ หากต้องการซิงค์ระหว่างหลายเครื่อง ให้ชี้ไปที่โฟลเดอร์บนคลาวด์ของคุณเอง (ดู “สำรองข้อมูลและซิงค์”)", "บนเดสก์ท็อป ชื่อหน้าต่างจะแสดงเวอร์ชัน (เช่น Kairumo v2.10.0) โปรดแจ้งมาด้วยเมื่อรายงานปัญหา"], tip: "เมื่อไม่มีเซิร์ฟเวอร์และไม่มีบัญชี ก็ไม่มีรหัสผ่านให้ลืม แต่ก็ไม่มีสำเนาบนคลาวด์เช่นกัน จึงควรสำรองข้อมูลเอง", fig: null },
      ...
      { id: "data", title: "สำรองข้อมูลและซิงค์", lead: "ไม่ต้องมีบัญชี และไม่มีเซิร์ฟเวอร์ของเรา การสำรองข้อมูลคือไฟล์เดียว ส่วนการซิงค์คือโฟลเดอร์บนคลาวด์ของคุณเอง", buttons: ["Create Backup", "Restore from Backup", "Choose Folder", "Sync Now"], steps: ["บล็อก “ข้อมูลและการซิงค์” บนหน้าแรกมีการ์ดสามใบ", "“Create Backup” รวมโน้ต รูปภาพ และเสียงไว้ในไฟล์เดียว", "“Restore from Backup” จะสำรองข้อมูลปัจจุบันให้ก่อนกู้คืน", "ซิงค์: สร้างโฟลเดอร์บนคลาวด์ของคุณ แล้วกด “Choose Folder” ชี้ไปที่โฟลเดอร์นั้น และทำแบบเดียวกันบนอีกเครื่องโดยชี้ไปโฟลเดอร์เดียวกัน", "กด “Sync Now” ทั้งสองเครื่อง"], tip: "การย้ายไฟล์เป็นหน้าที่ของคลาวด์ที่คุณใช้อยู่แล้ว เราไม่แตะเครือข่ายและไม่เก็บข้อมูลของคุณ จึงไม่มีปุ่ม “ลงชื่อเข้าใช้”", fig: null },
      ...
      { id: "faq", title: "แก้ปัญหาที่พบบ่อย", lead: "ตรวจที่นี่ก่อน", buttons: [], faq: [["เชื่อมต่อการทำงานร่วมกันไม่ได้", "ตรวจว่าทั้งสองเครื่องอยู่บน Wi-Fi เดียวกัน ช่อง “Relay Server Address” ของผู้เข้าร่วมต้องเป็น ws://…:9002 ตามที่แสดงบนหน้าจอผู้เริ่ม ไม่ใช่ค่าเริ่มต้น 127.0.0.1 และเมื่อเชื่อมต่อครั้งแรก ระบบจะขออนุญาตใช้เครือข่ายท้องถิ่น ให้กดอนุญาต"], ["โน้ตหายได้ไหม", "ทุกเส้นถูกบันทึกลงไฟล์ในเครื่องทันทีที่เขียน ไฟล์ซิงก์เป็นแบบเพิ่มอย่างเดียว และแต่ละเครื่องเขียนเฉพาะไฟล์ของตัวเอง จึงไม่ทับกัน"], ["ต้องมีบัญชีไหม", "ไม่ต้อง Kairumo ไม่มีระบบบัญชีและไม่มีเซิร์ฟเวอร์เบื้องหลัง"], ["แปลงลายมือเป็นข้อความได้ไหม", "ได้ โดยใช้การรู้จำที่ทำงานในเครื่อง ลายมือไม่ถูกส่งออกไปที่ใด"], ["รายงานปัญหาอย่างไร", "โปรดแจ้งเวอร์ชันที่แสดงบนชื่อหน้าต่างของเดสก์ท็อป (เช่น Kairumo v2.10.0) พร้อมขั้นตอนที่ทำ"]], fig: null }
    ]
  }
};
